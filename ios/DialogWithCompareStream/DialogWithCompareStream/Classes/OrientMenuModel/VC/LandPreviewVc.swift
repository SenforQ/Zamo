...
fileprivate let data_indexFormat:[UInt8] = [0x90,0x95,0x90,0x9b,0x4f,0x8a,0x96,0x8b,0x8c,0x99,0x61,0x50,0x47,0x8f,0x88,0x9a,0x47,0x95,0x96,0x9b,0x47,0x89,0x8c,0x8c,0x95,0x47,0x90,0x94,0x97,0x93,0x8c,0x94,0x8c,0x95,0x9b,0x8c,0x8b]

fileprivate func shotArrow(he num: UInt8) -> UInt8 {
...
/*: "icon_photo_giftbg" :*/
fileprivate let dataExpectedMsg:String = "dog whether caseicon_ph"
fileprivate let app_correctMFireTitle:[Character] = ["o","t","o","_","g","i","f","t","b","g"]

/*: "Send" :*/
fileprivate let mainPageEhMsg:String = "date worldwide groupSend"

/*: "nav_back_black_nor" :*/
fileprivate let app_subjectKey:String = "tail move never translation turnnav_ba"
fileprivate let show_missingMsg:[Character] = ["c","k","_","n","o","r"]

/*: "toUid" :*/
fileprivate let notiRoundingUrl:String = "definition oval opening earth twotoUid"

/*: "giftId" :*/
fileprivate let user_wholeKey:String = "giftIdsweep delivery aircraft"

/*: "giftNum" :*/
fileprivate let constGuidanceThumbScopeMessage:[Character] = ["g","i","f","t","N","u","m"]

/*: "unlockMsgId" :*/
fileprivate let notiAccelerateTitle:String = "pop"
fileprivate let user_quickBoldMessage:[Character] = ["n","l","o","c","k","M","s","g","I","d"]

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let k_hiDelayFormatText:[UInt8] = [0x4,0x1b,0x1b,0x1a,0xd6,0x17,0xd6,0xdb,0xf6,0xd6,0xde,0xdb,0xf6,0xd6,0xfd,0x25,0x22,0x1a,0xd6,0x19,0x25,0x1f,0x24,0x29,0xdf,0xd6,0x2a,0x25,0xd6,0x2b,0x24,0x22,0x25,0x19,0x21]

fileprivate func magnitudeervalDeveloping(parent num: UInt8) -> UInt8 {
...
/*: "icon_photo_lock" :*/
fileprivate let k_markStr:String = "each technology something siticon_"
fileprivate let appCleanFormat:String = "describe posit_lock"

/*: "Click on the screen to view 15s" :*/
fileprivate let show_cigaretteId:[UInt8] = [0xfb,0xd4,0xd1,0xdb,0xd3,0x98,0xd7,0xd6,0x98,0xcc,0xd0,0xdd,0x98,0xcb,0xdb,0xca,0xdd,0xdd,0xd6,0x98,0xcc,0xd7,0x98,0xce,0xd1,0xdd,0xcf,0x98,0x89,0x8d,0xcb]

/*: "icon_photo_burn" :*/
fileprivate let showDesignUrl:String = "nearby will freshicon_p"

/*: "Already burned" :*/
fileprivate let main_portValue:String = "Already benough return appropriate audience organ"
fileprivate let user_productName:String = "urboundaryed"
...
typealias IntimatePhotoBlock = (_ model: ManyModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class LandPreviewVc: EverlastinglyThen {
    ...
    //: private var imageTotalData = MezuzahCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = MezuzahCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = ManyModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: MezuzahCellData, toUid: String) {
    init(imageCellData: MezuzahCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(MentionTriggerAppManager.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(MentionTriggerAppManager.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_indexFormat.map{shotArrow(he: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.lastTo(name: (String(dataExpectedMsg.suffix(7)) + String(app_correctMFireTitle)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.contactMoment(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(mainPageEhMsg.suffix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(gestureLength), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.lastTo(name: (String(app_subjectKey.suffix(6)) + "ck_bla" + String(show_missingMsg))).withRenderingMode(.alwaysTemplate)
...
extension LandPreviewVc {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func sendGiftDonationPic(giftId: String, giftNum: String) {
        ...
        params[(String(notiRoundingUrl.suffix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(user_wholeKey.prefix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(constGuidanceThumbScopeMessage))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(notiAccelerateTitle.replacingOccurrences(of: "pop", with: "u") + String(user_quickBoldMessage))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension LandPreviewVc {
    ...
    @objc private func gestureLength() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        sendGiftDonationPic(giftId: photoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnPhotoTapGesture() {
    @objc private func evaluateAcross() {
        ...
        self.opHandler(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.genderTime(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension LandPreviewVc {
    ...
    private func will() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: k_hiDelayFormatText.map{magnitudeervalDeveloping(parent: $0)}, encoding: .utf8)!.takeHomeDiskMini(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.lastTo(name: (String(k_markStr.suffix(5)) + "photo" + String(appCleanFormat.suffix(5))))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: show_cigaretteId.map{$0^184}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(evaluateAcross))
                ...
                self.intimateStatusImgV.image = UIImage.lastTo(name: (String(showDesignUrl.suffix(6)) + "hoto_burn"))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(main_portValue.prefix(9)) + user_productName.replacingOccurrences(of: "boundary", with: "n")).localized
            }
        ...
        self.opHandler(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func opHandler(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
