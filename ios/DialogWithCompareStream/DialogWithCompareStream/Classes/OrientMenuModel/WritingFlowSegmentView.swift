...
fileprivate let data_boardScopeTitle:String = "设置普通状"
fileprivate let constTreatUrl:String = "态的文\u{5b57}颜\u{8272}"
fileprivate let notiMakeName:String = "RGB空间的颜色值may document string"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let user_maximumPath:String = "entity选中状"
fileprivate let data_aboutTitle:String = "色时 请\u{4f7f}用RG"
fileprivate let app_heatMsg:[Character] = ["B","空","间","的","\u{989c}","色","值"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let notiRatingKey:[UInt8] = [0xd4,0xd3,0xd4,0xc9,0x95,0xde,0xd2,0xd9,0xd8,0xcf,0x87,0x94,0x9d,0xd5,0xdc,0xce,0x9d,0xd3,0xd2,0xc9,0x9d,0xdf,0xd8,0xd8,0xd3,0x9d,0xd4,0xd0,0xcd,0xd1,0xd8,0xd0,0xd8,0xd3,0xc9,0xd8,0xd9]

private func threadPrevious(flag num: UInt8) -> UInt8 {
    return num ^ 189
}

/*: "设置的下标不合法!!" :*/
fileprivate let kFarKey:String = "设置的下size"
fileprivate let userUponData:String = "\u{5408}\u{6cd5}!!"

/*: "Bags" :*/
fileprivate let const_dotMessage:String = "minimize delay bring missing guardBags"
...
open class WritingFlowSegmentView: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: TipSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.arcRgb(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((data_boardScopeTitle.capitalized + constTreatUrl + "时 请使用" + String(notiMakeName.prefix(9))))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.arcRgb(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((user_maximumPath.replacingOccurrences(of: "entity", with: "设置") + "态的文\u{5b57}颜" + data_aboutTitle + String(app_heatMsg)))
        }
    ...
    fileprivate func arcRgb(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: TipSegmentStyle, titles: [String]) {
    ...
    @objc func streetName(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? HomeCustomLabel else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiRatingKey.map{threadPrevious(flag: $0)}, encoding: .utf8)!)
    }
}
...
public extension WritingFlowSegmentView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func materialAnimated(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (kFarKey.replacingOccurrences(of: "size", with: "标不") + userUponData))
    ...
    func noneSound(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        materialAnimated(0, animated: true)
    }
}
...
//: extension ScrollSegmentView {
extension WritingFlowSegmentView {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func weight() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = HomeCustomLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.streetName(_:)))
            ...
            if title == (String(const_dotMessage.suffix(4))).localized {
    ...
    fileprivate func reason() {
        ...
        if let firstLabel = labelsArray[0] as? HomeCustomLabel {
    ...
    func used(_: Bool) {
        ...
        if currentIndex == titles.firstIndex(of: (String(const_dotMessage.suffix(4))).localized) {
            //: redBagsLab.isHidden = true
            redBagsLab.isHidden = true
            //: TalkingChatGiftManager.share.showBagsRed = false
            RugReactiveCompatible.share.showBagsRed = false
        }

        //: guard currentIndex != oldIndex else { return }
        guard currentIndex != oldIndex else { return }
        //: let oldLabel = labelsArray[oldIndex] as! CustomLabel
        let oldLabel = labelsArray[oldIndex] as! HomeCustomLabel
        //: let currentLabel = labelsArray[currentIndex] as! CustomLabel
        let currentLabel = labelsArray[currentIndex] as! HomeCustomLabel
        // 让选中标签居中显示
        //: adjustTitleOffSetToCurrentIndex(currentIndex)
        etymology(currentIndex)
        // 动画效果
    ...
    public func etymology(_ currentIndex: Int) {
...
open class HomeCustomLabel: UILabel {
    /// 用来记录当前label的缩放比例
