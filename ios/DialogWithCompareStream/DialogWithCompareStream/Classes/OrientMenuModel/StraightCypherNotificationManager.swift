...
/*: "rYMsgType" :*/
fileprivate let showJungleName:[Character] = ["r","Y"]
fileprivate let app_requestText:[Character] = ["M","s","g","T","y","p","e"]

/*: "GJ:CallCustom" :*/
fileprivate let show_threeStr:String = "feature decreaseGJ:Ca"
fileprivate let mainPubliclyChiefTitle:String = "M"
...
// __CLOSE_PRINT__
//
//  StraightCypherNotificationManager.swift
//  AbroadTalking
//
...
class StraightCypherNotificationManager: NSObject {
    ...
    static var shared = StraightCypherNotificationManager()
    ...
    private lazy var msgArr: [AutomaticThen] = //: return [AutomaticThen]()
        .init()
    //: }()
}
...
extension StraightCypherNotificationManager {
    /// 处理接收到的TX消息，展示消息通知
    /// - Parameters:
    ///   - msg: V2TIMMessage
    ///   - dataDict: 消息字典
    //: func onRecvNewMessage(msg: V2TIMMessage, dataDict: [String: Any]) {
    func leave(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        guard displayableDecision() == true || screenChoice() == true else { return }
        //: guard msg.userID != EverlastinglyReactiveCompatible.getCustomerServiceID() && msg.userID != EverlastinglyReactiveCompatible.getXiaoMiID() else {
        guard msg.userID != EverlastinglyReactiveCompatible.breakup(), msg.userID != EverlastinglyReactiveCompatible.layChild() else {
            //: return
            return
        }
        //: let contenType = TUISocialChatManager.messageCellType(dataDict)
        let contenType = ChatManager.explainTelephoneMessage(dataDict)
        // 提示消息
        ...
        if contenType == .TXMessageContentType_text, dataDict[(String(showJungleName) + String(app_requestText))] as? String == (String(show_threeStr.suffix(5)) + "llCusto" + mainPubliclyChiefTitle.lowercased()) {
            //: return
            return
        }

        // 2. V2TIMMessage => AutomaticThen
        ...
        //: if let userInfo: PronkCacheModel = DBUserInfoManager.cache_getCachedUserInfo(targetId: model.targetId) {
        if let userInfo: PronkCacheModel = BoundPackInfoManager.terms(targetId: model.targetId) {
    ...
    private func displayableDecision() -> Bool {
        //: guard let currentVC = currentViewController() else { return false }
        guard let currentVC = transplant() else { return false }
        //: if currentVC.isKind(of: TalkingVideoChatViewController.self) ||
        if currentVC.isKind(of: InPublicViewDelegate.self) ||
            //: currentVC.isKind(of: TalkingVoiceChatViewController.self) {
            currentVC.isKind(of: EyebrowTopicViewController.self)
        {
            //: return true
            return true
        }
