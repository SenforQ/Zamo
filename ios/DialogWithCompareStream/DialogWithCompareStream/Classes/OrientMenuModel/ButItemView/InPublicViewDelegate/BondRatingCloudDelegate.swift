...
/*: "mf/call/GetUserInfo" :*/
fileprivate let userMeKey:String = "con in suspend only effectmf/c"
fileprivate let userHelloValue:[Character] = ["e","r","I","n","f","o"]

/*: "uid" :*/
fileprivate let showSituationKey:[UInt8] = [0x5d,0x51,0x4c]

fileprivate func modeHazard(hormone num: UInt8) -> UInt8 {
...
/*: "TRTC 随机视频匹配中." :*/
fileprivate let appReactPath:[Character] = ["T","R","T","C"," ","\u{968f}","机","\u{89c6}","\u{9891}","\u{5339}","配","中","."]

/*: "TRTC  :*/
fileprivate let app_entityKey:[Character] = ["T","R","T","C"," "]

/*: 视频通话. logId: :*/
fileprivate let constComputePath:String = "视\u{9891}通话. l"
fileprivate let k_againWomanLargeData:String = "ogId:hood medal deal peer"

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let k_frontUrl:[Character] = ["T","R","T","C"," ","视","频","通","话","\u{9500}","毁","."," ","l","o"]
fileprivate let notiUserData:String = "gId：maker month technique"

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let constStatementMsg:[Character] = ["T","R","T","C"," ","视","频","\u{901a}"]
fileprivate let mainExplainName:[Character] = ["话","错","\u{8bef}","."," ","e","r","r","C","o","d","e",":"]

/*: , errMsg: :*/
fileprivate let user_receiveValue:[Character] = [","," ","e","r","r","M","s","g"]
fileprivate let const_typicalText:[Character] = [":"]

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let const_blindMessage:[Character] = ["T","R","T","C"," ","视","频","通","话","S","D","K","\u{6536}","到","\u{8b66}","\u{544a}","."," ","w","a","r","n","i","n","g","C","o"]
fileprivate let app_hiddenUrl:[Character] = ["d","e",":"]

/*: , warningMsg: :*/
fileprivate let k_filterName:String = "anonymous remote come define foundation, wa"
fileprivate let data_minimumName:[Character] = ["s","g",":"]

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let user_movieStr:String = "TRTC \u{89c6}"
fileprivate let constLayerTapMessage:String = "\u{95f4}. re"
fileprivate let constDecreaseName:String = "sult:ignore draw minute location"

/*: , logId： :*/
fileprivate let show_failureMessage:String = ", logId：receive green title"

/*:  加入了房间" :*/
fileprivate let notiTransitionMinKey:String = "priority"
fileprivate let data_borderPath:[Character] = ["加","入","了","房","间"]

/*:  离开了房间" :*/
fileprivate let data_insertMessage:[Character] = [" ","离","开","了","房","间"]
...
enum CommunicateTotalLiteral: Int {
    //: case CallEnd     = 100
...
class CircumferenceChatConfig: NSObject {
    ...
    var remoteCameraStatus = LeadershipCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class BondRatingCloudDelegate: NSObject {
...
extension BondRatingCloudDelegate {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    ...
    class func completion(uid: String, completion: @escaping FinishBlock) {
        ...
        reqModel.requestPath = (String(userMeKey.suffix(4)) + "all/GetUs" + String(userHelloValue))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: showSituationKey.map{modeHazard(hormone: $0)}, encoding: .utf8)!: uid]
...
extension BondRatingCloudDelegate {
    ...
    func headLensHoodWork(_ frontCamera: Bool) {
        ...
        self.counteriorize(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func license(_ openCamera: Bool) {
        ...
        self.camera(isOpen: openCamera)
    }

    /// 更改视频采集渲染展示位置
    //: func changeRenderView() {
    func releasingHormoneView() {
        ...
        DispatchQueue.main.async {
            ...
            self.advertising(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension BondRatingCloudDelegate {
    ...
    private func counteriorize(front: Bool) {
    ...
    private func camera(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard user_voiceData == true else {
            //: if isOpen {
            if isOpen {
...
extension BondRatingCloudDelegate {
    ...
    func arraySettings() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(memoryPause(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(agreeNotif(notif:)),
    ...
    @objc private func memoryPause(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func bass(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc func agreeNotif(notif: Notification) {
...
extension BondRatingCloudDelegate {
    ...
    func differentiate(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            CountegrityThen.exterminate(msg: (String(appReactPath)))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            CountegrityThen.exterminate(msg: (String(app_entityKey)) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (constComputePath + String(k_againWomanLargeData.prefix(5))) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            advertising(smallWindow: false)
    ...
    func advertising(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard user_voiceData == true else {
    ...
    private func me() {
        ...
        CountegrityThen.exterminate(msg: (String(k_frontUrl) + String(notiUserData.prefix(4))) + "\(infoModel?.logId ?? 0).")
...
extension BondRatingCloudDelegate: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        CountegrityThen.exterminate(msg: (String(constStatementMsg) + String(mainExplainName)) + "\(errCode)" + (String(user_receiveValue) + String(const_typicalText)) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        CountegrityThen.exterminate(msg: (String(const_blindMessage) + String(app_hiddenUrl)) + "\(warningCode)" + (String(k_filterName.suffix(4)) + "rningM" + String(data_minimumName)) + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: userKeyFormat, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        CountegrityThen.exterminate(msg: (user_movieStr + "\u{9891}通话\u{8fdb}入房" + constLayerTapMessage + String(constDecreaseName.prefix(5))) + "\(result)" + (String(show_failureMessage.prefix(8))) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (notiTransitionMinKey.replacingOccurrences(of: "priority", with: " ") + String(data_borderPath)))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (String(data_insertMessage)))
    }
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != MentionTriggerAppManager.share.loginUserMode.userID else { return }
        guard userId != MentionTriggerAppManager.share.loginUserMode.userID else { return }
