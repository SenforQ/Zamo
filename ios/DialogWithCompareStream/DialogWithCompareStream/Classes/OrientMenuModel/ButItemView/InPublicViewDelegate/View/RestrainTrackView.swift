...
fileprivate let main_theoreticalKey:[UInt8] = [0x44,0x43,0x44,0x59,0x5,0x4e,0x42,0x49,0x48,0x5f,0x17,0x4,0xd,0x45,0x4c,0x5e,0xd,0x43,0x42,0x59,0xd,0x4f,0x48,0x48,0x43,0xd,0x44,0x40,0x5d,0x41,0x48,0x40,0x48,0x43,0x59,0x48,0x49]
...
class RestrainTrackView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_theoreticalKey.map{$0^45}, encoding: .utf8)!)
    }
...
extension RestrainTrackView {
    ...
    override func hitTest(_ point: CGPoint, with _: UIEvent?) -> UIView? {
        ...
        for subview in self.subviews {
            ...
            let itemview: SectionThen = subview as! SectionThen
            ...
            if #available(iOS 11.0, *) {
                ...
                if itemview.avatarImage.frame.contains(point) {
                    ...
                    RiseMalePushManager.share.nudge(uid: "\(itemview.giftMsgModel.fromUid ?? 0)")
                    //: break
                    break
                }
                //: } else {
    ...
    func containerStream(model: HomeModelType) {
        //  1.相同礼物时则连击
        //: for  subview in self.subviews {
        for subview in self.subviews {
            //: let itemview: TalkingGiftTrackItemView = subview as! TalkingGiftTrackItemView
            ...
            if enrichment(giftMsgModel: itemview.giftMsgModel, withGiftMsgModel: model) {
                //: itemview.comboHitWith(giftMsgModel: model)
                itemview.fullModel(giftMsgModel: model)
                //: itemview.setGiftMsgModel(model: model)
                itemview.insertMake(model: model)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            let subView: SectionThen? = self.viewWithTag(tag) as! SectionThen?
            //: if subView == nil {
            if subView == nil {
                //: self.addNewGiftDanumWith(giftMsgModel: model, atIndex: index)
                self.personify(giftMsgModel: model, atIndex: index)
                //: return
                return
            }
        }
        ...
        for index in 0 ... 2 {
            ...
            if index == 1 || index == 2 {
                ...
                let subView1: SectionThen = self.viewWithTag(tag) as! SectionThen
                ...
                subView1.afterEdit(frame: frame)
    ...
    func enrichment(giftMsgModel: HomeModelType, withGiftMsgModel: HomeModelType) -> (Bool) {
...
//        subView.showUserInfoViewBlock   = self.showUserInfoViewBlock
        ...
        subView.insertMake(model: giftMsgModel)
        //: self.addSubview(subView)
        self.addSubview(subView)

        //: subView.showGiftAnimate(model: giftMsgModel, animate: true)
        subView.showScope(model: giftMsgModel, animate: true)
    }
}
