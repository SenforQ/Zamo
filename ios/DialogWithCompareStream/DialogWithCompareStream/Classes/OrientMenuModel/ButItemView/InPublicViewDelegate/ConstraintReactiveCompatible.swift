...
/*: "#BAFBFF" :*/
fileprivate let appPartValue:[Character] = ["#","B","A","F","B","F","F"]
...
class ConstraintReactiveCompatible: NSObject {
    ...
    func crushModel(model: TrapModelType) -> TrapModelType {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func boxConnect() {
        ...
        let name = realiseLikeColor(text: firstStr, font: UIFont.wordPicture(fontSize: 14), color: UIColor(hex: (String(appPartValue)))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = realiseLikeColor(text: self.model.content, font: UIFont.wordPicture(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        universalText(attribText: self.model.msgAttribText)
    }
    ...
    func commentIncome() {
        ...
        let content = realiseLikeColor(text: self.model.transContent, font: UIFont.wordPicture(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func packageMinimize() -> NSMutableParagraphStyle {
    ...
    func realiseLikeColor(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = packageMinimize()
        //: return attribute
        return attribute
    }
    ...
    func universalText(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = dataDetailFormat - 36
