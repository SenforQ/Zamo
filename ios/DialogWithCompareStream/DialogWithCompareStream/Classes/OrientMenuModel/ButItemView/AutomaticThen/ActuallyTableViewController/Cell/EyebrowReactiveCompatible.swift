...
fileprivate let user_uponYaTitle:[Character] = ["T","S","y","s","t","e","m","M","e","s","s","a","g","e"]
fileprivate let kYetMessage:[Character] = ["C","e","l","l","_","R","e","u","s","e","I","d"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let k_adjustName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_talk_notic" :*/
fileprivate let userStripWeekWorldwideMsg:String = "icon_talexport remark upper would"
fileprivate let app_donationContent:String = "k_noticmember enter ago resolution"
...
// __CLOSE_PRINT__
//
//  EyebrowReactiveCompatible.swift
//  AbroadTalking
//
...
/**
 * 【模块名称】EyebrowReactiveCompatible
 * 【功能说明】用于实现聊天窗口中的消息单元。
...
@objc public protocol SheDedicationCore: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: EyebrowReactiveCompatible)
    func sort(cell: EyebrowReactiveCompatible)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: EyebrowReactiveCompatible)
    func book(cell: EyebrowReactiveCompatible)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: EyebrowReactiveCompatible)
    func format(cell: EyebrowReactiveCompatible)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: EyebrowReactiveCompatible)
    func enableApplyRunning(cell: EyebrowReactiveCompatible)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: EyebrowReactiveCompatible)
    func thanDirect(cell: EyebrowReactiveCompatible)
}

//: @objcMembers public class EyebrowReactiveCompatible: TCommonTableViewCell {
@objcMembers public class EyebrowReactiveCompatible: LifeViewCell {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\TriggerCellData.h
     */
    //: public var messageData: TriggerCellData?
    public var messageData: TriggerCellData?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(user_uponYaTitle) + String(kYetMessage))

    /**
     *  协议委托
     *  负责实现 SheDedicationCore 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: SheDedicationCore?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_adjustName.reversed(), encoding: .utf8)!)
    }
    ...
    override public func disembarrass(with data: BorderReactiveCompatible) {
        //: super.fill(with: data)
        super.disembarrass(with: data)
        //: let newData: TriggerCellData = data as! TriggerCellData
        let newData: TriggerCellData = data as! TriggerCellData
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.misestimate()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(compareObject(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.wordPicture(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(bridgeBroadcast(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(consumerRecognizer(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.lastTo(name: (String(userStripWeekWorldwideMsg.prefix(8)) + String(app_donationContent.prefix(7))))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(adjustColor(recognizer:)))
...
//: extension EyebrowReactiveCompatible {
extension EyebrowReactiveCompatible {
    //: @objc func onSelectMessageAvatar(recognizer: UIGestureRecognizer) {
    @objc func compareObject(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.enableApplyRunning(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func consumerRecognizer(recognizer: UIGestureRecognizer) {
