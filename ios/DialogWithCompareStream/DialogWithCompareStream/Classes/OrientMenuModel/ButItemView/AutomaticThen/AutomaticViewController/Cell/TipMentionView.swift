...
fileprivate let userButAlwaysUrl:[UInt8] = [0x13,0x14,0x13,0xe,0x52,0x19,0x15,0x1e,0x1f,0x8,0x40,0x53,0x5a,0x12,0x1b,0x9,0x5a,0x14,0x15,0xe,0x5a,0x18,0x1f,0x1f,0x14,0x5a,0x13,0x17,0xa,0x16,0x1f,0x17,0x1f,0x14,0xe,0x1f,0x1e]

/*: "move" :*/
fileprivate let app_illegalId:[Character] = ["m","o","v","e"]
...
class TipMentionView: UIView {
    ...
    var danumuMsgArray: [FlowMysteryMeasurable] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [UnveilingView] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<FlowMysteryMeasurable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userButAlwaysUrl.map{$0^122}, encoding: .utf8)!)
    }
    ...
    func after(_ danumuMsgModel: FlowMysteryMeasurable?) {
    ...
    @objc func insideDestroy(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(insideDestroy(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension TipMentionView: UIGestureRecognizerDelegate {
    ...
    func analysis(for danumuMsgModel: FlowMysteryMeasurable?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = UnveilingView(model: danumuMsgModel)
        ...
        var frame = CGRect(x: show_objectValue, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if DriveReactiveCompatible.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = show_objectValue
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (String(app_illegalId)))
...
//    }
    ...
    func counterior(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                analysis(for: model, isFirst: isFirst)
