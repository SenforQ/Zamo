...
fileprivate let dataRequestMsg:[UInt8] = [0x99,0x9e,0x99,0x84,0xd8,0x93,0x9f,0x94,0x95,0x82,0xca,0xd9,0xd0,0x98,0x91,0x83,0xd0,0x9e,0x9f,0x84,0xd0,0x92,0x95,0x95,0x9e,0xd0,0x99,0x9d,0x80,0x9c,0x95,0x9d,0x95,0x9e,0x84,0x95,0x94]

/*: "all" :*/
fileprivate let notiFastBeautifulContent:String = "awill"
...
// __CLOSE_PRINT__
//
//  StraightReactiveCompatible.swift
//  AbroadTalking
//
...
class StraightReactiveCompatible: SimplyView {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: AcquisitionReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataRequestMsg.map{$0^240}, encoding: .utf8)!)
    }
...
extension StraightReactiveCompatible {
    ...
    override func disembarrass(with data: BorderReactiveCompatible) {
        ...
        self.giftData = data as? AcquisitionReactiveCompatible
        ...
        self.giftImgView.contactMoment(urlStr: imgPreview)
        //: self.giftContentLab.attributedText = giftData.attributedString
        self.giftContentLab.attributedText = giftData.attributedString
        //: if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
        if giftData.msgModel.toUser.uid == (notiFastBeautifulContent.replacingOccurrences(of: "will", with: "ll")) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.lastTo(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.contactMoment(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
