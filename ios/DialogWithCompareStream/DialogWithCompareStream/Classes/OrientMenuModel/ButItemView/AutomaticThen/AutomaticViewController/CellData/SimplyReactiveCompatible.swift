...
fileprivate let const_cloudId:[UInt8] = [0x7d,0x7a,0x7d,0x60,0x3c,0x77,0x7b,0x70,0x71,0x66,0x2e,0x3d,0x34,0x7c,0x75,0x67,0x34,0x7a,0x7b,0x60,0x34,0x76,0x71,0x71,0x7a,0x34,0x7d,0x79,0x64,0x78,0x71,0x79,0x71,0x7a,0x60,0x71,0x70]

/*: "[未知消息]" :*/
fileprivate let k_finishData:String = "["
fileprivate let main_hugePath:[Character] = ["未","知","消","息","]"]

/*: "extra" :*/
fileprivate let notiBeneathUrl:[Character] = ["e","x","t","r","a"]

/*: "#0084FF" :*/
fileprivate let const_shakeFormat:String = "basic against she request#0084FF"

/*: "#FFEE4B" :*/
fileprivate let k_noteStrongValue:[Character] = ["#","F","F","E","E","4","B"]

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let user_directApproximateValue:[Character] = ["<","a","t",">","@","[","\\","S","\\","s","]","+","?","<","/","a","t",">"]

/*: "jumpKey" :*/
fileprivate let showNeverNumbConfirmStr:[Character] = ["j","u","m","p","K","e","y"]

/*: "user" :*/
fileprivate let k_picMonthInviteStr:String = "USER"

/*: "uid" :*/
fileprivate let userDrawingText:[UInt8] = [0x64,0x69,0x75]

/*: "#16D073" :*/
fileprivate let const_coverId:String = "#staff"
fileprivate let k_discourseValue:[Character] = ["6","D","0","7","3"]

/*: "#F95644" :*/
fileprivate let appBecomeKey:[Character] = ["#"]
fileprivate let noti_liftMsg:String = "address95644"
...
// __CLOSE_PRINT__
//
//  SimplyReactiveCompatible.swift
//  AbroadTalking
//
...
//: @objcMembers public class SimplyReactiveCompatible: TalkingChatMsgBaseCellData {
@objcMembers public class SimplyReactiveCompatible: SumroductionCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_cloudId.map{$0^20}, encoding: .utf8)!)
    }
    ...
    override public var msgModel: OrganModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (k_finishData.capitalized + String(main_hugePath))
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = ad(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func ad(msgInfoModel: ConfirmationModel) -> NSMutableAttributedString {
        ...
        guard dic[(String(notiBeneathUrl))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.roundChromaticColor() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(const_shakeFormat.suffix(7))))! : UIColor(hex: (String(k_noteStrongValue)))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(user_directApproximateValue)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(showNeverNumbConfirmStr))].stringValue == (k_picMonthInviteStr.lowercased()) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: userDrawingText.reversed(), encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: userDrawingText.reversed(), encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func someoneLike() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (const_coverId.replacingOccurrences(of: "staff", with: "1") + String(k_discourseValue)))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(appBecomeKey) + noti_liftMsg.replacingOccurrences(of: "address", with: "F")))!
        }
