...
/*: "serialQueue" :*/
fileprivate let app_sectionStr:[Character] = ["s","e","r","i","a","l","Q","u","e"]
fileprivate let data_extremeMsg:String = "UE"

/*: "fromUids" :*/
fileprivate let kTrackTitle:[UInt8] = [0xd2,0xc6,0xdb,0xd9,0xe1,0xdd,0xd0,0xc7]

private func tabRid(goal num: UInt8) -> UInt8 {
    return num ^ 180
}
...
let appTextCreatePath = 50

//: @objcMembers
@objcMembers
//: public class DBUserInfoManager: NSObject {
public class BoundPackInfoManager: NSObject {
    ...
    private static var _instance: BoundPackInfoManager?
    //: class func shared() -> DBUserInfoManager {
    class func starRandomStream() -> BoundPackInfoManager {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = BoundPackInfoManager()
            // 添加退出账号通知
    ...
    @objc private func level() {
        ...
        BoundPackInfoManager.featureDestroy()
    }
}
...
extension BoundPackInfoManager {
    /// 获取用户消息
    /// - Parameters:
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([PronkCacheModel]?, Error?) -> Void) {
    class func faceCompletion(with userIdsArr: [String], completion: @escaping ([PronkCacheModel]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if BoundPackInfoManager.starRandomStream().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(BoundPackInfoManager.starRandomStream().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            BoundPackInfoManager.starRandomStream().needReqUidsSet = nil
        }
        ...
        let pageSize = appTextCreatePath
        //: repeat {
        repeat {
            ...
            doingResistanceCompletion(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func outlet(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (String(app_sectionStr) + data_extremeMsg.lowercased()))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    ...
                    GoUpReactiveCompatible.lessInsert(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func option(message: V2TIMMessage) -> WeightMsgTable {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = ChatManager.strengthManager(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = WeightMsgTable.recordingLess(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = ChatManager.visible(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = WeightMsgTable.belowQuality(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension BoundPackInfoManager {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> PronkCacheModel? {
    class func terms(targetId: String) -> PronkCacheModel? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return ProvideInfoTable.createListener(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: PronkCacheModel?) {
    class func sameExit(with userInfo: PronkCacheModel?) {
        ...
        let model = BoundPackInfoManager.terms(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        ProvideInfoTable.belowPartner(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: main_deviceTitle, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func cache(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = BoundPackInfoManager.terms(targetId: targetId) else { return }
    ...
    class func controlTime(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = BoundPackInfoManager.terms(targetId: targetId) else { return }
    ...
    class func leftSlantingCaseHistory(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        ProvideInfoTable.streetwise(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        BoundPackInfoManager.starRandomStream().reqedUidsSet.remove(targetUid)
    }
}
...
extension BoundPackInfoManager {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([PronkCacheModel]?, Error?) -> Void) {
    private class func doingResistanceCompletion(with userIdsArr: [String], completion: @escaping ([PronkCacheModel]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = ProvideInfoTable.per(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if BoundPackInfoManager.starRandomStream().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            BoundPackInfoManager.perMale(with: noDBUidArr)
        }
    }
    ...
    private class func perMale(with userIdsArr: [String]) {
        ...
        BoundPackInfoManager.starRandomStream().reqedUidsSet = BoundPackInfoManager.starRandomStream().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: TalkingChatRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        GoUpReactiveCompatible.fieldFromCompletion(param: [String(bytes: kTrackTitle.map{tabRid(goal: $0)}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                BoundPackInfoManager.starRandomStream().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            BoundPackInfoManager.orientationGo(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: PronkCacheModel] = [:]
            var retInfoDict: [String: PronkCacheModel] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: main_deviceTitle, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [PronkCacheModel]) {
    private class func orientationGo(userInfoArr: [PronkCacheModel]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            ProvideInfoTable.belowPartner(aWrap)
        }
    }
}
