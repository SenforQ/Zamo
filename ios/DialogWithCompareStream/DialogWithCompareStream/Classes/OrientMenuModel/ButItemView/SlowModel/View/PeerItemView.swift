...
/*: "#ffffff" :*/
fileprivate let data_databaseFormat:String = "former arrow average#ffffff"

/*: "/dist/loungePlus/index.html" :*/
fileprivate let notiExitData:String = "interaction natural/dis"
fileprivate let noti_translationData:String = "sphere anyungeP"
fileprivate let user_reinId:String = "end greatest moon channel multipledex.html"

/*: "https://" :*/
fileprivate let showDialogMessage:[Character] = ["h","t","t","p","s"]
fileprivate let main_techniqueName:String = "://"

/*: "http://" :*/
fileprivate let k_removePanName:String = "ex make fromhttp://"
...
class PeerItemView: UIView {
    ...
    let Broadcast_TextColor = (String(data_databaseFormat.suffix(7)))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .wordPicture(fontSize: 12)
    ...
    convenience init(model: WritingTransformable?) {
        //: self.init()
        self.init()
    ...
    private func momentTask() {
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            var mutableAtt = parse.opend(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func mutualLip(withUrl url: String) {
        ...
        self.backgroundImageView.choiceRest(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.playerExist(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: WritingTransformable? {
        //: didSet {
        didSet {
            //: configModel()
            momentTask()
    ...
    var totalTime: CGFloat {
        ...
        return (show_objectValue + widthValue) * Broadcast_SpendTime / show_objectValue
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / show_objectValue
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / show_objectValue
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(featureAccessUsed(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension PeerItemView {
    /// 弹幕点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func featureAccessUsed(_: UITapGestureRecognizer) {
        ...
        case "1": // 个人中心
            ...
            RiseMalePushManager.share.nudge(uid: self.danumuMsgModel?.jumpKey)
        ...
        case "2": // 直播间
            ...
            if let uid = self.danumuMsgModel?.jumpKey {
                ...
                RiseMalePushManager.share.aboveSpace(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(notiExitData.suffix(4)) + "t/lo" + String(noti_translationData.suffix(5)) + "lus/in" + String(user_reinId.suffix(8)))) {
                    //: TalkingAppPushManager.share.func__pushToSubscribePageWebVC()
                    RiseMalePushManager.share.behindHairFill()
                    //: return
                    return
                }
                //: let httpsUrl = MentionTriggerAppManager.share.appConfigMode.urlH5Domain+url
                let httpsUrl = MentionTriggerAppManager.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(showDialogMessage) + main_techniqueName.capitalized), with: (String(k_removePanName.suffix(7))))
                //: TalkingAppPushManager.share.func__pushToWebVC(urlStr: httpUrl)
                RiseMalePushManager.share.errConfig(urlStr: httpUrl)
            }
