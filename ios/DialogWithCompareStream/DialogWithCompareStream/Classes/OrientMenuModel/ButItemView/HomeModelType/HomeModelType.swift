...
/*: "num" :*/
fileprivate let main_hoodStr:String = "NUM"
...
@objc public enum ExtensiveCountLiteral: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct HomeModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// ExtensiveCountLiteral
    ...
    mutating func legacy(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct RidTransformable: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (main_hoodStr.lowercased())
    }
}
