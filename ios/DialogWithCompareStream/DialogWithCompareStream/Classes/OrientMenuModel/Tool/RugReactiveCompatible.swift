...
/*: "gifts" :*/
fileprivate let constLessData:[Character] = ["g","i","f","t","s"]

/*: "vipGifts" :*/
fileprivate let main_mobileTurnKey:String = "vipGiftshark lovely"

/*: "packGifts" :*/
fileprivate let notiSinkRapKey:String = "pviack"

/*: "giftSendItem" :*/
fileprivate let const_portraitStr:String = "gactivet"
fileprivate let kPlacementId:String = "tgapm"
...
class RugReactiveCompatible: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [DeviseExpressionConvertible: [Any]]]()
    // 数据锁
    ...
    private var giftType = AuthorizeSequence.normal

    //: static let share = TalkingChatGiftManager()
    static let share = RugReactiveCompatible()
...
extension RugReactiveCompatible {
    /// 发送礼物面板数据
    /// - Parameters:
    ...
    func display(type: AuthorizeSequence = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        RugReactiveCompatible.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[DeviseExpressionConvertible.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            RugReactiveCompatible.share.towardSimultaneously(completion: { isSucceed in
            ...
            RugReactiveCompatible.share.reclaimModifyCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            RugReactiveCompatible.share.tool(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func tool(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = RugReactiveCompatible.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        GoUpReactiveCompatible.man(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [DeviseExpressionConvertible: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(String(constLessData))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<TipTransformable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[DeviseExpressionConvertible.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(main_mobileTurnKey.prefix(8)))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<TipTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[DeviseExpressionConvertible.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(notiSinkRapKey.replacingOccurrences(of: "via", with: "a") + "Gifts")] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<TipTransformable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[DeviseExpressionConvertible.Bags] = bagsArr
                }
                ...
                var firstModel = LifetimeArrModel()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(const_portraitStr.replacingOccurrences(of: "active", with: "if") + "SendI" + kPlacementId.replacingOccurrences(of: "gap", with: "e"))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<LifetimeArrModel>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func towardSimultaneously(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = RugReactiveCompatible.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        GoUpReactiveCompatible.endComponent(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func languageDelay(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        GoUpReactiveCompatible.groupGetCompletion(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<StandCoreHandyJSON>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func reclaimModifyCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = AuthorizeSequence.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        GoUpReactiveCompatible.visualizer(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func close(perPageCount: Int, dataType: DeviseExpressionConvertible) -> Int {
    ...
    func assessment(dataType: DeviseExpressionConvertible) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func perInput() -> [Any] {
        ...
        if RugReactiveCompatible.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live {
        } else if RugReactiveCompatible.share.giftType == .live {
    ...
    func briskUp(dataType: DeviseExpressionConvertible, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension RugReactiveCompatible {
    ...
    @objc private func brightPackage() {
        //: gift_clearMemoryCache(showRed: true)
        priceWith(showRed: true)
    }
}
...
extension RugReactiveCompatible {
    ...
    func priceWith(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        RugReactiveCompatible.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
