...
fileprivate let userOptionPictureText:[UInt8] = [0x4a,0x4d,0x4a,0x57,0xb,0x40,0x4c,0x47,0x46,0x51,0x19,0xa,0x3,0x4b,0x42,0x50,0x3,0x4d,0x4c,0x57,0x3,0x41,0x46,0x46,0x4d,0x3,0x4a,0x4e,0x53,0x4f,0x46,0x4e,0x46,0x4d,0x57,0x46,0x47]

private func sweepingSatisfy(issue num: UInt8) -> UInt8 {
    return num ^ 35
}

/*: "ThrowWeightThen" :*/
fileprivate let user_transportStr:String = "say down temporaryDropM"
fileprivate let show_oppositeStr:String = "explorembexplore"
fileprivate let kAidUnderId:String = "Cellowner list"

/*: "roomId" :*/
fileprivate let userCoordinatorValue:String = "roomIdinto able name want"

/*: "type" :*/
fileprivate let noti_limitId:String = "tdestroype"

/*: "page" :*/
fileprivate let kBalanceContent:[Character] = ["p","a","g","e"]

/*: "icon_chats_pcr" :*/
fileprivate let k_workerFormat:String = "reason dimension musticon_"
fileprivate let noti_stealAnswerMsg:[Character] = ["_","p","c","r"]

/*: "All Numbers" :*/
fileprivate let noti_foreData:String = "All Ntell mp"
fileprivate let dataOptionQuantityId:String = "remove"

/*: "all" :*/
fileprivate let showStepKey:String = "thumbl"

/*: "Recent Gifts" :*/
fileprivate let noti_filterTitle:[Character] = ["R","e","c","e","n","t"]
fileprivate let showCompleteMsg:[Character] = [" ","G","i","f","t","s"]
...
protocol ExtensiveViewDelegate {
...
class AccessibleDataSource: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    //: var menuDelegate: DropDownMemberMenuViewDelegate?
    var menuDelegate: ExtensiveViewDelegate?
    ...
    public var triangleHeight = Config.ExtensiveConfig.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.ExtensiveConfig.adjustPullDown
    ...
    fileprivate lazy var contentView: UIView = self.promptbookMedia()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.fullFirst()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = AccessibleDataSource(frame: .zero)
    ...
    init(frame: CGRect, anchorView: UIView, roomID: String) {
        ...
        super.init(frame: frame)
        //: let window = EverlastinglyReactiveCompatible.getWindow()
        let window = EverlastinglyReactiveCompatible.insideWindow()
        //: self.roomID = roomID
        self.roomID = roomID
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userOptionPictureText.map{sweepingSatisfy(issue: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func promptbookMedia() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func fullFirst() -> UITableView {
    ...
    private func minInside() {
        ...
        mTable.register(ThrowWeightThen.self, forCellReuseIdentifier: (String(user_transportStr.suffix(5)) + show_oppositeStr.replacingOccurrences(of: "explore", with: "e") + "rMenu" + String(kAidUnderId.prefix(4))))
    ...
    //: func reqData() {
    func comprehensiveData() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(userCoordinatorValue.prefix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(noti_limitId.replacingOccurrences(of: "destroy", with: "y"))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(String(kBalanceContent))] = String(pageIndex)
        ...
        RugReactiveCompatible.share.languageDelay(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = StandCoreHandyJSON()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(k_workerFormat.suffix(5)) + "chats" + String(noti_stealAnswerMsg))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(noti_foreData.prefix(5)) + "umber" + dataOptionQuantityId.replacingOccurrences(of: "remove", with: "s")).localized
                //: model.uid = "all"
                model.uid = (showStepKey.replacingOccurrences(of: "thumb", with: "al"))
...
extension AccessibleDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(user_transportStr.suffix(5)) + show_oppositeStr.replacingOccurrences(of: "explore", with: "e") + "rMenu" + String(kAidUnderId.prefix(4))), for: indexPath) as! ThrowWeightThen
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! StandCoreHandyJSON
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! StandCoreHandyJSON
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.roundResolution(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.meanSunTrack(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: show_objectValue, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.appColor()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .landEnableSize(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .modusOperandi()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(noti_filterTitle) + String(showCompleteMsg)).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(noti_foreData.prefix(5)) + "umber" + dataOptionQuantityId.replacingOccurrences(of: "remove", with: "s")).localized
        }
