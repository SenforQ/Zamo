...
fileprivate let noti_interactionPath:[UInt8] = [0xd3,0xd4,0xd3,0xce,0x92,0xd9,0xd5,0xde,0xdf,0xc8,0x80,0x93,0x9a,0xd2,0xdb,0xc9,0x9a,0xd4,0xd5,0xce,0x9a,0xd8,0xdf,0xdf,0xd4,0x9a,0xd3,0xd7,0xca,0xd6,0xdf,0xd7,0xdf,0xd4,0xce,0xdf,0xde]

private func refMight(heart num: UInt8) -> UInt8 {
    return num ^ 186
}

/*: "icon_rank_coin" :*/
fileprivate let notiPartnerNetStr:[UInt8] = [0xb7,0xb1,0xbd,0xbc,0xad,0xc0,0xaf,0xbc,0xb9,0xad,0xb1,0xbd,0xb7,0xbc]

fileprivate func solarReminder(medium num: UInt8) -> UInt8 {
...
/*: "icon_lounge" :*/
fileprivate let k_mayKey:String = "IC"
fileprivate let mainFlowMessage:String = "far public related rapon_lo"

/*: "icon_rank_coin2" :*/
fileprivate let mainPlayerPath:[Character] = ["i","c","o","n","_","r","a","n","k","_","c","o","i","n","2"]
...
class ExtensiveViewCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_interactionPath.map{refMight(heart: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.modusOperandi()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .landEnableSize(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = EarthThen().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.lastTo(name: String(bytes: notiPartnerNetStr.map{solarReminder(medium: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .landEnableSize(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.tillOfCreation(colors: UIColor.spectralColor(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.lastTo(name: (k_mayKey.lowercased() + String(mainFlowMessage.suffix(5)) + "unge"))
    }
}
...
extension ExtensiveViewCell {
    //: func updateRankCell(model: SocialRankItemModel, index: Int) {
    func switchlyIndex(model: SocialTransformable, index: Int) {
        ...
        headPic.contactMoment(urlStr: model.headPic)
    ...
    func today(type: Int) {
        ...
        let imageStr = type == 0 ? String(bytes: notiPartnerNetStr.map{solarReminder(medium: $0)}, encoding: .utf8)! : (String(mainPlayerPath))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.lastTo(name: imageStr), for: .normal)
    }
}
