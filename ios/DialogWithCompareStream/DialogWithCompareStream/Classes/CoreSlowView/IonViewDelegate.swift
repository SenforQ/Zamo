...
fileprivate let const_hereMessage:[UInt8] = [0xb6,0xbb,0xb6,0xc1,0x75,0xb0,0xbc,0xb1,0xb2,0xbf,0x87,0x76,0x6d,0xb5,0xae,0xc0,0x6d,0xbb,0xbc,0xc1,0x6d,0xaf,0xb2,0xb2,0xbb,0x6d,0xb6,0xba,0xbd,0xb9,0xb2,0xba,0xb2,0xbb,0xc1,0xb2,0xb1]

fileprivate func leadAstray(sh num: UInt8) -> UInt8 {
...
/*: "Not yet~" :*/
fileprivate let data_promptValue:[Character] = ["N","o","t"," ","y","e"]
fileprivate let appGeneralTitle:String = "filter greatest dogt~"

/*: "#8C7AFF" :*/
fileprivate let main_clearPath:String = "#8C7AFFeh per video mark deadline"
...
class IonViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_hereMessage.map{leadAstray(sh: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var dataArray: [SocialTransformable] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(PinReactiveCompatible.self, forCellReuseIdentifier: PinReactiveCompatible.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(ExtensiveViewCell.self, forCellReuseIdentifier: ExtensiveViewCell.className())
    }
    ...
    lazy var tableHeaderView = PerplexityThen(frame: CGRect(x: 0, y: 0, width: show_objectValue, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = AuthorizeEmptyView().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(data_promptValue) + String(appGeneralTitle.suffix(2))).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(main_clearPath.prefix(7))))
...
extension IonViewDelegate: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: PinReactiveCompatible.className(), for: indexPath) as! PinReactiveCompatible
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: ExtensiveViewCell.className(), for: indexPath) as! ExtensiveViewCell
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.switchlyIndex(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.today(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: PinReactiveCompatible.className()) as! PinReactiveCompatible
...
extension IonViewDelegate: JXSegmentedListContainerViewListDelegate {
...
extension IonViewDelegate {
    //: private func setupSubviews() {
    private func speakerNor() {
