...
fileprivate let user_listenerValue:[UInt8] = [0x71,0x74,0x68,0x66,0x79,0x6e,0x74,0x73,0x25,0x66,0x7a,0x79,0x6d,0x74,0x77,0x6e,0x7f,0x66,0x79,0x6e,0x74,0x73,0x58,0x79,0x66,0x79,0x7a,0x78,0x25,0x6e,0x78,0x25]

fileprivate func fitIt(fleet num: UInt8) -> UInt8 {
    let value = Int(num) - 5
    if value < 0 {
...
fileprivate let noti_dragUrl:[Character] = ["g","e","t"," ","l","o","c","a","t","i","o","n"," ","f","a","i","l","e","d","."," ","e","r","r","o","r"]
fileprivate let const_calculationUrl:String = "delivery"
...
// __CLOSE_PRINT__
//
//  OutsideReactiveCompatible.swift
//  AbroadTalking
//
...
class OutsideReactiveCompatible: NSObject {
    ...
    var locationType: AssistantBinarySum?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: OutsideReactiveCompatible?
    //: class func shared() -> TalkingLocationManager {
    class func untilGuidance() -> OutsideReactiveCompatible {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = OutsideReactiveCompatible()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    //: class func destroy() {
    class func presentationByDestroy() {
    ...
    func signalVoice(block: @escaping Locationblock) {
    ...
    func packageResistance(block: @escaping Authblock) {
    ...
    func openSelect() -> AssistantBinarySum {
        //: switch locationPermission() {
        switch popularLocation() {
        //: case .notDetermined:
        case .notDetermined:
            //: return .firstEntry
            return .firstEntry
    ...
    func popularLocation() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: user_listenerValue.map{fitIt(fleet: $0)}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension OutsideReactiveCompatible: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(noti_dragUrl) + const_calculationUrl.replacingOccurrences(of: "delivery", with: ":")) + "\(error.localizedDescription)")
    }
}
