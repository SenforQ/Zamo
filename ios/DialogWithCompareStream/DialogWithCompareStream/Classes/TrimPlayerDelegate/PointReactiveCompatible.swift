...
fileprivate let show_albumKey:[UInt8] = [0xef,0xe8,0xef,0xf2,0xae,0xe5,0xe9,0xe2,0xe3,0xf4,0xbc,0xaf,0xa6,0xee,0xe7,0xf5,0xa6,0xe8,0xe9,0xf2,0xa6,0xe4,0xe3,0xe3,0xe8,0xa6,0xef,0xeb,0xf6,0xea,0xe3,0xeb,0xe3,0xe8,0xf2,0xe3,0xe2]

private func quickSplit(team num: UInt8) -> UInt8 {
    return num ^ 134
}

/*: "F5F5F5" :*/
fileprivate let user_earlyKey:[Character] = ["F","5","F","5","F","5"]
...
class PointReactiveCompatible: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_albumKey.map{quickSplit(team: $0)}, encoding: .utf8)!)
    }
    ...
    func canCell(isListTableCell: Bool) {
    ...
    lazy var userInfoView: RocketReactiveCompatible = {
        //: let  view = TalkingMomentUserInfoView.init()
        let view = RocketReactiveCompatible()
    ...
    lazy var textContentView: BeThen = {
    ...
    lazy var photoView: GuideCollectionView = {
        ...
        let view = GuideCollectionView(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var bottomView: DelimitationThen = {
        //: let  view = TalkingMomentItemBottomView.init()
        let view = DelimitationThen()
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(user_earlyKey)))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
extension PointReactiveCompatible {
    //: func configCell(model: TalkingMomentModel) {
    func cellRed(model: DigitiserMomentModel) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.beyondBright(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.cosmopolitan(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == EyebrowOutputStream.Photo.rawValue || model.momentType == EyebrowOutputStream.Live.rawValue {
...
//            }
            ...
            photoView.via(model: cunrrenModel)
...
//            }
            ...
            videoView.atJaw(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.when(model: cunrrenModel)
...
//        }
    ...
    @objc func mommentDelay() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: cunrrenModel.videoInfo!.videoUrl)
        let vc = TrimPlayerDelegate(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != MentionTriggerAppManager.share.loginUserMode.sex {
        if cunrrenModel.sex != MentionTriggerAppManager.share.loginUserMode.sex {
    ...
    func focus() {
        ...
        if cunrrenModel.momentType == EyebrowOutputStream.Video.rawValue {
            //: videoView.setupPlayer()
            videoView.rawNativeBaseballPlayer()
        }
    }
...
extension PointReactiveCompatible {
    ...
    private func circle() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(mommentDelay))
