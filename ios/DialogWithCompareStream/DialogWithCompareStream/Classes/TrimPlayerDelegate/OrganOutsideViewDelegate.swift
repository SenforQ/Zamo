...
/*: "Cancel" :*/
fileprivate let noti_hisAbsValue:[Character] = ["C","a","n","c","e","l"]

/*: "Post" :*/
fileprivate let showAdvertisingKey:String = "Postmanage accounting someone yellow complaint"

/*: "Upload failed" :*/
fileprivate let notiAdditionValue:[Character] = ["U","p","l","o","a","d"," ","f","a"]
fileprivate let show_subjectMsg:String = "ILED"

/*: "imgUrl" :*/
fileprivate let k_discourseUrl:String = "imgUrlprepare beauty log home letter"

/*: "videoUrl" :*/
fileprivate let showPortId:[Character] = ["v"]
fileprivate let app_narrowId:String = "resistance mini success conduct instanceideoUrl"

/*: "content" :*/
fileprivate let mainGlobalText:String = "presentationntent"

/*: "location" :*/
fileprivate let data_replaceMsg:[Character] = ["l","o","c","a","t","i","o","n"]

/*: "Video review in progress" :*/
fileprivate let data_absTitle:[Character] = ["V","i","d","e","o"," ","r","e","v","i","e","w"," ","i","n"," ","p","r","o"]
fileprivate let k_lockPath:String = "grlosess"

/*: "Shoot video at least 5 seconds" :*/
fileprivate let mainMouthFreshExtraStr:String = "Shoot pregnant spend render organ ample"
fileprivate let app_speedData:String = " at leagain tab"
fileprivate let main_revenueStr:String = "refuse sample evaluation treat identity5 secon"
fileprivate let constTenderDatabaseUrl:[Character] = ["d","s"]

/*: "Camera" :*/
fileprivate let user_funWarningUrl:[Character] = ["C","a","m","e","r","a"]

/*: "Photo" :*/
fileprivate let const_multiPath:[Character] = ["P","h","o","t","o"]

/*: "Video" :*/
fileprivate let kPromptStr:String = "Videoanalysis say life enable become"
...
// __CLOSE_PRINT__
//
//  OrganOutsideViewDelegate.swift
//  AbroadTalking
//
...
protocol QuantityegrityObjectProtocol: NSObjectProtocol {
    // 发布成功
    //: func ShareSucceed()
    func buttonTool()
}
...
class OrganOutsideViewDelegate: EverlastinglyThen {
    //: weak var delegate: TalkingFreeViewControllerDelegate?
    weak var delegate: QuantityegrityObjectProtocol?
    ...
    private lazy var reqManager: TopicThen = //: return TopicThen()
        .init()
    //: }()
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(noti_hisAbsValue)).localized, for: .normal)
    ...
    private lazy var postBtn: UIButton = {
        ...
        btn.setTitle((String(showAdvertisingKey.prefix(4))).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.tillOfCreation(colors: UIColor.spectralColor(), size: CGSize(width: 70, height: 30)), for: .normal)
        //: btn.layer.cornerRadius = 15
        btn.layer.cornerRadius = 15
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(postBtnClickEvent), for: .touchUpInside)
        btn.addTarget(self, action: #selector(acquire), for: .touchUpInside)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
    ...
    private lazy var collectionView: LooseView = {
        //: let collectionView = FreeCollectionView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        let collectionView = LooseView(frame: CGRect.zero, collectionViewLayout: UICollectionViewFlowLayout())
        ...
        collectionView.deleteActionBlock = { [weak self] in
            ...
            if self.collectionView.imageArray.count == 0 {
                ...
                self.collectionView.contentType = EyebrowOutputStream.nones
                //: self.videoPath = ""
                self.videoPath = ""
                //: self.coverPath = ""
                self.coverPath = ""
...
extension OrganOutsideViewDelegate {
    /// 发布动态
    ...
    func send() {
        ...
        reqManager.eavesdropLive(photos: self.collectionView.imageArray, content: content) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            ...
            NotificationCenter.default.post(name: kLessName, object: self, userInfo: nil)
        }
    }
    ...
    func telecasting() {
        ...
        ButSignatureProvider.shared.doMethod(type: 1, coverPath: self.coverPath, videoPath: self.videoPath) { coverUrl, videoUrl, succeed in
            //: guard succeed else {
            guard succeed else {
                //: ProgressHUD.dismiss()
                LovelyLifetimeProgressHUD.arcCurDismiss()
                //: self.func__showStatusBarErrorMsg(showMsg: "Upload failed".localized)
                self.outBarMsg(showMsg: (String(notiAdditionValue) + show_subjectMsg.lowercased()).localized)
                //: return
                return
            }
            ...
            dict[(String(k_discourseUrl.prefix(6)))] = coverUrl
            //: dict["videoUrl"] = videoUrl
            dict[(String(showPortId) + String(app_narrowId.suffix(7)))] = videoUrl
            //: dict["content"] = content
            dict[(mainGlobalText.replacingOccurrences(of: "presentation", with: "co"))] = content
            //: dict["location"] = ""
            dict[(String(data_replaceMsg))] = ""

            //: TalkingMomentRequestTool.releaseVideoMoment(params: dict) {[weak self] succeed, result, errorModel in
            TopicThen.viaLicense(params: dict) { [weak self] succeed, _, _ in
                //: ProgressHUD.dismiss()
                LovelyLifetimeProgressHUD.arcCurDismiss()
                //: guard let self = self else { return }
                guard let self = self else { return }
                ...
                self.noExtra(showMsg: (String(data_absTitle) + k_lockPath.replacingOccurrences(of: "lose", with: "e")).localized)
                //: self.navigationController?.popViewController(animated: true)
                self.navigationController?.popViewController(animated: true)
                //: self.delegate?.ShareSucceed()
                self.delegate?.buttonTool()
                //: NotificationCenter.default.post(name: POST_EXPLORE_SUCCESS_NOTIFICATION, object: self, userInfo: nil)
                NotificationCenter.default.post(name: kLessName, object: self, userInfo: nil)
            }
        }
...
extension OrganOutsideViewDelegate {
    ...
    func vidCamera() {
        ...
        GatePermissionTool.successfullyFor(true) { (isOpen: Bool) in
    ...
    func chooseRecordAlbum() {
        ...
        GatePermissionTool.funcWithoutSquareBlock(true) { (isOpen: Bool) in
            ...
            let vc = CapturePickTool.subjectMatter(maxCount: count, allowPhoto: true, allowVideo: false)
            ...
            vc.didFinishPickingVideoHandle = { [weak self] (_: UIImage?,
                    ...
                    CapturePickTool.pathOff(asset: assets) { filePath in
                        ...
                        if seconds >= 5 {
                            ...
                            self.equilibrium(url: filePath!)
                            //: } else {
                        } else {
                            //: DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                                //: self.func__showStatusBarErrorMsg(showMsg: "Shoot video at least 5 seconds".localized)
                                self.outBarMsg(showMsg: (String(mainMouthFreshExtraStr.prefix(6)) + "video" + String(app_speedData.prefix(6)) + "ast " + String(main_revenueStr.suffix(7)) + String(constTenderDatabaseUrl)).localized)
                            }
                        }
                    }
            }
    ...
    func equilibrium(url: URL) {
        //: let videoVC = TalkingVideoEditorController(maxDuration: 60.0, videoURL: url) { videoInfo in
        let videoVC = AccessibleViewController(maxDuration: 60.0, videoURL: url) { videoInfo in
...
extension OrganOutsideViewDelegate: UIImagePickerControllerDelegate & UINavigationControllerDelegate {
        ...
        self.dismiss(animated: true) {
            ...
            if mediaType == kUTTypeMovie as String {
                ...
                if seconds >= 5 {
                    ...
                    self.equilibrium(url: info[.mediaURL]! as! URL)
...
// MARK: - CollectionObjectProtocol

//: extension TalkingFreeViewController: FreeCollectionViewDelegate {
extension OrganOutsideViewDelegate: CollectionObjectProtocol {
    /// 选择图片
    ...
    func exist() {
        ...
        if self.collectionView.contentType == .Photo {
            ...
            vc.resList(cellTitleList: [(String(user_funWarningUrl)).localized, (String(const_multiPath)).localized])
            //: }else {
        } else {
            //: vc.initwithList(cellTitleList: ["Camera".localized, "Photo".localized, "Video".localized])
            vc.resList(cellTitleList: [(String(user_funWarningUrl)).localized, (String(const_multiPath)).localized, (String(kPromptStr.prefix(5))).localized])
        }
    ...
    func spread(index: IndexPath) {
        ...
        let vc = AliveViewDelegate(images: self.collectionView.imageArray, index: index.row, type: .publish)
        // 删除按钮block
    ...
    func judge() {
        ...
        let vc = TrimPlayerDelegate(videoPath: self.videoPath)
        //: vc.modalPresentationStyle = .fullScreen
        vc.modalPresentationStyle = .fullScreen
        //: self.present(vc, animated: true)
        self.present(vc, animated: true)
...
extension OrganOutsideViewDelegate {
    ...
    func incomeUi() {
        ...
        cancelBtn.snp.makeConstraints { make in
            ...
            make.top.equalTo(app_pathData + 7)
        ...
        collectionView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(appMarginFormat)
