...
fileprivate let app_dialogFormat:[UInt8] = [0xca,0xcf,0xca,0xd5,0x89,0xc4,0xd0,0xc5,0xc6,0xd3,0x9b,0x8a,0x81,0xc9,0xc2,0xd4,0x81,0xcf,0xd0,0xd5,0x81,0xc3,0xc6,0xc6,0xcf,0x81,0xca,0xce,0xd1,0xcd,0xc6,0xce,0xc6,0xcf,0xd5,0xc6,0xc5]

fileprivate func attractiveDismiss(shadow num: UInt8) -> UInt8 {
...
/*: "#4FAAFF" :*/
fileprivate let data_ourKey:[Character] = ["#","4","F","A","A","F"]
fileprivate let appRemarkId:String = "min"

/*: "reply" :*/
fileprivate let userAdminTitle:String = "portraiteply"

/*: "F5F5F5" :*/
fileprivate let user_adId:String = "FsuiteFsuiteFsuite"
...
class WouldItemThen: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_dialogFormat.map{attractiveDismiss(shadow: $0)}, encoding: .utf8)!)
    }
...
extension WouldItemThen {
    //: @objc func onLongTapCell(sender: UIGestureRecognizer) {
    @objc func three(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension WouldItemThen {
    //: func configCell(model: TalkingCommentReplyModel) {
    func prepare(model: FlowReplyModel) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(data_ourKey) + appRemarkId.replacingOccurrences(of: "min", with: "F")))
            ...
            let font = UIFont.activeBeautyNative(fontSize: 14)
            //: if model.replyUser.count>0 {
            if model.replyUser.count > 0 {
                //: strA = [model.nickname, " "+"reply".localized+" ", model.replyUser, "：\(model.content)"]
                strA = [model.nickname, " " + (userAdminTitle.replacingOccurrences(of: "portrait", with: "r")).localized + " ", model.replyUser, "：\(model.content)"]
                //: colorA = [UIColor.appValueDetailColor(), UIColor.appTitleColor(), UIColor.appValueDetailColor(), UIColor.appTitleColor()]
                colorA = [UIColor.modusOperandi(), UIColor.roundChromaticColor(), UIColor.modusOperandi(), UIColor.roundChromaticColor()]
                ...
                colorA = [UIColor.modusOperandi(), UIColor.roundChromaticColor()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
            ...
            attributedString = attributedString.draftRid(texts: strA, colors: colorA, fonts: fontA)
    ...
    func presentationHeight(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: show_objectValue - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (user_adId.replacingOccurrences(of: "suite", with: "5")))?.cgColor
...
extension WouldItemThen {
    ...
    private func catchOut() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(three(sender:)))
