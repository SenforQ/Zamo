...
fileprivate let notiAnotherPath:[UInt8] = [0x6c,0x71,0x6c,0x77,0x2b,0x66,0x72,0x67,0x68,0x75,0x3d,0x2c,0x23,0x6b,0x64,0x76,0x23,0x71,0x72,0x77,0x23,0x65,0x68,0x68,0x71,0x23,0x6c,0x70,0x73,0x6f,0x68,0x70,0x68,0x71,0x77,0x68,0x67]

fileprivate func connectSave(brow num: UInt8) -> UInt8 {
...
class MotivationReusableView: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiAnotherPath.map{connectSave(brow: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func income() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(board))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(thumbTap))
...
extension MotivationReusableView: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func board(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func thumbTap(tap: UIGestureRecognizer) {
    ...
    func child(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.contactMoment(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func commitAdmin(image: UIImage) {
        //: imageView.image = image
        imageView.image = image
    }

    //: func refreshImageContainerViewCenter () {
    func amplitudeModulationCenter() {
