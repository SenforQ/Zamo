...
/*: "tab" :*/
fileprivate let app_compareName:String = "myb"

/*: "page" :*/
fileprivate let userKitData:String = "pbouncege"

/*: "uid" :*/
fileprivate let showBeFailMsg:[Character] = ["u","i","d"]

/*: "model" :*/
fileprivate let notiLittleListId:[Character] = ["m","o","d","e","l"]

/*: "There's no feed yet." :*/
fileprivate let notiCoreValue:[Character] = ["T","h","e","r","e","\'","s"," ","n","o"," ","f","e","e","d"]
fileprivate let notiTransactionTitle:[Character] = [" ","y","e","t","."]
...
class WeightMomentVc: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    ...
    var seleteCell = PointReactiveCompatible()
    //: var lastVideoUrl = ""
    var lastVideoUrl = ""
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: constSceneId, right: 0)
        //: return table
        return table
...
extension WeightMomentVc {
    //: func reqData() {
    func cell() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(app_compareName.replacingOccurrences(of: "my", with: "ta"))] = 1
        //: dict["page"] = pageIndex
        dict[(userKitData.replacingOccurrences(of: "bounce", with: "a"))] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        TopicThen.lay(params: dict) { succeed, result, errorModel in
...
extension WeightMomentVc {
    ...
    @objc func supererogatoryNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(String(showBeFailMsg))] as! String
    ...
    @objc func animaGold(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: DigitiserMomentModel = userinfo[(String(notiLittleListId))] as! DigitiserMomentModel
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
    ...
    @objc func stripped(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: DigitiserMomentModel = userinfo[(String(notiLittleListId))] as! DigitiserMomentModel
...
extension WeightMomentVc: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = PointReactiveCompatible.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? PointReactiveCompatible
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = PointReactiveCompatible(style: .default, reuseIdentifier: identifier)
        }
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == EyebrowOutputStream.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: PointReactiveCompatible = cell as? PointReactiveCompatible else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
...
extension WeightMomentVc: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        channel(scrollView: scrollView)
    }
    ...
    func channel(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: PointReactiveCompatible = mainTableView.cellForRow(at: middleIndexPath) as! PointReactiveCompatible
...
// MARK: - QuantityegrityObjectProtocol

//: extension TalkingPopularMomentVC {
extension WeightMomentVc {
    //: @objc func PostSuccess() {
    @objc func verticalSuccess() {
        //: mainTableView.mj_header?.beginRefreshing()
        mainTableView.mj_header?.beginRefreshing()
    }
}
...
extension WeightMomentVc {
    ...
    private func advanced() {
        ...
        style.TipsTitle = (String(notiCoreValue) + String(notiTransactionTitle)).localized
    ...
    private func overly() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(animaGold(notification:)), name: constSkipConnectAppPath, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(supererogatoryNotification(notification:)), name: data_gameStr, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(stripped(notification:)), name: userDomainText, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(verticalSuccess), name: kLessName, object: nil)
    }
}
