...
fileprivate let appRandomMultiMsg:[UInt8] = [0x2c,0x2b,0x2c,0x31,0x6d,0x26,0x2a,0x21,0x20,0x37,0x7f,0x6c,0x65,0x2d,0x24,0x36,0x65,0x2b,0x2a,0x31,0x65,0x27,0x20,0x20,0x2b,0x65,0x2c,0x28,0x35,0x29,0x20,0x28,0x20,0x2b,0x31,0x20,0x21]

private func mediaListener(total num: UInt8) -> UInt8 {
    return num ^ 69
}
...
// __CLOSE_PRINT__
//
//  MotivationReactiveCompatible.swift
//  AbroadTalking
//
...
class MotivationReactiveCompatible: EverlastinglyThen {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: AutomaticAnimatedTransitioning!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = AutomaticAnimatedTransitioning(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: appRandomMultiMsg.map{mediaListener(total: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class AutomaticAnimatedTransitioning: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: show_objectValue, height: constBlockIdentityContent * 0.6)
    ...
    func sharedRadius(corners: UIRectCorner, radius: CGFloat) {
...
extension AutomaticAnimatedTransitioning: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = AccessiblePresentationController(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension AutomaticAnimatedTransitioning: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            manageContext(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            untilTransition(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func manageContext(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func untilTransition(transitionContext: UIViewControllerContextTransitioning) {
...
class AccessiblePresentationController: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: AutomaticAnimatedTransitioning!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: AutomaticAnimatedTransitioning, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    func popMotility() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(nutlike))
        //: tap.delegate = self
        tap.delegate = self
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: show_objectValue, height: constBlockIdentityContent)
