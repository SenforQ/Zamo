...
fileprivate let user_fourWrittenTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_live_toast_bg" :*/
fileprivate let user_titleNothingPath:String = "btn_liexpected nearby satisfy"
fileprivate let notiModeCutSuccessfulName:String = "st_bgattach outside family"

/*: "btn_live_toast_arrow" :*/
fileprivate let showMiniData:String = "connection child itbtn_l"
fileprivate let dataShapeMsg:String = "ive_someone pack pic"
fileprivate let user_fireStr:String = "tip memory would changet_arrow"
...
var userMaleText = false

//: class TalkingLiveTabToast: UIView {
class IonView: UIView {
    // 气泡间距
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_fourWrittenTitle.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bubbleImgView: UIImageView = {
        ...
        var img = UIImage.lastTo(name: (String(user_titleNothingPath.prefix(6)) + "ve_toa" + String(notiModeCutSuccessfulName.prefix(5)))).resizableImage(withCapInsets: bubbleInsets, resizingMode: .stretch)
        //: imgV.image = img
        imgV.image = img
        //: return imgV
        return imgV
    ...
    private lazy var arrowImgView: UIImageView = {
        ...
        imgV.image = UIImage.lastTo(name: (String(showMiniData.suffix(5)) + String(dataShapeMsg.prefix(4)) + "toas" + String(user_fireStr.suffix(7))))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension IonView {
    ...
    func animaMax(superView: UIView) {
        //: guard LIVETABTOASTISSHOW == false else { return }
        guard userMaleText == false else { return }
        ...
        //: guard MentionTriggerAppManager.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        guard MentionTriggerAppManager.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        //: LIVETABTOASTISSHOW = true
        userMaleText = true

        //: textLab.text = MentionTriggerAppManager.share.appUserConfigMode.liveTabTips
        textLab.text = MentionTriggerAppManager.share.appUserConfigMode.liveTabTips
        //: let maxWidth = ScreenWidth - 48
        let maxWidth = show_objectValue - 48
