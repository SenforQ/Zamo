...
/*: "cellID" :*/
fileprivate let appCalledNowPath:String = "raw speed character dog agentcellID"

/*: "init(coder:) has not been implemented" :*/
fileprivate let userSpeciesUrl:[UInt8] = [0x9a,0x9d,0x9a,0x87,0xdb,0x90,0x9c,0x97,0x96,0x81,0xc9,0xda,0xd3,0x9b,0x92,0x80,0xd3,0x9d,0x9c,0x87,0xd3,0x91,0x96,0x96,0x9d,0xd3,0x9a,0x9e,0x83,0x9f,0x96,0x9e,0x96,0x9d,0x87,0x96,0x97]

private func deleteModify(make num: UInt8) -> UInt8 {
    return num ^ 243
}

/*: "Video Call" :*/
fileprivate let user_resolutionMsg:String = "Videslide split guard"
fileprivate let appActiveLovelyStr:[Character] = ["o"," ","C","a","l","l"]

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let data_mysterySlideMessage:String = "Video Cacomposition personal cooperative wave"
fileprivate let data_modifyProfileFormat:String = "ll list statute"
fileprivate let userBadName:String = "(%@ Poslim pull found"
fileprivate let notiTellMessage:String = "/Min)send definition theme shot"

/*: "Voice Call" :*/
fileprivate let app_allianceData:String = "me thread fineVoice"
fileprivate let notiBridgeHeadFormat:String = " Callperiod to exception"

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let userBlockValue:[Character] = ["V","o","i","c","e"," ","C","a","l","l"," ","(","%","@"," ","P","o"]
fileprivate let dataTradeAttentionFormat:String = "intdoorway"
fileprivate let k_tellUrl:String = "/Min)occur implement lat talk mount"

/*: "#F8F8F8" :*/
fileprivate let data_orientationMsg:String = "keep tell category#F8F8"
fileprivate let k_assistTitle:[Character] = ["F","8"]

/*: "Cancel" :*/
fileprivate let userConversionMsg:String = "Cancelpermission flag fail temp it"

/*: "#F5F5F5" :*/
fileprivate let noti_connectionName:[Character] = ["#","F","5","F","5","F","5"]

/*: "icon_lounge" :*/
fileprivate let appChinUrl:String = "icon_lofull want processor burn"
fileprivate let noti_universalCollectContent:String = "ureferge"
...
class WeightPopView: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: GloryPopType = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.roundChromaticColor()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.landEnableSize(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.modusOperandi()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.landEnableSize(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.thirdRevenue()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangMediumFont(fontSize: 16)
    var titleFont = UIFont.wordPicture(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(appCalledNowPath.suffix(6)))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + userDiskUrl) : userDiskUrl
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userSpeciesUrl.map{deleteModify(make: $0)}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func resList(cellTitleList: [String]!) {
        // 初始化
    ...
    func aPrice(type: GloryPopType, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if MentionTriggerAppManager.share.appStatus == AidNameConvertible.normal.rawValue {
                ...
                if MentionTriggerAppManager.share.loginUserMode.sex == TopicKeyRepresentable.male.rawValue {
                    ...
                    let videoAttrStr = String.positForExpensiveness(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.oppositeBy(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.thirdRevenue(), .font: UIFont.landEnableSize(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(user_resolutionMsg.prefix(4)) + String(appActiveLovelyStr)).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(data_mysterySlideMessage.prefix(8)) + String(data_modifyProfileFormat.prefix(3)) + String(userBadName.prefix(6)) + "ints" + String(notiTellMessage.prefix(5))).takeHomeDiskMini(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(app_allianceData.suffix(5)) + String(notiBridgeHeadFormat.prefix(5))).localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(userBlockValue) + dataTradeAttentionFormat.replacingOccurrences(of: "doorway", with: "s") + String(k_tellUrl.prefix(5))).takeHomeDiskMini(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(user_resolutionMsg.prefix(4)) + String(appActiveLovelyStr)).localized, (String(app_allianceData.suffix(5)) + String(notiBridgeHeadFormat.prefix(5))).localized]
            }
        }
    ...
    public func rubric() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: constBlockIdentityContent - tableViewHight, width: show_objectValue, height: tableViewHight), style: .plain)
        ...
        tableView.register(HarvestThen.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.factorSource(width: show_objectValue,
...
extension WeightPopView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? HarvestThen
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = HarvestThen(style: .default, reuseIdentifier: cellID)
        }

        //: if MentionTriggerAppManager.share.loginUserMode.sex == Gender.male.rawValue,
        if MentionTriggerAppManager.share.loginUserMode.sex == TopicKeyRepresentable.male.rawValue,
           //: MentionTriggerAppManager.share.loginUserMode.loungePlus == true,
           MentionTriggerAppManager.share.loginUserMode.loungePlus == true,
           //: type == .Call {
           type == .Call
        {
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && MentionTriggerAppManager.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && MentionTriggerAppManager.share.appStatus == AidNameConvertible.normal.rawValue {
    ...
    func tableView(_: UITableView, viewForFooterInSection _: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: show_objectValue, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: show_objectValue, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(data_orientationMsg.suffix(5)) + String(k_assistTitle)))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: show_objectValue, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((String(userConversionMsg.prefix(6))).localized, for: .normal)
    ...
    func tableView(_: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: show_objectValue, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: show_objectValue - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: show_objectValue, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(noti_connectionName)))
...
class HarvestThen: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.lastTo(name: (String(appChinUrl.prefix(7)) + noti_universalCollectContent.replacingOccurrences(of: "refer", with: "n")))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(noti_connectionName)))
        //: return lab
        return lab
        //: }()
    }()
