...
fileprivate let const_sharedKey:[UInt8] = [0x21,0x26,0x21,0x3c,0x60,0x2b,0x27,0x2c,0x2d,0x3a,0x72,0x61,0x68,0x20,0x29,0x3b,0x68,0x26,0x27,0x3c,0x68,0x2a,0x2d,0x2d,0x26,0x68,0x21,0x25,0x38,0x24,0x2d,0x25,0x2d,0x26,0x3c,0x2d,0x2c]

private func containerRun(scene num: UInt8) -> UInt8 {
    return num ^ 72
}

/*: "FFD855" :*/
fileprivate let const_statusStr:String = "actualD855"
...
class HearthrugView: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - dataInputMsg
                ...
                self.boundModePosition()
                //: self.refreshSliderValidScope(startX: self.videoStartX)
                self.wholeX(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_sharedKey.map{containerRun(scene: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (const_statusStr.replacingOccurrences(of: "actual", with: "FF")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: mainTableCellStr, y: 0, width: dataInputMsg, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - dataInputMsg, y: 0, width: dataInputMsg, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.judgeAdvertising(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: mainTableCellStr, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.judgeAdvertising(0, 0, 0, 0.6)
...
extension HearthrugView {
    ...
    func wholeX(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - dataInputMsg,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - dataInputMsg)
    }
}
...
extension HearthrugView {
    ...
    @objc func someMore(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.wholeX(startX: videoStartX)
    ...
    @objc func slueBe(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension HearthrugView {
    ...
    private func onlyAt() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(someMore(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(slueBe(gesture:)))
    ...
    func megascopic(_ xposition: CGFloat) {
    ...
    private func boundModePosition() {
