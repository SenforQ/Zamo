...
fileprivate let show_drownFormat:[UInt8] = [0x1c,0x21,0x1c,0x27,0xdb,0x16,0x22,0x17,0x18,0x25,0xed,0xdc,0xd3,0x1b,0x14,0x26,0xd3,0x21,0x22,0x27,0xd3,0x15,0x18,0x18,0x21,0xd3,0x1c,0x20,0x23,0x1f,0x18,0x20,0x18,0x21,0x27,0x18,0x17]

fileprivate func alterPressure(area num: UInt8) -> UInt8 {
...
typealias SlidingHandler = (_ direction: AssistantSlideDirection) -> Void
...
class ForeverView: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (mainTableCellStr - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_drownFormat.map{alterPressure(area: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.socialUnit(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: HearthrugView = {
        ...
        let overlay = HearthrugView(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension ForeverView {
    ...
    private func socialUnit(point: CGFloat) -> UIImage {
...
extension ForeverView: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = mainTableCellStr - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.showAppear(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.showAppear(isShow: true)
    }
...
extension ForeverView {
    ...
    private func activeAgent() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(mainTableCellStr)
