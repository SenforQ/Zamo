...
/*: "Finish" :*/
fileprivate let userShrinkUrl:String = "agent shallFinish"

/*: "Cancel" :*/
fileprivate let app_publishPath:String = "Cancelcalculate gesture"

/*: "btn_photo_choice_pre" :*/
fileprivate let k_actualClothesUrl:String = "btn_equity am to orange"
fileprivate let main_crushTitle:String = "o_chohappy support her"

/*: "btn_photo_add_nor" :*/
fileprivate let appSaveName:String = "path job disabled ingredient activitybtn_pho"
fileprivate let data_extentMsg:[Character] = ["t","o","_","a","d","d","_","n","o","r"]

/*: "/tmp" :*/
fileprivate let userDonationValue:String = "space go catch greatest assert/tmp"

/*: "Export session failed" :*/
fileprivate let app_preserveId:[Character] = ["E","x","p","o","r","t"," ","s","e","s","s","i","o","n"," "]
fileprivate let notiNumbValue:String = "faiyounged"

/*: "Export canceled" :*/
fileprivate let appStockData:String = "Exporrid resume pi perform"
fileprivate let user_toneMessage:String = "ableed"

/*: "Successful!" :*/
fileprivate let app_libraryMotionAdditionalStr:[Character] = ["S","u","c","c","e","s","s","f","u","l"]
fileprivate let mainMyId:String = "tap"

/*: "Unsupported video formats" :*/
fileprivate let kPurchaseName:String = "Unsulat inmate mark pair"
fileprivate let noti_feeId:String = "ed vcut body look proposal"
fileprivate let userFoundationName:String = "dotrmats"

/*: "yyyy-MM-dd-HH:mm:ss-SSS" :*/
fileprivate let userCareData:[Character] = ["y","y","y","y","-","M","M","-","d","d","-","H","H",":","m","m",":","s","s","-","S","S","S"]

/*: "/tmp/video- :*/
fileprivate let noti_startValue:String = "/tmp/vglobal share dial not"

/*: .mp4" :*/
fileprivate let noti_curStr:[Character] = [".","m","p","4"]
...
class CapturePickTool: NSObject {
    ...
    class func subjectMatter(allowImgCrop: Bool = false, needCircleCrop: Bool = false, maxCount: Int, allowPhoto: Bool, allowVideo: Bool) -> TZImagePickerController {
...
//        imagePicker?.preferredLanguage = "en"
        ...
        if allowImgCrop {
            ...
            var height = show_objectValue * 488 / 375
            //: if needCircleCrop == true {
            if needCircleCrop == true { // 切圆图
                //: height = ScreenWidth
                height = show_objectValue
            }
            //: imagePicker?.cropRect = CGRect(x: 0, y: (ScreenHeight-height)/2, width: ScreenWidth, height: height)
            imagePicker?.cropRect = CGRect(x: 0, y: (constBlockIdentityContent - height) / 2, width: show_objectValue, height: height)
        }
        //: if allowVideo {
        if allowVideo {
        ...
        imagePicker?.doneBtnTitleStr = (String(userShrinkUrl.suffix(6))).localized
        //: imagePicker?.cancelBtnTitleStr = "Cancel".localized
        imagePicker?.cancelBtnTitleStr = (String(app_publishPath.prefix(6))).localized

        //: imagePicker?.barItemTextFont = UIFont.pingfangFont(type: .Regular, fontSize: 15)
        imagePicker?.barItemTextFont = UIFont.landEnableSize(type: .Regular, fontSize: 15)
        //: imagePicker?.barItemTextColor = UIColor.appValueColor()
        imagePicker?.barItemTextColor = UIColor.todaySkinColor()
        ...
        imagePicker?.naviTitleFont = UIFont.landEnableSize(type: .Medium, fontSize: 17)
        //: imagePicker?.naviTitleColor = UIColor.appTitleColor()
        imagePicker?.naviTitleColor = UIColor.roundChromaticColor()
        ...
        imagePicker?.iconThemeColor = UIColor.thirdRevenue()

        //: imagePicker?.allowPickingOriginalPhoto = false
        imagePicker?.allowPickingOriginalPhoto = false
        //: imagePicker?.photoSelImage = UIImage.BundleImageNamed(name: "btn_photo_choice_pre")
        imagePicker?.photoSelImage = UIImage.lastTo(name: (String(k_actualClothesUrl.prefix(4)) + "phot" + String(main_crushTitle.prefix(5)) + "ice_pre"))
        //: imagePicker?.takePictureImage = UIImage.BundleImageNamed(name: "btn_photo_add_nor")
        imagePicker?.takePictureImage = UIImage.lastTo(name: (String(appSaveName.suffix(7)) + String(data_extentMsg)))
        ...
        imagePicker?.photoPickerPageDidRefreshStateBlock = { (_: UICollectionView?,
                ...
                doneButton?.titleLabel?.font = UIFont.landEnableSize(type: .Medium, fontSize: 14)
                ...
                if text.count > 0 && Int(text)! > 0 {
                    ...
                    doneButton?.setTitle((String(userShrinkUrl.suffix(6))).localized + "(\(text))", for: .normal)
                    //: } else {
                } else {
                    //: doneButton?.setTitle("Finish".localized, for: .normal)
                    doneButton?.setTitle((String(userShrinkUrl.suffix(6))).localized, for: .normal)
                }
                //: return
        }
        ...
        imagePicker?.photoPickerPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                doneButton?.backgroundColor = UIColor.thirdRevenue()
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
        ...
        imagePicker?.photoPreviewPageDidLayoutSubviewsBlock = { (_: UICollectionView?,
                ...
                selectButton?.frame = CGRect(x: show_objectValue - 5 - (selectButton?.frame.size.width)!, y: (selectButton?.frame.origin.y)!, width: (selectButton?.frame.size.width)!, height: (selectButton?.frame.size.height)!)
                //: let text = numberLabel?.text ?? "0"
                let text = numberLabel?.text ?? "0"
                //: if text.count > 0 && Int(text)! > 0 {
                if text.count > 0 && Int(text)! > 0 {
    ...
    class func pathOff(asset: PHAsset?, completion: @escaping (_ filePath: URL?) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: asset!, options: options) { (asset: AVAsset?, _: AVAudioMix?, _) in
            ...
            if asset!.isKind(of: AVURLAsset.self) {
                ...
                if !FileManager.default.fileExists(atPath: NSHomeDirectory().appending((String(userDonationValue.suffix(4))))) {
                    //: do {
                    do {
                        //: try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending("/tmp"), withIntermediateDirectories: true)
                        try FileManager.default.createDirectory(atPath: NSHomeDirectory().appending((String(userDonationValue.suffix(4)))), withIntermediateDirectories: true)
                        //: } catch {
                    } catch {}
                }
                ...
                exportSession?.exportAsynchronously(completionHandler: {
                    ...
                    case .failed:
                        ...
                        printLog(message: (String(app_preserveId) + notiNumbValue.replacingOccurrences(of: "young", with: "l")))
                    //: case .cancelled:
                    case .cancelled:
                        //: printLog(message: "Export canceled")
                        printLog(message: (String(appStockData.prefix(5)) + "t canc" + user_toneMessage.replacingOccurrences(of: "able", with: "el")))
                    //: case .completed:
                    case .completed:
                        //: printLog(message: "Successful!")
                        printLog(message: (String(app_libraryMotionAdditionalStr) + mainMyId.replacingOccurrences(of: "tap", with: "!")))
                        //: DispatchQueue.main.async {
                        DispatchQueue.main.async {
                ...
                printLog(message: (String(kPurchaseName.prefix(4)) + "pport" + String(noti_feeId.prefix(4)) + "ideo " + userFoundationName.replacingOccurrences(of: "dot", with: "fo")))
            }
        }
    }
    ...
    private class func behindPage() -> String {
        //: let formater = DateFormatter()
        let formater = DateFormatter()
        //: formater.dateFormat = "yyyy-MM-dd-HH:mm:ss-SSS"
        formater.dateFormat = (String(userCareData))
        //: let outputPath = NSHomeDirectory().appending("/tmp/video-\(formater.string(from: Date()))-\(Int.random(in: 0...10000000)).mp4")
        let outputPath = NSHomeDirectory().appending((String(noti_startValue.prefix(6)) + "ideo-") + "\(formater.string(from: Date()))-\(Int.random(in: 0 ... 10_000_000))" + (String(noti_curStr)))
        //: return outputPath
        return outputPath
    }
}
