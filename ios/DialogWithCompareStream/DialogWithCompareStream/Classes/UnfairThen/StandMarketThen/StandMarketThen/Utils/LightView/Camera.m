...
@interface MicDueData : NSObject

+ (instancetype)sharedInstance;

//: 请打开相机权限
@property (nonatomic, copy) NSString *data_halfClipFormat;

//: Width
@property (nonatomic, copy) NSString *dataCharmFormat;

//: Height
@property (nonatomic, copy) NSString *const_ridStr;

@end

@implementation MicDueData

+ (instancetype)sharedInstance {
    static MicDueData *instance = nil;
...
+ (NSData *)MicDueDataToData:(NSArray<NSString *> *)value {
...
- (Byte *)MicDueDataToCache:(Byte *)data {
    int jump = data[0];
    Byte disable = data[1];
    int withinMonth = data[2];
    for (int i = withinMonth; i < withinMonth + jump; i++) {
    ...
    data[withinMonth + jump] = 0;
    return data + withinMonth;
}

- (NSString *)StringFromMicDueData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self MicDueDataToCache:data]];
}

//: 请打开相机权限
- (NSString *)data_halfClipFormat {
    if (!_data_halfClipFormat) {
		NSArray<NSString *> *origin = @[@"21", @"95", @"4", @"79", @"71", @"14", @"22", @"69", @"232", @"242", @"68", @"27", @"223", @"70", @"250", @"23", @"69", @"251", @"25", @"69", @"252", @"226", @"72", @"248", @"239", @"8"];
		NSData *data = [MicDueData MicDueDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _data_halfClipFormat = [self StringFromMicDueData:value];
    }
    return _data_halfClipFormat;
}

//: Height
- (NSString *)const_ridStr {
    if (!_const_ridStr) {
		NSArray<NSString *> *origin = @[@"6", @"63", @"3", @"135", @"164", @"168", @"166", @"167", @"179", @"5"];
		NSData *data = [MicDueData MicDueDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _const_ridStr = [self StringFromMicDueData:value];
    }
    return _const_ridStr;
}

//: Width
- (NSString *)dataCharmFormat {
    if (!_dataCharmFormat) {
		NSArray<NSString *> *origin = @[@"5", @"38", @"5", @"18", @"148", @"125", @"143", @"138", @"154", @"142", @"50"];
		NSData *data = [MicDueData MicDueDataToData:origin];
        Byte *value = (Byte *)data.bytes;
        _dataCharmFormat = [self StringFromMicDueData:value];
    }
    return _dataCharmFormat;
}
                         ...
                         ask:(BOOL)needYuvOutput
{
    ...
    if (self) {
        ...
        self.videoDevice = [self polaroidLandCameraThroughPosition:iDevicePosition];
...
- (void)line:(CGPoint)point location:(CGRect)frame
{
    ...
    [self renounce:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           placement:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                doingLevel:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)renounce:(AVCaptureFocusMode)focusMode placement:(AVCaptureExposureMode)exposureMode doingLevel:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self enableApplication:^(AVCaptureDevice *captureDevice) {
...
- (void)enableApplication:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self polaroidLandCameraThroughPosition:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self reAuthorization]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:[MicDueData sharedInstance].dataCharmFormat] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:[MicDueData sharedInstance].const_ridStr] floatValue];
...
- (CGRect)origin:(CGRect)rect roundCapacity:(BOOL)bScaleToFit
{
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:[MicDueData sharedInstance].dataCharmFormat] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:[MicDueData sharedInstance].const_ridStr] floatValue];
...
- (BOOL)reAuthorization
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:[MicDueData sharedInstance].data_halfClipFormat
...
- (AVCaptureDevice *)polaroidLandCameraThroughPosition:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(bright:index:cur:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate bright:captureOutput index:sampleBuffer cur:connection];
        }
    }
