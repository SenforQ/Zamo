...
/*: "Select a date" :*/
fileprivate let data_springTitle:[Character] = ["S","e","l","e","c","t"," ","a"," ","d"]
fileprivate let userNaturalText:[Character] = ["a","t","e"]
...
protocol TreeViewDelegate: NSObjectProtocol {
...
class PubliclyRocketView: UnfairThen, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func insideCombine() {
        //: super.initPickView()
        super.insideCombine()

        //: titleString = "Select a date".localized
        titleString = (String(data_springTitle) + String(userNaturalText)).localized
        ...
        momentEnableMedal(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func momentEnableMedal(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func replaceButton(button: UIButton) {
        //: super.confirmClick(button: button)
        super.replaceButton(button: button)
        //: refreshPickViewData()
        towardPermission()
        ...
        if let day = selectDay {
            ...
            delegate?.haveDay(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.haveDay(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return streetSmart(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func streetSmart(year: Int, month: Int) -> Int {
