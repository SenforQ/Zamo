...
class RugRecognizerDelegate: EverlastinglyThen {
    //: var type = LoginType.PhoneLogin
    var type = LoginExpressionConvertible.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    private lazy var pwdView: SmartReactiveCompatible = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = SmartReactiveCompatible(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension RugRecognizerDelegate {
    /// 修改密码
    //: private func req_modifyPwd(_ pwd: String, _ vcode: String?) {
    private func example(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        ...
        SpeakRequestTool.mutual(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
