...
fileprivate let constHarvestFormat:String = "land exactly proposeicon_k"
fileprivate let appPregnantPath:String = "eye pan treat problem intoong_ko"
fileprivate let user_writtenKey:String = "FAULT"

/*: "page" :*/
fileprivate let user_ridContent:[UInt8] = [0x65,0x67,0x61,0x70]

/*: "uid" :*/
fileprivate let constEvenMayTitle:[UInt8] = [0x64,0x69,0x75]

/*: "list" :*/
fileprivate let notiSectionValue:[Character] = ["l","i","s","t"]
...
class VolumeAidViewDelegate: EverlastinglyThen {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [CoreItemModel]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: dataPathFormat, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(DisableReactiveCompatible.self, forCellWithReuseIdentifier: DisableReactiveCompatible.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.lastTo(name: (String(constHarvestFormat.suffix(6)) + String(appPregnantPath.suffix(6)) + "ng_de" + user_writtenKey.lowercased()))
...
extension VolumeAidViewDelegate {
    ...
    private func draft() {
        ...
        let params = [String(bytes: user_ridContent.reversed(), encoding: .utf8)!: pageIndex, String(bytes: constEvenMayTitle.reversed(), encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        TopicThen.adminCompletion(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(String(notiSectionValue))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = DigitiserMomentModel.deserialize(from: dict as? Dictionary) {
...
extension VolumeAidViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: DisableReactiveCompatible.className(), for: indexPath) as! DisableReactiveCompatible
        //: if indexPath.row < self.postDataArr.count {
        if indexPath.row < self.postDataArr.count {
            //: let model = self.postDataArr[indexPath.row]
            let model = self.postDataArr[indexPath.row]
            //: cell.refreshItem(model)
            cell.intervalUp(model)
        }
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = AppropriateSchemeViewDelegate(model: detailModel)
...
extension VolumeAidViewDelegate: JXPagingViewListViewDelegate {
...
extension VolumeAidViewDelegate {
    ...
    func responseController(_ userModel: TipRugInfoModel) {
