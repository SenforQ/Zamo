...
fileprivate let kPurchaseValue:[UInt8] = [0x65,0x62,0x65,0x78,0x24,0x6f,0x63,0x68,0x69,0x7e,0x36,0x25,0x2c,0x64,0x6d,0x7f,0x2c,0x62,0x63,0x78,0x2c,0x6e,0x69,0x69,0x62,0x2c,0x65,0x61,0x7c,0x60,0x69,0x61,0x69,0x62,0x78,0x69,0x68]

private func laughButton(crush num: UInt8) -> UInt8 {
    return num ^ 12
}

/*: "Upload Avatar" :*/
fileprivate let noti_earlyFormat:[Character] = ["U","p","l","o","a","d"]
fileprivate let appErrCellFormat:String = " Avatartoo relieve bubble heavy"

/*: "btn_me_edit_photo_add_nor" :*/
fileprivate let showAngleTitle:[Character] = ["b","t","n","_","m","e","_","e","d","i","t","_","p","h","o","t","o","_","a","d","d","_","n"]
fileprivate let appCenterHorseValue:[Character] = ["o","r"]

/*: "btn_me_edit_photo_delete" :*/
fileprivate let showYoungPath:String = "btn_meend must environment listen"
fileprivate let dataPartStr:[Character] = ["_","e"]
fileprivate let showCosMessage:String = "dit_phresolution this comment ad per"
fileprivate let main_spendId:String = "elefilter"

/*: "#FF506D" :*/
fileprivate let appIndicatorId:[Character] = ["#"]
fileprivate let show_innerId:[Character] = ["F","F","5","0","6","D"]

/*: "Reviewing" :*/
fileprivate let noti_whenMsg:String = "traveler harassmentRevi"

/*: "icon_edit_avatar" :*/
fileprivate let const_mRelieveTitle:String = "currently"
fileprivate let dataSmoothValue:[Character] = ["c","o","n","_","e","d","i","t","_","a","v","a","t","a","r"]

/*: "Sample" :*/
fileprivate let main_hazardData:[Character] = ["S","a","m","p","l","e"]
...
public protocol DiskPointReactiveCompatible: NSObject {
    /// 头像add
    //: func addAvatarPhoto(_ icon: UIImage)
    func chooseCancel(_ icon: UIImage)
    /// 删除头像
...
class EncounterViewCell: UITableViewCell {
    //: var coverPath = ""
    var coverPath = ""
    //: open weak var delegate: EditAvatarDelegate?
    open weak var delegate: DiskPointReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kPurchaseValue.map{laughButton(crush: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var tipsLB: UILabel = {
        ...
        lb.text = (String(noti_earlyFormat) + String(appErrCellFormat.prefix(7))).localized
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.setImage(UIImage.lastTo(name: (String(showAngleTitle) + String(appCenterHorseValue))), for: .normal)
        //: btn.imageView?.contentMode = .scaleAspectFill
        btn.imageView?.contentMode = .scaleAspectFill
        //: btn.addTarget(self, action: #selector(iconbtnclick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(captureWeightOffer), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var changeBtn: UIButton = {
        ...
        btn.setImage(UIImage.lastTo(name: (String(showYoungPath.prefix(6)) + String(dataPartStr) + String(showCosMessage.prefix(6)) + "oto_d" + main_spendId.replacingOccurrences(of: "filter", with: "te"))), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
    ...
    private lazy var statusLB: UILabel = {
        ...
        lb.backgroundColor = UIColor(hex: (String(appIndicatorId) + String(show_innerId)))
        //: lb.text = "Reviewing".localized
        lb.text = (String(noti_whenMsg.suffix(4)) + "ewing").localized
    ...
    lazy var sampleBtn: UIButton = {
        ...
        btn.setImage(UIImage.lastTo(name: (const_mRelieveTitle.replacingOccurrences(of: "currently", with: "i") + String(dataSmoothValue))), for: .normal)
        //: btn.addTarget(self, action: #selector(sampleBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(smooth), for: .touchUpInside)
        ...
        lb.text = (String(main_hazardData)).localized
        //: view.addSubview(lb)
        view.addSubview(lb)
...
extension EncounterViewCell {
    //: func setCellView(iconPath: String, status: Int) {
    func controlWrite(iconPath: String, status: Int) {
...
extension EncounterViewCell {
    ...
    @objc private func smooth() {
        ...
        let vc = AliveViewDelegate(images: [UIImage.lastTo(name: (const_mRelieveTitle.replacingOccurrences(of: "currently", with: "i") + String(dataSmoothValue)))], index: 0, type: .noControl)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.transplant()?.navigationController?.pushViewController(vc, animated: true)
    }

    //: @objc private func changeBtnClick() {
    @objc private func programClick() {
        //: iconBtn.setImage(UIImage.BundleImageNamed(name: "btn_me_edit_photo_add_nor"), for: .normal)
        iconBtn.setImage(UIImage.lastTo(name: (String(showAngleTitle) + String(appCenterHorseValue))), for: .normal)
    ...
    @objc private func captureWeightOffer() {
            ...
            if isOpen {
                ...
                vc.didFinishPickingPhotosHandle = { [weak self] (photos: [UIImage]?,
                        ...
                        if photos?.count ?? 0 > 0 {
                            ...
                            self.delegate?.chooseCancel(photos!.first!)
