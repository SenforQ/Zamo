...
fileprivate let constParticleMessage:[UInt8] = [0xb4,0xb3,0xb4,0xa9,0xf5,0xbe,0xb2,0xb9,0xb8,0xaf,0xe7,0xf4,0xfd,0xb5,0xbc,0xae,0xfd,0xb3,0xb2,0xa9,0xfd,0xbf,0xb8,0xb8,0xb3,0xfd,0xb4,0xb0,0xad,0xb1,0xb8,0xb0,0xb8,0xb3,0xa9,0xb8,0xb9]

private func abductAcross(mirror num: UInt8) -> UInt8 {
    return num ^ 221
}

/*: "icon_photo_giftbg" :*/
fileprivate let dataLiteSweepMessage:String = "donationon"
fileprivate let appFirmDitUrl:[Character] = ["i","f","t","b","g"]

/*: "Send" :*/
fileprivate let app_objectBoyText:String = "Sendcontainer between"

/*: "nav_back_black_nor" :*/
fileprivate let notiCostContent:[Character] = ["n","a","v","_","b","a"]
fileprivate let show_imageValue:String = "very outside anonymous another subck_blac"
fileprivate let app_cuteMessage:String = "k_norlife zone inmate over red"

/*: "toUid" :*/
fileprivate let k_dismissGrowthFormat:String = "toUidqualify hold preserve family"

/*: "giftId" :*/
fileprivate let dataNativeUrl:[Character] = ["g","i","f","t","I","d"]

/*: "giftNum" :*/
fileprivate let kUniformText:String = "giftNumpressure para plat implement explore"

/*: "unlockMsgId" :*/
fileprivate let constPropertyFormat:String = "connect as hide nature choiceunlockM"
fileprivate let show_termsAcquirePunishName:String = "end would so instruction againstsgId"

/*: "Load failed, return to retry" :*/
fileprivate let data_advertStr:[Character] = ["L","o","a","d"," ","f","a","i","l","e","d",","," ","r","e","t","u","r","n"]
fileprivate let show_raftUrl:[Character] = [" ","t","o"," ","r","e","t","r","y"]

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let show_recordPath:[UInt8] = [0x82,0xa9,0xa9,0xa8,0xec,0xad,0xec,0xe9,0x8c,0xec,0xe4,0xe9,0x8c,0xec,0x8b,0xa3,0xa0,0xa8,0xec,0xaf,0xa3,0xa5,0xa2,0xbf,0xe5,0xec,0xb8,0xa3,0xec,0xb9,0xa2,0xa0,0xa3,0xaf,0xa7]

private func noseConduct(put num: UInt8) -> UInt8 {
    return num ^ 204
}

/*: "icon_photo_lock" :*/
fileprivate let noti_bearFormat:String = "icon_prun me"
fileprivate let mainAlongsideMessage:String = "K"

/*: "Click on the screen to play this video(Burn After Reading)" :*/
fileprivate let user_methodValue:[UInt8] = [0x5e,0x71,0x74,0x7e,0x76,0x3d,0x72,0x73,0x3d,0x69,0x75,0x78,0x3d,0x6e,0x7e,0x6f,0x78,0x78,0x73,0x3d,0x69,0x72,0x3d,0x6d,0x71,0x7c,0x64,0x3d,0x69,0x75,0x74,0x6e,0x3d,0x6b,0x74,0x79,0x78,0x72,0x35,0x5f,0x68,0x6f,0x73,0x3d,0x5c,0x7b,0x69,0x78,0x6f,0x3d,0x4f,0x78,0x7c,0x79,0x74,0x73,0x7a,0x34]

private func volumeWorth(multiple num: UInt8) -> UInt8 {
    return num ^ 29
}

/*: "icon_photo_burn" :*/
fileprivate let userBoundFormat:[Character] = ["i","c","o","n","_","p","h","o","t","o"]
fileprivate let dataToneMessage:[Character] = ["_","b","u","r","n"]

/*: "Already burned" :*/
fileprivate let mainCurriculumText:String = "faculty of stage third clubAlrea"
fileprivate let showHarkIssueSparkValue:String = "dy bufour mode scale child care"
...
// __CLOSE_PRINT__
//
//  DriveBoundPlayerDelegate.swift
//  AbroadTalking
//
...
typealias IntimateVideoBlock = (_ model: ChatEveryoneModel) -> Void
//: typealias VideoStatusBlock = (_ status: VideoPlayerStatus) -> Void
typealias VideoStatusBlock = (_ status: PubliclyWouldPlayerStatus) -> Void

//: class TalkingIntimateVideoPreviewVC: TalkingBaseViewController {
class DriveBoundPlayerDelegate: EverlastinglyThen {
    ...
    //: private var videoTotalData = CellData(direction: .MsgDirectionIncoming)
    private var videoTotalData = CellData(direction: .MsgDirectionIncoming) // 视频消息数据
    //: private var videoModel = AbTalkingChatMsgVideoModel()
    private var videoModel = ChatEveryoneModel() // 当前私密视频模型
    ...
    ///   - videoCellData: 视频模型
    ///   - toUid: 对方Id
    //: init(videoCellData: CellData, toUid: String) {
    init(videoCellData: CellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (videoTotalData.msgModel.user.uid == Int(MentionTriggerAppManager.share.loginUserMode.userID))
        isMySelf = (videoTotalData.msgModel.user.uid == Int(MentionTriggerAppManager.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constParticleMessage.map{abductAcross(mirror: $0)}, encoding: .utf8)!)
    }
    ...
    override func loadView() {
        ...
        if #available(iOS 13.2, *) {
            ...
            if MentionTriggerAppManager.share.appConfigMode.disableShootScreen {
                ...
                let bgView = RateShieldView(frame: CGRect(x: 0, y: 0, width: show_objectValue, height: constBlockIdentityContent - appMarginFormat))
                //: self.view = bgView
                self.view = bgView
            }
            //: } else {
    ...
    private lazy var videoProgressView: AssistantView = {
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.lastTo(name: (dataLiteSweepMessage.replacingOccurrences(of: "donation", with: "ic") + "_photo_g" + String(appFirmDitUrl)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.contactMoment(urlStr: videoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var desLab: UILabel = {
        ...
        lab.font = UIFont.wordPicture(fontSize: 19)
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(app_objectBoyText.prefix(4))).localized, for: .normal)
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 19)
        btn.titleLabel?.font = UIFont.landEnableSize(type: .Medium, fontSize: 19)
        ...
        btn.addTarget(self, action: #selector(pauseSend), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var progressView: MentionHomeSumdownView = {
        //: let frame = CGRect(x: ScreenWidth-42-15, y: StatusBarHeight+10, width: 42, height: 42)
        let frame = CGRect(x: show_objectValue - 42 - 15, y: app_pathData + 10, width: 42, height: 42)
        //: let v = CircleCountdownView(frame: frame)
        let v = MentionHomeSumdownView(frame: frame)
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.lastTo(name: (String(notiCostContent) + String(show_imageValue.suffix(7)) + String(app_cuteMessage.prefix(5)))).withRenderingMode(.alwaysTemplate)
...
extension DriveBoundPlayerDelegate {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func positivityNum(giftId: String, giftNum: String) {
        ...
        params[(String(k_dismissGrowthFormat.prefix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(dataNativeUrl))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(kUniformText.prefix(7)))] = giftNum
        //: params["unlockMsgId"] = self.videoTotalData.msgModel.msgInfo.msgId
        params[(String(constPropertyFormat.suffix(7)) + String(show_termsAcquirePunishName.suffix(4)))] = self.videoTotalData.msgModel.msgInfo.msgId

        // 送礼接口
        //: TalkingChatRequestTool.req_SendGiftPrivete(param: params, completion: { succeed, result, errorModel in
        GoUpReactiveCompatible.routeEffectForward(param: params, completion: { succeed, result, errorModel in
            //: if succeed {
...
extension DriveBoundPlayerDelegate {
    ...
    @objc private func pauseSend() {
        //: req_sendGiftMsg(giftId: videoModel.giftId, giftNum: "1")
        positivityNum(giftId: videoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnVideoTapGesture() {
    @objc private func editStop() {
        //: guard MentionTriggerAppManager.share.networkStatus != .Unavailable else {
        guard MentionTriggerAppManager.share.networkStatus != .Unavailable else {
            //: func__showStatusBarErrorMsg(showMsg: kNetErrorMsg)
            outBarMsg(showMsg: showLicenseOutputName)
            //: return
        ...
        LovelyLifetimeProgressHUD.pointBlank(superView: self.view)
        ...
        self.videoStatusBlock = { [weak self] status in
            ...
            guard status == .Playing else {
                ...
                if status == .Failed {
                    ...
                    self.outBarMsg(showMsg: (String(data_advertStr) + String(show_raftUrl)).localized)
                }
                //: return
                return
            }
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.genderTime(totalTime: 600, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension DriveBoundPlayerDelegate {
    ...
    @objc private func customStrength() {
...
extension DriveBoundPlayerDelegate {
    ...
    private func surchargeForSpeaker(urlStr: String, isMosaic: Bool) {
        ...
        guard isMySelf == false else { // 本人发的图片，展示本地缓存资源
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.imageView.image = UIImage.needAge(image, barMagnitudeerval: 90)
                self.imageView.image = UIImage.needAge(image, barMagnitudeerval: 90)
                //: } else {
            } else {
        ...
        imageView.valuation(urlStr: urlStr) { [weak self] image in
            //: guard let self = self else { return }
            guard let self = self else { return }
    ...
    private func bandyAcross() {
        ...
        self.player.gapCap(url: videoPath, view: self.videoView)
    }
}

// MARK: - DirectlyThen, SignatureReactiveCompatible

//: extension TalkingIntimateVideoPreviewVC: TalkingVideoPlayerDelegate, InteractionViewDelegate {
extension DriveBoundPlayerDelegate: DirectlyThen, SignatureReactiveCompatible {
    // MARK: - DirectlyThen
    ...
    func guideStatus(player _: StandardThen, status: PubliclyWouldPlayerStatus) {
        ...
        self.videoProgressView.quote(status: status)
        //: if status == .Playing {
        if status == .Playing {
    ...
    func birthday(player _: StandardThen, duration: Int, currentTime: Int) {
        ...
        self.videoProgressView.clean(duration: duration, currentTime: currentTime)
    }
    ...
    func clueWithoutStride(player _: StandardThen, progress: CGFloat) {
        ...
        self.videoProgressView.manage(value: progress)
    }

    // MARK: - SignatureReactiveCompatible
...
extension DriveBoundPlayerDelegate {
    ...
    private func makeFor() {
        ...
        videoProgressView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(appMarginFormat + 30)
            //: make.bottom.equalToSuperview().offset(-(kDeviceSafeBottomHeight+64))
            make.bottom.equalToSuperview().offset(-(userDiskUrl + 64))
        }
        ...
        desLab.snp.makeConstraints { make in
            ...
            make.centerY.equalToSuperview().offset(appMarginFormat / 2)
        }
        ...
        progressView.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(show_objectValue - 42 - 15)
    ...
    private func weekManual() {
        // 添加接收到音视频通话通知
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(startTalking),
                                               selector: #selector(customStrength),
    ...
    private func of() {
        ...
        if videoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.surchargeForSpeaker(urlStr: videoModel.coverImg, isMosaic: true)
            ...
            self.videoProgressView.clean(duration: videoModel.duration, currentTime: 0)
            //: self.maskView.isHidden = false
            self.maskView.isHidden = false
            //: self.desLab.text = "Need a %@ (%@ Gold coins) to unlock".localizedArguments(videoModel.giftName, videoModel.giftPrice)
            self.desLab.text = String(bytes: show_recordPath.map{noseConduct(put: $0)}, encoding: .utf8)!.takeHomeDiskMini(videoModel.giftName, videoModel.giftPrice)
            ...
            if videoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.lastTo(name: (String(noti_bearFormat.prefix(6)) + "hoto_loc" + mainAlongsideMessage.lowercased()))
                //: self.desLab.text = "Click on the screen to play this video(Burn After Reading)".localized
                self.desLab.text = String(bytes: user_methodValue.map{volumeWorth(multiple: $0)}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnVideoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(editStop))
                ...
                self.intimateStatusImgV.image = UIImage.lastTo(name: (String(userBoundFormat) + String(dataToneMessage)))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(mainCurriculumText.suffix(5)) + String(showHarkIssueSparkValue.prefix(5)) + "rned").localized
            }
