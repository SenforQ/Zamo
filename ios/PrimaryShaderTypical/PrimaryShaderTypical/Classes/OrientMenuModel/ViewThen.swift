...
fileprivate let noti_darkValue:[UInt8] = [0x64,0x63,0x64,0x79,0x25,0x6e,0x62,0x69,0x68,0x7f,0x37,0x24,0x2d,0x65,0x6c,0x7e,0x2d,0x63,0x62,0x79,0x2d,0x6f,0x68,0x68,0x63,0x2d,0x64,0x60,0x7d,0x61,0x68,0x60,0x68,0x63,0x79,0x68,0x69]

private func claimExecutive(future num: UInt8) -> UInt8 {
    return num ^ 13
}

/*: "Reply" :*/
fileprivate let constConcreteFeatureRemarkValue:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let app_stretchKey:[Character] = ["#","F","9","5","1","5","1"]
...
let dataTotalRequestKey = (app_pathData + 80.0)
...
class ViewThen: UIView {
    ...
    var startDismissBlock: ((_ model: AutomaticThen) -> Void)? // 开始移除视图Block
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_darkValue.map{claimExecutive(future: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .landEnableSize(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .landEnableSize(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(constConcreteFeatureRemarkValue)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(app_stretchKey))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension ViewThen {
    ...
    func writer(_ model: AutomaticThen?) {
    ...
    func suite(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -dataTotalRequestKey
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func noRefuse() {
        ...
        if currModel.chatType == .service {
            ...
            RiseMalePushManager.share.bubbleAdjust(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func busy(gesture: UISwipeGestureRecognizer) {
...
extension ViewThen {
    ...
    func hideTake() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.choiceRest(urlStr: userInfo.headPic, placeImg: UIImage.delayConvert(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.sinceHeadQuit(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
            ...
            headImgView.image = UIImage.appearLine()
            //: iconBorder.isHidden = true
            iconBorder.isHidden = true
        }

        //: msgShowLabel.attributedText = TalkingChatListTableCell.handleShowC2CDetailTextOfMessage(model: currModel)
        msgShowLabel.attributedText = TriggerThen.controlRender(model: currModel)
    }

    /// 添加视图
    //: private func setupSubviews() {
    private func overDecision() {
        //: self.frame = CGRect(x: 0, y: -TalkingMessageNotificationView_H, width: ScreenWidth, height: TalkingMessageNotificationView_H)
        self.frame = CGRect(x: 0, y: -dataTotalRequestKey, width: show_objectValue, height: dataTotalRequestKey)
