...
fileprivate let notiLipName:[UInt8] = [0xd7,0xd0,0xd7,0xca,0x96,0xdd,0xd1,0xda,0xdb,0xcc,0x84,0x97,0x9e,0xd6,0xdf,0xcd,0x9e,0xd0,0xd1,0xca,0x9e,0xdc,0xdb,0xdb,0xd0,0x9e,0xd7,0xd3,0xce,0xd2,0xdb,0xd3,0xdb,0xd0,0xca,0xdb,0xda]

private func quickImpression(incident num: UInt8) -> UInt8 {
    return num ^ 190
}

/*: "icon_coinxxx" :*/
fileprivate let dataSearchedStr:String = "icon_cell bar destination"

/*: "icon_gift_num_bg" :*/
fileprivate let k_mannerValue:String = "feature fair whole value advertisingicon_g"
fileprivate let k_jungleNativeKey:String = "m_bgwrite fade future constraint"
...
class DirtyReusableView: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = TipTransformable()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiLipName.map{quickImpression(incident: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.landEnableSize(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: EarthThen = {
        //: let btn = TalkingButton.init()
        let btn = EarthThen()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.landEnableSize(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.lastTo(name: (String(dataSearchedStr.prefix(5)) + "coinxxx")), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.lastTo(name: (String(k_mannerValue.suffix(6)) + "ift_nu" + String(k_jungleNativeKey.prefix(4))))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.landEnableSize(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension DirtyReusableView {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func observerQueryed(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.thirdRevenue().cgColor
    ...
    func rowCompartment(currenmodel: TipTransformable, giftType: DeviseExpressionConvertible) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.contactMoment(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.contactMoment(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.landEnableSize(type: .Regular, fontSize: 11),
...
extension DirtyReusableView {
    ...
    func form() {
        ...
        bgView.backgroundColor = UIColor.indoors()
