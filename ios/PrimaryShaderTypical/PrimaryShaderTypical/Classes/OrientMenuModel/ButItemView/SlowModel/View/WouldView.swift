...
fileprivate let showAirMsg:[UInt8] = [0xd1,0xd6,0xd1,0xcc,0x90,0xdb,0xd7,0xdc,0xdd,0xca,0x82,0x91,0x98,0xd0,0xd9,0xcb,0x98,0xd6,0xd7,0xcc,0x98,0xda,0xdd,0xdd,0xd6,0x98,0xd1,0xd5,0xc8,0xd4,0xdd,0xd5,0xdd,0xd6,0xcc,0xdd,0xdc]
...
@objc protocol ExtensiveObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func betweenScroll()

    //: @objc optional func endScroll()
    @objc optional func transformScroll()
}

//: class TalkingDanmuMsgListTableView: UIView {
class WouldView: UIView {
    ...
    var msgArray = [AssistantDecideTransformable]() /// 消息数组(数据源)
    //: var  tempMsgArray = Array<TalkingLiveRoomDanmuModel>()
    var tempMsgArray = [AssistantDecideTransformable]() /// 用于存储消息还未刷新到tableView的时候接收到的消息
    //: var inPending = false
    var inPending = false /// 是否处于爬楼状态
    //: var refreshTimer: Timer?
    var refreshTimer: Timer? /// 刷新定时器
    //: var reloadType: ReloadLiveMsgRoomType?
    var reloadType: ScaleOutsideHashable?
    //: weak var delegate: TalkingDanmuMsgListTableViewDelegate?
    weak var delegate: ExtensiveObjectProtocol?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showAirMsg.map{$0^184}, encoding: .utf8)!)
    }
...
extension WouldView {
    ...
    func miniGuide() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(labouring), userInfo: nil, repeats: true)
    }

    //: func stopTimer() {
    func incrementalCost() {
...
extension WouldView {
    ...
    func pointFront(msgModel: AssistantDecideTransformable?) {
    ...
    func menuBottom() {
        ...
        if AllHeight > notiReadId {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < notiReadId {
                ...
                self.msgTableView.height = notiReadId
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = notiReadId - AllHeight
        ...
        optical(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func optical(animated: Bool) {
...
extension WouldView: UITableViewDelegate, UITableViewDataSource {
    ...
    func extremeFrame(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.transformScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(k_successValue)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = DanmuBassCell.multiTransition(tableView: tableView, msg: msgModel, indexPath: indexPath)
