...
fileprivate let show_automaticallyRandomKey:[UInt8] = [0x28,0x2f,0x28,0x35,0x69,0x22,0x2e,0x25,0x24,0x33,0x7b,0x68,0x61,0x29,0x20,0x32,0x61,0x2f,0x2e,0x35,0x61,0x23,0x24,0x24,0x2f,0x61,0x28,0x2c,0x31,0x2d,0x24,0x2c,0x24,0x2f,0x35,0x24,0x25]

private func factorMakeup(bit num: UInt8) -> UInt8 {
    return num ^ 65
}

/*: "move" :*/
fileprivate let showDesignName:String = "publishve"
...
class MoveView: UIView {
    ...
    var danumuMsgArray: [WritingTransformable] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [PeerItemView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<WritingTransformable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_automaticallyRandomKey.map{factorMakeup(bit: $0)}, encoding: .utf8)!)
    }
    ...
    func ready(_ danumuMsgModel: WritingTransformable?) {
    ...
    @objc func thatAction(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(thatAction(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension MoveView: UIGestureRecognizerDelegate {
    ...
    func premium(for danumuMsgModel: WritingTransformable?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = PeerItemView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (showDesignName.replacingOccurrences(of: "publish", with: "mo")))
    ...
    func popGender(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                premium(for: model, isFirst: isFirst)
