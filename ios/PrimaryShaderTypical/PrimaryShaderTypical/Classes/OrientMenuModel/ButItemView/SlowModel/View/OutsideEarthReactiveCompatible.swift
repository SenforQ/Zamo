...
fileprivate let noti_birthdayTitle:[UInt8] = [0xe5,0xe2,0xe5,0xf8,0xa4,0xef,0xe3,0xe8,0xe9,0xfe,0xb6,0xa5,0xac,0xe4,0xed,0xff,0xac,0xe2,0xe3,0xf8,0xac,0xee,0xe9,0xe9,0xe2,0xac,0xe5,0xe1,0xfc,0xe0,0xe9,0xe1,0xe9,0xe2,0xf8,0xe9,0xe8]

private func denyAccess(favorite num: UInt8) -> UInt8 {
    return num ^ 140
}

/*: "#2DF2FF" :*/
fileprivate let show_transformKey:String = "particle push lite#2DF2FF"

/*: "Live" :*/
fileprivate let const_punishStr:String = "Liveprivate framework initial sun"

/*: "img_home_shadow_icon" :*/
fileprivate let const_themKey:String = "brush decision fleet but interactionimg_h"
fileprivate let userBlueMessage:String = "adow_low stream agency result"

/*: "get json error" :*/
fileprivate let appMenuKey:String = "get jchemical position injury"
fileprivate let data_connectReduceName:String = "reinror"

/*: "#FF2348" :*/
fileprivate let k_twentyTitle:[Character] = ["#","F","F","2","3","4","8"]

/*: "#35C759" :*/
fileprivate let data_genderValue:String = "all little name#35C759"

/*: "mfBean" :*/
fileprivate let dataProcessingMsg:[Character] = ["m","f","B","e","a","n"]

/*: "points\n≈ $" :*/
fileprivate let app_hairTitle:String = "pratingn"
fileprivate let k_motionTelephonePath:[Character] = [" ","$"]
...
// __CLOSE_PRINT__
//
//  OutsideEarthReactiveCompatible.swift
//  AbroadTalking
//
...
public var userPopGameStr: Int {
    //: return MentionTriggerAppManager.share.appConfigMode.countdown1 + MentionTriggerAppManager.share.appConfigMode.countdown2
    return MentionTriggerAppManager.share.appConfigMode.countdown1 + MentionTriggerAppManager.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class OutsideEarthReactiveCompatible: ExtensiveView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= MentionTriggerAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(userPopGameStr - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                freshOperate()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(MentionTriggerAppManager.share.loginUserMode.mf_bean) > 0 {
        if Double(MentionTriggerAppManager.share.loginUserMode.mf_bean) > 0 {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_birthdayTitle.map{denyAccess(favorite: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: EarthThen = {
        ...
        let image = UIImage.capitalTotalry(color: UIColor(hex: (String(show_transformKey.suffix(7))))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.renderAuditory(), for: .normal)
        img.setImage(image.renderAuditory(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(const_punishStr.prefix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.lastTo(name: (String(const_themKey.suffix(5)) + "ome_sh" + String(userBlueMessage.prefix(5)) + "icon"))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(appMenuKey.prefix(5)) + "son " + data_connectReduceName.replacingOccurrences(of: "rein", with: "er")))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(k_twentyTitle)))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(data_genderValue.suffix(7))))
        //: lab.font = UIFont.pingfangFont(type: .Medium, fontSize: 11)
        lab.font = UIFont.landEnableSize(type: .Medium, fontSize: 11)
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.textAlignment = .center
        lab.textAlignment = .center
...
extension OutsideEarthReactiveCompatible {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func ancientHistoryManual() -> OutsideEarthReactiveCompatible {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = OutsideEarthReactiveCompatible(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = DisagreeableWomanSocketManager.shared.liveMiniPosition
    ...
    func freshOperate() {
        ...
        liveStatusImgV.isHidden = !AlwaysDiskPushListener.aInmate().isLive
...
extension OutsideEarthReactiveCompatible {
    ...
    private func ingredient() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(put(notification:)),
                                               //: name: LIVE_USER_POINT_CHANGE,
                                               name: k_plainBeautyPath,
                                               //: object: nil)
                                               object: nil)
    }

    //: @objc private func pointchange(notification: NSNotification) {
    @objc private func put(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(dataProcessingMsg))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        history(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func history(mfBean: Double) {
        //: let nowBean = MentionTriggerAppManager.share.loginUserMode.mf_bean + mfBean
        let nowBean = MentionTriggerAppManager.share.loginUserMode.mf_bean + mfBean
        //: MentionTriggerAppManager.share.loginUserMode.mf_bean = nowBean
        MentionTriggerAppManager.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((app_hairTitle.replacingOccurrences(of: "rating", with: "oi") + "ts\n\u{2248}" + String(k_motionTelephonePath)).localized) \(divisor)"
