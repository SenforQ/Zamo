...
fileprivate let const_hourName:[UInt8] = [0xe2,0xe5,0xe2,0xff,0xa3,0xe8,0xe4,0xef,0xee,0xf9,0xb1,0xa2,0xab,0xe3,0xea,0xf8,0xab,0xe5,0xe4,0xff,0xab,0xe9,0xee,0xee,0xe5,0xab,0xe2,0xe6,0xfb,0xe7,0xee,0xe6,0xee,0xe5,0xff,0xee,0xef]

/*: "no data available" :*/
fileprivate let kProductionValue:String = "whato"
fileprivate let dataOverMsg:String = " datproperty standard detect union invitation"
fileprivate let kTellMerelyTextName:String = "ILABLE"

/*: "list" :*/
fileprivate let dataBrushKey:[Character] = ["l","i","s","t"]
...
class DiskThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_hourName.map{$0^139}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(scriptClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var emptyView: UILabel = {
        ...
        lab.text = (kProductionValue.replacingOccurrences(of: "what", with: "n") + String(dataOverMsg.prefix(4)) + "a ava" + kTellMerelyTextName.lowercased()).localized
        //: lab.textColor = .white
        lab.textColor = .white
...
extension DiskThen {
    ...
    private func cardsAndList() {
        ...
        MeBeThen.gap(category: 2) { [weak self] _, result, _ in
            //: ProgressHUD.dismiss()
            ...
            json[(String(dataBrushKey))].arrayObject?.forEach { dict in
...
extension DiskThen {
    ...
    private func databaseRequest() -> CGFloat {
        ...
        return lineNum * userNearbyMsg + 24.0 + userDiskUrl
    }
}
...
extension DiskThen: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.rapSession(model: dataArr[indexPath.row])
        //: return cell
        return cell
    }
...
extension DiskThen {
    ...
    func reportFrom(from: RiseFromType) {
        //: switch from {
        switch from {
        //: case .LiveRoom:
        case .LiveRoom:
            //: dataArr = TalkingLiveManager.shared().liveRoomModel.gameList
            dataArr = AlwaysDiskPushListener.aInmate().liveRoomModel.gameList
            //: showAnimation()
            sampleIn()
    ...
    private func sampleIn() {
        ...
        self.contentView.frame = CGRect(x: 0, y: constBlockIdentityContent, width: show_objectValue, height: databaseRequest())
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y -= self.contentView.height
            self.contentView.frame.origin.y -= self.contentView.height
...
extension DiskThen {
    ...
    private func anonConstraint() {
        ...
        contentView.frame = CGRect(x: 0, y: constBlockIdentityContent, width: show_objectValue, height: databaseRequest())
