...
fileprivate let data_columnMsg:[UInt8] = [0x8f,0x88,0x8f,0x92,0xce,0x85,0x89,0x82,0x83,0x94,0xdc,0xcf,0xc6,0x8e,0x87,0x95,0xc6,0x88,0x89,0x92,0xc6,0x84,0x83,0x83,0x88,0xc6,0x8f,0x8b,0x96,0x8a,0x83,0x8b,0x83,0x88,0x92,0x83,0x82]

private func onTheJob(span num: UInt8) -> UInt8 {
    return num ^ 230
}

/*: "Online Users" :*/
fileprivate let noti_maxText:String = "exit dit per broad punishOnlin"
...
class OrganVolumeView: UIView {
    //: private let contentV_H = (344+kDeviceSafeBottomHeight)
    private let contentV_H = (344 + userDiskUrl)
    //: private var DataSource: [TalkingLiveRoomOnlineListModel] = []
    private var DataSource: [RestrictionListModel] = []
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_columnMsg.map{onTheJob(span: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(earlyReject), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var contentView: UIView = {
        ...
        v.factorSource(width: show_objectValue,
                       //: height: contentV_H,
                       height: contentV_H,
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(noti_maxText.suffix(5)) + "e Users").localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(SlowThen.self, forCellReuseIdentifier: SlowThen.className())
...
extension OrganVolumeView {
    //: private func reqData() {
    private func sufficientCellMap() {
        //: TalkingAudienceManager().req_liveRoomOnlineList(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
        ManageressThen().whoDoing(chatGroupId: self.chatGroupId) { succeed, result, _ in
            ...
            if succeed {
                ...
                var dataArr: [RestrictionListModel] = []

                //: if let datas = Array<TalkingLiveRoomOnlineListModel>.deserialize(from: array as? Array) {
                if let datas = Array<RestrictionListModel>.deserialize(from: array as? Array) {
                    //: dataArr.append(contentsOf: (datas as? [TalkingLiveRoomOnlineListModel] ?? []))
                    dataArr.append(contentsOf: (datas as? [RestrictionListModel] ?? []))
                }
...
extension OrganVolumeView: UITableViewDelegate, UITableViewDataSource {
...
extension OrganVolumeView {
    ...
    func pauseScene(chatGroupId: String) {
...
extension OrganVolumeView {
    ...
    private func makeupWhen() {
        ...
        let style = StandardEmptyStyle()
        //: emptyView = EmptyView.init(frame: CGRect(x: 0, y: -100, width: ScreenWidth, height: contentV_H+100), style: style)
        emptyView = EmptyView(frame: CGRect(x: 0, y: -100, width: show_objectValue, height: contentV_H + 100), style: style)
