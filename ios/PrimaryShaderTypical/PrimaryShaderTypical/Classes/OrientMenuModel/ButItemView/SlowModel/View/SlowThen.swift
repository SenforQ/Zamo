...
fileprivate let noti_developingStr:[UInt8] = [0x28,0x2f,0x28,0x35,0x69,0x22,0x2e,0x25,0x24,0x33,0x7b,0x68,0x61,0x29,0x20,0x32,0x61,0x2f,0x2e,0x35,0x61,0x23,0x24,0x24,0x2f,0x61,0x28,0x2c,0x31,0x2d,0x24,0x2c,0x24,0x2f,0x35,0x24,0x25]

/*: "#F5F5F5" :*/
fileprivate let showNutKey:String = "laugh tension am#F5F5F5"

/*: "level_ :*/
fileprivate let notiTransactionStr:[Character] = ["l","e","v","e","l"]
fileprivate let data_orientationPath:String = "_"
...
class SlowThen: UITableViewCell {
    //: var currenModel = TalkingLiveRoomOnlineListModel()
    var currenModel = RestrictionListModel()
    //: var index = 0
    var index = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_developingStr.map{$0^65}, encoding: .utf8)!)
    }
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(showNutKey.suffix(7))))
        //: return view
        return view
        //: }()
    }()
...
extension SlowThen {
    ...
    func spaceActive(model: RestrictionListModel, index: Int) {
        //: currenModel = model
        currenModel = model
        //: self.index = index
        self.index = index
        ...
        IconImg.contactMoment(urlStr: model.headPic)
        //: nameLabel.text = model.nickname
        nameLabel.text = model.nickname
        //: levelIcon.image = UIImage.BundleImageNamed(name: "level_\(model.level )")
        levelIcon.image = UIImage.lastTo(name: (String(notiTransactionStr) + data_orientationPath.capitalized) + "\(model.level)")
    }
}
...
extension SlowThen {
    ...
    private func formerWithSubviews() {
