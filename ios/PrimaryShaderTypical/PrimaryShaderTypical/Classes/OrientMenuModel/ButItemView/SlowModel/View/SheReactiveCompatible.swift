...
fileprivate let main_governMsg:[UInt8] = [0xe7,0xe0,0xe7,0xfa,0xa6,0xed,0xe1,0xea,0xeb,0xfc,0xb4,0xa7,0xae,0xe6,0xef,0xfd,0xae,0xe0,0xe1,0xfa,0xae,0xec,0xeb,0xeb,0xe0,0xae,0xe7,0xe3,0xfe,0xe2,0xeb,0xe3,0xeb,0xe0,0xfa,0xeb,0xea]

private func pressEh(eastern num: UInt8) -> UInt8 {
    return num ^ 142
}

/*: "bth_live_follow_pre" :*/
fileprivate let noti_stripContent:String = "lite theme origin tradebth_"
fileprivate let constAnonFormat:String = "turn pin tee dial_follow"
fileprivate let mainWedFormat:String = "_pretiming profit unable"

/*: "btn_live_mini_nor" :*/
fileprivate let data_flowPath:String = "memory play entitle ehbtn_l"
fileprivate let notiChildMessage:[Character] = ["i","v","e","_","m","i"]
fileprivate let k_termsKey:String = "ni_norassert pump popular typical"

/*: "#2DF2FF" :*/
fileprivate let dataSignalTitle:String = "recording tip outer#2DF2FF"

/*: "Live" :*/
fileprivate let k_processStr:String = "unit appearanceLive"

/*: "#FF506D" :*/
fileprivate let user_firstName:String = "#FF506Dprovince about generate general distance"

/*: "removeAttentionUid" :*/
fileprivate let showCompelUrl:[Character] = ["r","e","m","o","v","e","A","t","t","e","n"]
fileprivate let kProductionData:[Character] = ["t","i","o","n","U","i","d"]

/*: "attentionUid" :*/
fileprivate let noti_visibleKey:[Character] = ["a","t","t","e","n","t","i","o"]
fileprivate let kAccommodateValue:String = "nUidold compute"

/*: "source" :*/
fileprivate let notiActionContent:String = "announcement"
fileprivate let k_banFormat:[Character] = ["o","u","r","c","e"]

/*: "num" :*/
fileprivate let user_whiteName:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let show_sessionMsg:String = "UID:union cur automatically"

/*: "btn_live_closed_nor" :*/
fileprivate let user_commonData:[Character] = ["b","t","n","_","l","i","v","e","_","c","l","o","s","e","d","_"]
fileprivate let appRetMsg:[Character] = ["n","o","r"]
...
class SheReactiveCompatible: UIView {
    //: weak var weakVC: TalkingBaseViewController?
    weak var weakVC: EverlastinglyThen?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        //: didSet {
        didSet {
            //: if noFaceTime >= MentionTriggerAppManager.share.appConfigMode.countdown1 {
            if noFaceTime >= MentionTriggerAppManager.share.appConfigMode.countdown1 { // 展示倒计时
                //: self.liveStatusImgV.isHidden = true
                self.liveStatusImgV.isHidden = true
                //: self.countdownView.isHidden = false
                self.countdownView.isHidden = false
                //: self.countdownLab.text = "\(MAX_NOFACETIME-noFaceTime)s"
                self.countdownLab.text = "\(userPopGameStr - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.countdown(countdown: "\(userPopGameStr - noFaceTime)")
                //: } else {
            } else {
    ...
    private var liveModel = SlowModel()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_governMsg.map{pressEh(eastern: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(sortAdjust), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nickNameLab: UILabel = {
        ...
        lab.font = UIFont.wordPicture(fontSize: 11)
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.lastTo(name: (String(noti_stripContent.suffix(4)) + "live" + String(constAnonFormat.suffix(7)) + String(mainWedFormat.prefix(4)))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(charmMakeup), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.lastTo(name: (String(data_flowPath.suffix(5)) + String(notiChildMessage) + String(k_termsKey.prefix(6)))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(chief), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: EarthThen = {
        ...
        let image = UIImage.capitalTotalry(color: UIColor(hex: (String(dataSignalTitle.suffix(7))))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.renderAuditory(), for: .normal)
        img.setImage(image.renderAuditory(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(k_processStr.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var countdownView: UIView = {
        ...
        icon.backgroundColor = UIColor(hex: (String(user_firstName.prefix(7))))
    ...
    private lazy var countdownLab: UILabel = {
        ...
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(MentionTriggerAppManager.share.appConfigMode.countdown2)s"
        lab.text = " \(MentionTriggerAppManager.share.appConfigMode.countdown2)s"
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.pastAccess(color: UIColor.black.withAlphaComponent(0.25), forState: .normal)
        //: btn.addTarget(self, action: #selector(onlineNumBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(belowMove), for: .touchUpInside)
...
extension SheReactiveCompatible {
    ...
    @objc func attention(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(showCompelUrl) + String(kProductionData))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(noti_visibleKey) + String(kAccommodateValue.prefix(4)))] = uid
            //: dict["source"] = "1"
            dict[(notiActionContent.replacingOccurrences(of: "announcement", with: "s") + String(k_banFormat))] = "1"
        }
        //: TalkingChatRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
        GoUpReactiveCompatible.adCompletion(isAttention: !isAttention, params: dict) { succeed, _, _ in
            //: completion?(succeed)
            completion?(succeed)
        }
    }
...
extension SheReactiveCompatible {
    ...
    @objc private func belowMove() {
        ...
        self.onlineListView.pauseScene(chatGroupId: liveModel.chatGroupId)
    }

    /// 获取
    //: @objc private func getOnlineNumData() {
    @objc private func iconPrepare() {
        //: TalkingAudienceManager().req_liveRoomOnlineNum(chatGroupId: liveModel.chatGroupId, completion:{ succeed, result, errorModel in
        ManageressThen().capacityPoolroomCompletion(chatGroupId: liveModel.chatGroupId, completion: { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: let num = json["num"].string
            let num = json[(String(user_whiteName))].string
    ...
    private func place() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(iconPrepare), userInfo: nil, repeats: true)
    }

    //: func stopTimer() {
    func approximateRange() {
...
extension SheReactiveCompatible {
    ...
    func eyebrowPencil(_ liveModel: SlowModel) {
        ...
        uidLab.text = (String(show_sessionMsg.prefix(4))) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.choiceRest(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.delayConvert(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            ...
            miniBtn.setImage(UIImage.lastTo(name: (String(data_flowPath.suffix(5)) + String(notiChildMessage) + String(k_termsKey.prefix(6)))), for: .normal)
            //: refreshFollow(isAttention: true)
            petName(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.BundleImageNamed(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.lastTo(name: (String(user_commonData) + String(appRetMsg))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            petName(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
    ...
    @objc func chief() {
        ...
        if isAnchor { // 改为小窗口
            ...
            AlwaysDiskPushListener.aInmate().pavement(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            transplant()?.navigationController?.popViewController(animated: false)
        }
    }

    //: @objc private func iconBgButtonClick() {
    @objc private func sortAdjust() {
        //: showUserCard(uid: String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid))
        paperBy(uid: String(AlwaysDiskPushListener.aInmate().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func paperBy(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = LifetimeThen(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any PointBorderViewDelegate
        //: userCard.show()
        userCard.multiHiddenPrevious()
    }

    /// 刷新关注按钮
    //: private func refreshFollow(isAttention: Bool) {
    private func petName(isAttention: Bool) {
    ...
    private func occurStatus() {
        //: self.liveStatusImgV.isHidden = !TalkingLiveManager.shared().isLive
        self.liveStatusImgV.isHidden = !AlwaysDiskPushListener.aInmate().isLive
    ...
    @objc private func charmMakeup() {
        //: req_followRequest(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
        attention(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.petName(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension SheReactiveCompatible {
    ...
    private func convertYearConstraint() {
        //: self.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 68))
        self.frame = CGRect(x: 0, y: 0, width: show_objectValue, height: app_pathData + actualWidth(w: 68))
