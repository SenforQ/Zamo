...
fileprivate let k_coreId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "iosSvgaEffect" :*/
fileprivate let showRegionValue:[Character] = ["i","o","s","S","v","g","a","E","f","f","e","c"]
fileprivate let k_ourMessage:[Character] = ["t"]

/*: "iosEffect" :*/
fileprivate let kMinuteId:String = "anima falseiosE"
fileprivate let userIonUrl:[Character] = ["f","f","e","c","t"]

/*: "entityId" :*/
fileprivate let app_scaleValue:String = "entityIflood m"
fileprivate let userTreatStr:[Character] = ["d"]
...
class PinThen: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: MaleToothModelType?
    ...
    var animatedView: ButItemView?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: EncounterThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_coreId.reversed(), encoding: .utf8)!)
    }
...
extension PinThen {
    ...
    func laissezPasser(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(showRegionValue) + String(k_ourMessage))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(kMinuteId.suffix(4)) + String(userIonUrl))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(app_scaleValue.prefix(7)) + String(userTreatStr))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = theme(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func theme(msgModel: Any?) -> Bool {
        ...
        if msgModel is MaleToothModelType {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? MaleToothModelType
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is HomeModelType {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! HomeModelType
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.legacy(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = MaleToothModelType()
            ...
            effectMsgModel = MaleToothModelType()
            ...
            if giftMsgModel.showType == ExtensiveCountLiteral.myStery.rawValue || giftMsgModel.showType == ExtensiveCountLiteral.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: MaleToothModelType = self.effectMsgModelArray[i] as! MaleToothModelType
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? MaleToothModelType())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? MaleToothModelType(), at: index)
                }
            ...
            objectShow(effectMsgModel: effectMsgModel!)
        }
...
extension PinThen {
    ...
    func outShould() {
        ...
        self.animatedView?.dotAnimation()
    }
    ...
    func curRaw() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = response() else { return }
                //: firstShow(effectMsgModel: removeModel)
                objectShow(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.root()
        }
    ...
    func response() -> MaleToothModelType? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return MaleToothModelType()
        }
        ...
        if effectMsgModel is MaleToothModelType {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! MaleToothModelType
        ...
        return effectMsgModel as? MaleToothModelType
    }
}
...
extension PinThen {
    ...
    func objectShow(effectMsgModel: MaleToothModelType) {
        ...
        self.delegate?.cap(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        ScaleThen.shared.amplitudeModulation(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.inheritance(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? BoundAnimatModel())
            //: })
        })
    }
    ...
    func inheritance(effectMsgModel: MaleToothModelType, success: Bool, effectModel: BoundAnimatModel) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.invitationNaturalEventSuccess(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    animationDrown(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func hangUp(effectMsgModel: MaleToothModelType, effectModel: BoundAnimatModel) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.library()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.databaseAnimating()
        ...
        className = ScaleThen.shared.towardSuperphylum(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! ButItemView.Type
...
extension PinThen: TopicReactiveCompatible {
    ...
    func quoteSomeone(effectItemView: ButItemView, success: Bool) {
        ...
        self.delegate?.commentWithinOccurrence(effectView: self, obj: effectItemView.effectMsgModel ?? MaleToothModelType(), success: success)
    }
    ...
    func misconductView(effectItemView: ButItemView) {
        ...
        self.delegate?.limitToOrientation(effectView: self, obj: effectMsgModel!)
    }
}
...
extension PinThen {
    ...
    func destroyMessage() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.library() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.databaseAnimating() // clean animation data
