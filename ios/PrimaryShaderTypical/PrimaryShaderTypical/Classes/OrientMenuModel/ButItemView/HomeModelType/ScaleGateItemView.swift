...
fileprivate let showAttentionMsg:[UInt8] = [0x31,0x36,0x31,0x2c,0x70,0x3b,0x37,0x3c,0x3d,0x2a,0x62,0x71,0x78,0x30,0x39,0x2b,0x78,0x36,0x37,0x2c,0x78,0x3a,0x3d,0x3d,0x36,0x78,0x31,0x35,0x28,0x34,0x3d,0x35,0x3d,0x36,0x2c,0x3d,0x3c]

private func strongComplaint(apply num: UInt8) -> UInt8 {
    return num ^ 88
}

/*: "%@.mp4" :*/
fileprivate let data_transitionId:String = "%@.mp4wind enthusiasm attach"

/*: "http" :*/
fileprivate let k_activityMsg:String = "httsea"
...
class ScaleGateItemView: ButItemView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showAttentionMsg.map{strongComplaint(apply: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(inviteDownTap))
    ...
    override func identity() {
        ...
        let path = ScaleThen.shared.fileWithDivinatoryLifeStory(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? MaleToothModelType())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(data_transitionId.prefix(6))), path)
    ...
    @objc func inviteDownTap() {}
}

///  --- HWDMP4PlayDelegate

//: extension TalkingVapAnimationView: HWDMP4PlayDelegate {
extension ScaleGateItemView: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((k_activityMsg.replacingOccurrences(of: "sea", with: "p"))) {
