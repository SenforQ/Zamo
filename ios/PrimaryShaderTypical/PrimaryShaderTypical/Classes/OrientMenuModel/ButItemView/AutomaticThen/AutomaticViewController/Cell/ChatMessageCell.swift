...
fileprivate let show_actionLetterCopyPath:[UInt8] = [0x54,0x53,0x54,0x49,0x15,0x5e,0x52,0x59,0x58,0x4f,0x7,0x14,0x1d,0x55,0x5c,0x4e,0x1d,0x53,0x52,0x49,0x1d,0x5f,0x58,0x58,0x53,0x1d,0x54,0x50,0x4d,0x51,0x58,0x50,0x58,0x53,0x49,0x58,0x59]

private func depthEnablece(yellow num: UInt8) -> UInt8 {
    return num ^ 61
}

/*: "icon_chat_translate_nor" :*/
fileprivate let user_upperPath:String = "icopath"
fileprivate let kStraightMsg:String = "TRANSLA"
fileprivate let appStatutePath:[Character] = ["t","e","_","n","o","r"]

/*: "icon_chat_translate_pre" :*/
fileprivate let app_cooperativePath:String = "ICON"
fileprivate let app_analyzeWhenValue:String = "till province_chat"
fileprivate let kOpKey:String = "lacornere"

/*: "icon_chat_miss" :*/
fileprivate let dataListenContent:String = "icoyoung"
fileprivate let kConsumerName:String = "forwardss"

/*: "icon_chat_yes" :*/
fileprivate let app_alongsideValue:[Character] = ["i","c"]
fileprivate let userPicFailConstraintPath:String = "additional file nut enteron_cha"
...
class ChatMessageCell: SimplyView {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: SimplyReactiveCompatible?
    var textData: SimplyReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_actionLetterCopyPath.map{depthEnablece(yellow: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transLB: YYLabel = {
        ...
        label.font = UIFont.activeBeautyNative(fontSize: 16)
        //: return label
        return label
        //: }()
    }()
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.lastTo(name: (user_upperPath.replacingOccurrences(of: "path", with: "n") + "_chat_" + kStraightMsg.lowercased() + String(appStatutePath))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.lastTo(name: (app_cooperativePath.lowercased() + String(app_analyzeWhenValue.suffix(5)) + "_trans" + kOpKey.replacingOccurrences(of: "corner", with: "t") + "_pre")), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(cornerBackground), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension ChatMessageCell {
    ...
    override func disembarrass(with data: BorderReactiveCompatible) {
        ...
        //: textData = data as? SimplyReactiveCompatible
        textData = data as? SimplyReactiveCompatible
        ...
        if imgPreview.count > 0, textData.msgModel.msgInfo.messageType == .MessageTypeWelcome {
            ...
            giftImagV.contactMoment(urlStr: imgPreview)
        }
        ...
        //: let maxWidth = EverlastinglyReactiveCompatible.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = EverlastinglyReactiveCompatible.laden() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.lastTo(name: (dataListenContent.replacingOccurrences(of: "young", with: "n") + "_chat_" + kConsumerName.replacingOccurrences(of: "forward", with: "mi")))
            ...
            self.transStatusImagV.image = UIImage.lastTo(name: (String(app_alongsideValue) + String(userPicFailConstraintPath.suffix(6)) + "t_yes"))
        }
    ...
    func perennial() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(week(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func week(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
