...
/*: "#ffffff" :*/
fileprivate let kHeavyData:[Character] = ["#","f","f","f","f","f","f"]

/*: "earned" :*/
fileprivate let kPresentValue:String = "communicate"
fileprivate let show_describeProcessMissFormat:[Character] = ["a","r","n","e","d"]

/*: "Coins in" :*/
fileprivate let main_columnFormat:String = "Coinmagnitude trail"

/*: "Error:  :*/
fileprivate let showCostAnnouncementData:String = "Error: family volume able category earth"

/*: "#C946D2" :*/
fileprivate let mainStateId:String = "main cycle lock attach healthy#C946D2"

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let noti_wordValue:[Character] = ["b","t","n","_","c","h","a","t","_","f","l","o","a"]
fileprivate let showFunThatActivityStr:String = "first mutualtS_user"
fileprivate let noti_referKey:String = "put enough sureBK_nor"
...
class UnveilingView: UIView {
    ...
    private let TextColor = (String(kHeavyData))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: FlowMysteryMeasurable?) {
        //: self.init()
        self.init()
    ...
    private func configMaleMeanSun() {
        ...
        if danumuMsgModel.headPic.count > 0 {
            ...
            headAnimatedImageView.contactMoment(urlStr: danumuMsgModel.headPic)
        }
        //: if danumuMsgModel.bgPic.count > 0 {
        if danumuMsgModel.bgPic.count > 0 {
            //: updateBackground(withUrl: danumuMsgModel.bgPic)
            blind(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (kPresentValue.replacingOccurrences(of: "communicate", with: "e") + String(show_describeProcessMissFormat))) {
                ...
                if let range = attrStr.string.range(of: (String(main_columnFormat.prefix(4)) + "s in")) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.landEnableSize(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(showCostAnnouncementData.prefix(7))) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func blind(withUrl url: String) {
        //: self.backgroundImageView.setUrlImage(urlStr: url) {[weak self] result, image in
        self.backgroundImageView.choiceRest(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.playerExist(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: FlowMysteryMeasurable? {
        //: didSet {
        didSet {
            //: configModel()
            configMaleMeanSun()
    ...
    var totalTime: CGFloat {
        ...
        return (show_objectValue + widthValue) * SpendTime / show_objectValue
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / show_objectValue
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / show_objectValue
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(mainStateId.suffix(7))))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.lastTo(name: (String(noti_wordValue) + String(showFunThatActivityStr.suffix(7)) + String(noti_referKey.suffix(6))))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(tinkleMedium(_:)))
        //: return t
        return t
        //: }()
    }()
    ...
    lazy var contentLabel: UILabel = {
        ...
        l.font = UIFont.wordPicture(fontSize: 12)
        //: l.textColor = .white
        l.textColor = .white
        //: l.numberOfLines = 1
        l.numberOfLines = 1
...
extension UnveilingView {
    /// 点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func tinkleMedium(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = MentionTriggerAppManager.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = MentionTriggerAppManager.share.appConfigMode.urlH5Domain + gameUrl
        ...
        RiseMalePushManager.share.errConfig(urlStr: httpsUrl, webConfig: config)
    }
}
