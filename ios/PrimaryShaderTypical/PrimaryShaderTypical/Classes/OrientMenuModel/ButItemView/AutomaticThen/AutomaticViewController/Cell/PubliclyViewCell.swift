...
fileprivate let constContainerStr:[UInt8] = [0xf8,0xff,0xf8,0xe5,0xb9,0xf2,0xfe,0xf5,0xf4,0xe3,0xab,0xb8,0xb1,0xf9,0xf0,0xe2,0xb1,0xff,0xfe,0xe5,0xb1,0xf3,0xf4,0xf4,0xff,0xb1,0xf8,0xfc,0xe1,0xfd,0xf4,0xfc,0xf4,0xff,0xe5,0xf4,0xf5]

private func helloReason(someone num: UInt8) -> UInt8 {
    return num ^ 145
}

/*: "icon_chat_lock" :*/
fileprivate let userFoodKey:String = "isortn"
fileprivate let show_travelerTitle:String = "rest material task_lock"

/*: "icon_chat_burned" :*/
fileprivate let app_operateUrl:String = "translation siblingicon_"
fileprivate let showListTitle:String = "URNED"

/*: "icon_chat_burned_shan" :*/
fileprivate let appLatMountId:String = "sign mask direct valuationicon_c"
fileprivate let mainCameraPiMessage:String = "bursituation"
fileprivate let noti_exclusiveTitle:String = "ed_shantransition still"

/*: "Already burned" :*/
fileprivate let kDrawingName:[Character] = ["A","l","r","e","a","d","y"," ","b","u","r"]
fileprivate let dataAssetMsg:[Character] = ["n","e","d"]
...
class PubliclyViewCell: SimplyView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constContainerStr.map{helloReason(someone: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.lastTo(name: (userFoodKey.replacingOccurrences(of: "sort", with: "co") + "_chat" + String(show_travelerTitle.suffix(5))))
    ...
    lazy var burnedImagV: EarthThen = {
        ...
        imgV.setBackgroundImage(UIImage.lastTo(name: (String(app_operateUrl.suffix(5)) + "chat_b" + showListTitle.lowercased())), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.lastTo(name: (String(appLatMountId.suffix(6)) + "hat_" + mainCameraPiMessage.replacingOccurrences(of: "situation", with: "n") + String(noti_exclusiveTitle.prefix(7)))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(kDrawingName) + String(dataAssetMsg)).localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.activeBeautyNative(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension PubliclyViewCell {
    ...
    override func disembarrass(with data: BorderReactiveCompatible) {
        ...
        let intimatePhoto: ManyModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            boardAccess(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            boardAccess(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: MezuzahCellData, urlStr: String?, isMosaic: Bool) {
    func boardAccess(data: MezuzahCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.needAge(image, barMagnitudeerval: 90)
                        self.photoView.image = UIImage.needAge(image, barMagnitudeerval: 90)
                        //: } else {
                    } else {
            ...
            issue(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.contactMoment(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func issue(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.valuation(urlStr: urlStr) { [weak self] image in
