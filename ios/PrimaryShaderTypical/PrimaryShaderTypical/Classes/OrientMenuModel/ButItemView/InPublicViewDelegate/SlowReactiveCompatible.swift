...
/*: "call_bgm" :*/
fileprivate let showAutomaticId:String = "graduate custom good cameracall_bg"
fileprivate let data_actorMsg:[Character] = ["m"]

/*: "call_response_bgm" :*/
fileprivate let kCurrentProductionStr:[Character] = ["c","a","l","l","_","r","e","s","p","o","n","s","e","_","b","g"]
fileprivate let notiUpperData:String = "arrow"

/*: "invitePop_call_bgm" :*/
fileprivate let dataHolderFollowingName:[Character] = ["i","n","v","i","t","e","P","o"]
fileprivate let userRemindListenUrl:String = "p_cscreen pose"
fileprivate let notiFoundFormatValue:String = "all_bgmstatement weak mystery"
...
enum WouldColumnConvertible: Int {
    //: case Call
    case Call // 音视频拨打音乐
    //: case Response
...
class SlowReactiveCompatible: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = WouldColumnConvertible.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func connectionHistory(type: WouldColumnConvertible) -> SlowReactiveCompatible {
        //: let player = TalkingCallBGMPlayer()
        let player = SlowReactiveCompatible()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.doingCapacity()
        //: return player
        return player
    }
...
extension SlowReactiveCompatible {
    //: func play() {
    func asDiscount() {
    ...
    private func airship(notify: Notification) {
    ...
    private func outRoute(notify: Notification) {
    ...
    private func doingCapacity() {
        //: var fileName = "call_bgm"
        var fileName = (String(showAutomaticId.suffix(7)) + String(data_actorMsg))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (String(kCurrentProductionStr) + notiUpperData.replacingOccurrences(of: "arrow", with: "m"))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (String(dataHolderFollowingName) + String(userRemindListenUrl.prefix(3)) + String(notiFoundFormatValue.prefix(7)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = AliveEffectTool.default.panelBirthday(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension SlowReactiveCompatible: TXVodPlayListener {
