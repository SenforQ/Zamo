...
fileprivate let appApplyName:[UInt8] = [0xa9,0xae,0xa9,0xb4,0xe8,0xa3,0xaf,0xa4,0xa5,0xb2,0xfa,0xe9,0xe0,0xa8,0xa1,0xb3,0xe0,0xae,0xaf,0xb4,0xe0,0xa2,0xa5,0xa5,0xae,0xe0,0xa9,0xad,0xb0,0xac,0xa5,0xad,0xa5,0xae,0xb4,0xa5,0xa4]

/*: "#FFD13A" :*/
fileprivate let mainBeneathAnonymousTitle:String = "bot join contact access#FFD13A"

/*: "gift_gb_umnad_1" :*/
fileprivate let appAdjustKey:[Character] = ["g","i","f","t","_","g","b"]
fileprivate let notiSmartKey:String = "so above toward comprehensive_umnad_1"

/*: "icon_circle_pic_default" :*/
fileprivate let notiTimingMsg:String = "pose vote multi cur impressionicon_cir"
fileprivate let main_haveTitle:[Character] = ["c","l","e","_","p","i","c","_","d","e","f","a","u","l","t"]

/*: "Open lucky blind-box" :*/
fileprivate let dataCentralPath:String = "title moonOpen"
fileprivate let showFamilyPath:String = "vid visualky b"
fileprivate let constMagnitudeIllegalData:String = "-boxvisitor tap administrator share"

/*: "Sent " :*/
fileprivate let show_actorKey:[Character] = ["S","e","n","t"," "]

/*: "PingFangSC-Semibold" :*/
fileprivate let app_firstThanksName:String = "tap aidPing"
fileprivate let constPrivacyEarlyMessage:String = "front incident night design transactionSC-Semib"
fileprivate let show_lowerValue:String = "ocooperatived"

/*: "Bombard" :*/
fileprivate let userEntityName:[Character] = ["B","o","m","b","a","r","d"]
...
class SectionThen: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = HomeModelType()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [LifetimeArrModel()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appApplyName.map{$0^192}, encoding: .utf8)!)
    }
    ...
    private lazy var nickLb: UILabel = {
        ...
        lb.font = .activeBeautyNative(fontSize: 14)
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(mainBeneathAnonymousTitle.suffix(7))))
        //: lb.font = UIFont.pingfangRugularFont(fontSize: 13)
        lb.font = UIFont.activeBeautyNative(fontSize: 13)
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var pluralTip: InPublicReactiveCompatible = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = InPublicReactiveCompatible()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: InPublicReactiveCompatible = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = InPublicReactiveCompatible()
...
extension SectionThen {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func insertMake(model: HomeModelType) {
        ...
        self.bgImage.image = UIImage.lastTo(name: (String(appAdjustKey) + String(notiSmartKey.suffix(8))))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.lastTo(name: (String(notiTimingMsg.suffix(8)) + String(main_haveTitle)))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == ExtensiveCountLiteral.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(dataCentralPath.suffix(4)) + " luc" + String(showFamilyPath.suffix(4)) + "lind" + String(constMagnitudeIllegalData.prefix(4))).localized
            //: }else {
        } else {
        ...
        self.avatarImage.choiceRest(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.choiceRest(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(show_actorKey)).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(app_firstThanksName.suffix(4)) + "Fang" + String(constPrivacyEarlyMessage.suffix(8)) + show_lowerValue.replacingOccurrences(of: "cooperative", with: "l")), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = lateSplit(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(app_firstThanksName.suffix(4)) + "Fang" + String(constPrivacyEarlyMessage.suffix(8)) + show_lowerValue.replacingOccurrences(of: "cooperative", with: "l")), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = lateSplit(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.draftRid(texts: strs, colors: colors, fonts: fonts)
        ...
        eyeTotal(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func eyeTotal(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(app_firstThanksName.suffix(4)) + "Fang" + String(constPrivacyEarlyMessage.suffix(8)) + show_lowerValue.replacingOccurrences(of: "cooperative", with: "l")), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = lateSplit(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(app_firstThanksName.suffix(4)) + "Fang" + String(constPrivacyEarlyMessage.suffix(8)) + show_lowerValue.replacingOccurrences(of: "cooperative", with: "l")), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = lateSplit(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.judgeAdvertising(150, 77, 20, 1), UIColor.white, UIColor.judgeAdvertising(150, 77, 20, 1)]
        ...
        attText = attText.draftRid(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.glycerolize(attributedText: attText)
        ...
        countLb.infoOpenMy(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func fullModel(giftMsgModel: HomeModelType) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(priority), object: nil)
        ...
        self.perform(#selector(priority), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func showScope(model _: HomeModelType, animate _: Bool) {
...
extension SectionThen {
    ...
    func lateSplit(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension SectionThen {
    ...
    private func runSubviews() {
        ...
        font = lateSplit(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.draftRid(texts: [(String(userEntityName)).localized], colors: [UIColor.judgeAdvertising(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.infoOpenMy(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.glycerolize(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.infoOpenMy(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
