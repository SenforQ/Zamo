...
fileprivate let showPhoneUrl:[UInt8] = [0xef,0xe8,0xef,0xf2,0xae,0xe5,0xe9,0xe2,0xe3,0xf4,0xbc,0xaf,0xa6,0xee,0xe7,0xf5,0xa6,0xe8,0xe9,0xf2,0xa6,0xe4,0xe3,0xe3,0xe8,0xa6,0xef,0xeb,0xf6,0xea,0xe3,0xeb,0xe3,0xe8,0xf2,0xe3,0xe2]

private func commonFollowing(bag num: UInt8) -> UInt8 {
    return num ^ 134
}
...
// __CLOSE_PRINT__
//
//  EarthGuideReactiveCompatible.swift
//  AbroadTalking
//
...
enum TelevisionMagnitudeLiteral: Int {
    //: case TimeGap = 0
...
@objc protocol LovelyObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func am()

    //: @objc optional func endScroll()
    @objc optional func straightaway()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class EarthGuideReactiveCompatible: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(upgrade))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showPhoneUrl.map{commonFollowing(bag: $0)}, encoding: .utf8)!)
    }
...
extension EarthGuideReactiveCompatible {
    //: func startTimer() {
    func lateBoard() {
        //: stopTimer()
        outTitle()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(makeUpQuote), userInfo: nil, repeats: true)
    }
...
extension EarthGuideReactiveCompatible {
    ...
    func addRelationChin(msgModel: TrapModelType?) {
    ...
    func reverting() {
        ...
        if AllHeight > noti_successCollectionValue {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < noti_successCollectionValue {
                ...
                self.msgTableView.height = noti_successCollectionValue
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = noti_successCollectionValue - AllHeight
        ...
        liveAnimated(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func liveAnimated(animated: Bool) {
    ...
    func forePending(inPending: Bool) {
...
extension EarthGuideReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(main_succeedRequestCornerFormat)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ExtensiveReactiveCompatible.starTerms(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension EarthGuideReactiveCompatible {
    ...
    func tabSubviews() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: dataDetailFormat, height: noti_successCollectionValue)
    }
}
