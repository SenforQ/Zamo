...
/*: "uid" :*/
fileprivate let appDarkStr:String = "uideveloping"

/*: "onlineStatus" :*/
fileprivate let app_profileFormat:String = "oangleliangle"

/*: "isNewUser" :*/
fileprivate let show_mistakeMStr:[Character] = ["i","s","N","e","w","U"]
fileprivate let user_keepId:[Character] = ["s","e","r"]

/*: "isTPAuth" :*/
fileprivate let data_becomeKey:String = "income spec violation medal accuracyisTPAu"
fileprivate let kEnjoyQualifyStr:[Character] = ["t","h"]

/*: "totalIntimate" :*/
fileprivate let notiMultiUrl:[Character] = ["t","o","t","a","l"]
fileprivate let dataConstraintId:[Character] = ["I","n"]
fileprivate let kPlusPointKey:String = "TIMATE"

/*: "userStatus" :*/
fileprivate let kCompelKey:String = "userSupon schedule middle"
fileprivate let noti_curtainLaterKey:String = "icontus"

/*: "intimate" :*/
fileprivate let app_materialUrl:String = "globaltimate"
...
// __CLOSE_PRINT__
//
//  ChatSumegrityListManager.swift
//  AbroadTalking
//
...
enum PointPropertyProtocol: Int {
    //: case top  = 0
    case top = 0 // 置顶
    //: case normal = 1
    case normal = 1 // 默认
...
class ChatSumegrityListManager {
    //: var isHalf = false
    ...
    private var moreMsgModel: AutomaticThen = //: return AutomaticThen.getPlaceholderModel(chatType: .moreMsg)
        .garnerHeadTypeRecord(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func metadataApply() {
        //: self.officialArr = AssistantV2Listener.shared.getOfficialList()
        self.officialArr = AssistantV2Listener.shared.voiceOverSuspend()
        //: self.topArr = AssistantV2Listener.shared.topConvList
        self.topArr = AssistantV2Listener.shared.topConvList
        //: self.normalArr = AssistantV2Listener.shared.norConvList
        self.normalArr = AssistantV2Listener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        vigour()
    ...
    func unmixed(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: AssistantV2Listener.shared.getChatRoomData { succeed in
        AssistantV2Listener.shared.earlier { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = AssistantV2Listener.shared.chatRoomList
                self.chatRoomArr = AssistantV2Listener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func seeBecome(indexPath: IndexPath) -> AutomaticThen? {
    ...
    func capitalTo(model: AutomaticThen?) -> AutomaticThen? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: PronkCacheModel? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: PronkCacheModel? = BoundPackInfoManager.terms(targetId: targetId)
    ...
    func afterExitFinish(conversationList: [AutomaticThen], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        BoundPackInfoManager.faceCompletion(with: tempArr) { _, error in
    ...
    func nextVisible(conversationModel: AutomaticThen) {
    ...
    func recoup(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: AssistantV2Listener.shared.func__getConversationList { arr, leftMore in
        AssistantV2Listener.shared.scheduleCompletion { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [AutomaticThen] = //: return Array<AutomaticThen>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [AutomaticThen] = //: return Array<AutomaticThen>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [AutomaticThen] = //: return Array<AutomaticThen>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [AutomaticThen] = //: return Array<AutomaticThen>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [AutomaticThen] = //: return Array<AutomaticThen>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [AutomaticThen] = //: return Array<AutomaticThen>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [AutomaticThen] = //: return Array<AutomaticThen>()
        .init()
    //: }()
}
...
extension ChatSumegrityListManager {
    /// 检查消息列表中是否已经建立过会话
...
extension ChatSumegrityListManager {
    ...
    func vigour() {
        //: self.moreMsgArr = AssistantV2Listener.shared.moreMsgList
        self.moreMsgArr = AssistantV2Listener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func schmooze(indexPath: IndexPath) -> AutomaticThen? {
    ...
    func outerSpace() -> AutomaticThen {
        ...
        moreMsgModel.unreadCount = plusOnTotal()
...
extension ChatSumegrityListManager {
    ...
    func enableLine(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        BoundPackInfoManager.outlet(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(appDarkStr.replacingOccurrences(of: "developing", with: "d"))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(app_profileFormat.replacingOccurrences(of: "angle", with: "n") + "eStatus")].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(show_mistakeMStr) + String(user_keepId))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(data_becomeKey.suffix(6)) + String(kEnjoyQualifyStr))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(notiMultiUrl) + String(dataConstraintId) + kPlusPointKey.lowercased())].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(kCompelKey.prefix(5)) + noti_curtainLaterKey.replacingOccurrences(of: "icon", with: "ta"))].int ?? 1
                    ...
                    self.routeGroup(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func statusBy(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(appDarkStr.replacingOccurrences(of: "developing", with: "d"))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(app_profileFormat.replacingOccurrences(of: "angle", with: "n") + "eStatus")] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(show_mistakeMStr) + String(user_keepId))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(kCompelKey.prefix(5)) + noti_curtainLaterKey.replacingOccurrences(of: "icon", with: "ta"))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(kCompelKey.prefix(5)) + noti_curtainLaterKey.replacingOccurrences(of: "icon", with: "ta"))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension ChatSumegrityListManager {
    ...
    func enoughData() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in AssistantV2Listener.shared.norConvList {
        for model in AssistantV2Listener.shared.norConvList {
            //: if model.totalIntimate >= MentionTriggerAppManager.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= MentionTriggerAppManager.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.capitalTo(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in AssistantV2Listener.shared.topConvList {
        for model in AssistantV2Listener.shared.topConvList {
            //: if model.totalIntimate >= MentionTriggerAppManager.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= MentionTriggerAppManager.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func termsTarget(convModel: AutomaticThen, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func graduatedTablePath(indexPath: IndexPath) -> AutomaticThen? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func routeGroup(uid: String, intimate: Int) {
        ...
        BoundPackInfoManager.cache(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func age(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(appDarkStr.replacingOccurrences(of: "developing", with: "d"))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(app_materialUrl.replacingOccurrences(of: "global", with: "in"))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
