...
fileprivate let notiFrameStr:[UInt8] = [0xd0,0xd7,0xd0,0xcd,0x91,0xda,0xd6,0xdd,0xdc,0xcb,0x83,0x90,0x99,0xd1,0xd8,0xca,0x99,0xd7,0xd6,0xcd,0x99,0xdb,0xdc,0xdc,0xd7,0x99,0xd0,0xd4,0xc9,0xd5,0xdc,0xd4,0xdc,0xd7,0xcd,0xdc,0xdd]

/*: "SHBDropMenuCell" :*/
fileprivate let mainModestUrl:String = "source return already enough goodSHBDro"
fileprivate let show_globalValue:String = "pMenuexposure birth maximum"

/*: "title" :*/
fileprivate let noti_triggerMessage:String = "liftiliftle"
...
// __CLOSE_PRINT__
//
//  AccessibleReactiveCompatible.swift
//  DropMenu
//
...
public class AccessibleReactiveCompatible: UIView {
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.ExtensiveConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.VolumeBe.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.VolumeBe.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.VolumeBe.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? ScaleMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
    ...
    public class func pullBack(anchorView: UIView, titleArray: [String], imageArray: [UIImage]) -> AccessibleReactiveCompatible {
        ...
        let menuView = purchase(anchorView: anchorView, menuArray: menuArray)
        //: return menuView
        return menuView
    }

    //: public class func pullDropDrownMenu(anchorView: UIView, menuArray: [DropMenuModel]) -> DropDownMenuView {
    public class func purchase(anchorView: UIView, menuArray: [OrientMenuModel]) -> AccessibleReactiveCompatible {
        ...
        let menuView = AccessibleReactiveCompatible()
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiFrameStr.map{$0^185}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func straddleView() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func asFrame() -> UITableView {
    ...
    private func narrowProperty() {
        //: addSubview(contentView)
        addSubview(contentView)
        //: mTable.register(DropMenuCell.self, forCellReuseIdentifier: "SHBDropMenuCell")
        mTable.register(DirectViewCell.self, forCellReuseIdentifier: (String(mainModestUrl.suffix(6)) + String(show_globalValue.prefix(5)) + "Cell"))
    ...
    private func agentExpected() -> CGFloat {
...
extension AccessibleReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(mainModestUrl.suffix(6)) + String(show_globalValue.prefix(5)) + "Cell"), for: indexPath) as! DirectViewCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (noti_triggerMessage.replacingOccurrences(of: "lift", with: "t")))
        //: animateRemoveView()
        animateView()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.app(index: indexPath.row, title: cellModel.title ?? "")
    }
}
