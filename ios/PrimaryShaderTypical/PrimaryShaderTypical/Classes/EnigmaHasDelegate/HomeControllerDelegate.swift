...
class HomeControllerDelegate: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: RocketItemType?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.iconAudienceSatisfy()
            }
        }
    ...
    func setupFollowing() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.doMinimize(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.appColor()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.roundChromaticColor(), NSAttributedString.Key.font: UIFont.landEnableSize(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.doMinimize(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.roundChromaticColor(), NSAttributedString.Key.font: UIFont.landEnableSize(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.roundChromaticColor()
...
extension HomeControllerDelegate: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            ToothReactiveCompatible.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
...
extension HomeControllerDelegate: UIGestureRecognizerDelegate {
