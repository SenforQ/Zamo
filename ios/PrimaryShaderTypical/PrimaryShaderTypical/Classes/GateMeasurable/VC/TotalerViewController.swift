...
fileprivate let show_tapMsg:[UInt8] = [0x37,0x30,0x37,0x2a,0x76,0x3d,0x31,0x3a,0x3b,0x2c,0x64,0x77,0x7e,0x36,0x3f,0x2d,0x7e,0x30,0x31,0x2a,0x7e,0x3c,0x3b,0x3b,0x30,0x7e,0x37,0x33,0x2e,0x32,0x3b,0x33,0x3b,0x30,0x2a,0x3b,0x3a]

private func wedAssist(organ num: UInt8) -> UInt8 {
    return num ^ 94
}

/*: "Post" :*/
fileprivate let userGoStr:String = "import crush currencyPost"

/*: "Profile" :*/
fileprivate let noti_easeData:String = "home depth videoPr"
fileprivate let user_removeTitle:String = "oconductle"

/*: "bth_me_follow_nor" :*/
fileprivate let notiGalleryMsg:String = "finite reject betterbth_m"
fileprivate let notiPickPath:String = "e_folbear job lead"

/*: "removeAttentionUid" :*/
fileprivate let notiResolutionPath:String = "accountingemo"
fileprivate let main_giveText:String = "tionUidopposite me acquire search"

/*: "attentionUid" :*/
fileprivate let data_disagreeUrl:String = "acompletecompleteen"
fileprivate let app_streetText:String = "exclusive with ustionUid"

/*: "bth_me_follow_pre" :*/
fileprivate let show_argumentPath:[UInt8] = [0x65,0x72,0x70,0x5f,0x77,0x6f,0x6c,0x6c,0x6f,0x66,0x5f,0x65,0x6d,0x5f,0x68,0x74,0x62]
...
class TotalegrityTrimReactiveCompatible: NSObject {
...
class TotalerViewController: EverlastinglyThen {
    //: var uid: String = ""
    var uid: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_tapMsg.map{wedAssist(organ: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var navView: RocketView = { // 导航
    ...
    private lazy var headerView: ConductView = { // banner
    ...
    private lazy var bottomView: FlowView = { // bottom
        //: let v = TalkingUserDetailBottomView(frame: .zero)
        let v = FlowView(frame: .zero)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var segmentedView: JXSegmentedView = {
        ...
        let segmentedView = JXSegmentedView(frame: CGRect(x: 0, y: app_pathData, width: show_objectValue, height: 44))
        ...
        segmentedDataSource.titleNormalColor = UIColor.modusOperandi()
        //: segmentedDataSource.titleSelectedColor = UIColor.appTitleColor()
        segmentedDataSource.titleSelectedColor = UIColor.roundChromaticColor()
        ...
        indicator.indicatorColor = UIColor.roundChromaticColor()
    ...
    private lazy var pagerView: JXPagingListRefreshView = {
        ...
        view.pinSectionHeaderVerticalOffset = Int(appMarginFormat)
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var titleArray: [String] = //: return ["Post".localized, "Profile".localized]
        [(String(userGoStr.suffix(4))).localized, (String(noti_easeData.suffix(2)) + user_removeTitle.replacingOccurrences(of: "conduct", with: "fi")).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [EverlastinglyThen] = //: return [postVC, profileVC]
        [postVC, profileVC]
    //: }()

    //: private lazy var postVC: TalkingUserDetailPostViewController = {
    private lazy var postVC: VolumeAidViewDelegate = {
        //: let vc = TalkingUserDetailPostViewController()
        let vc = VolumeAidViewDelegate()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var profileVC: BoundViewDelegate = {
        //: let vc = TalkingUserDetailProfileViewController()
        let vc = BoundViewDelegate()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var followBtn: UIImageView = {
        ...
        img.image = UIImage.lastTo(name: (String(notiGalleryMsg.suffix(5)) + String(notiPickPath.prefix(5)) + "low_nor"))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(followButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(fee))
...
extension TotalerViewController {
    /// follow
    //: private func req_followRequest() {
    private func appealRequest() {
        ...
        if userInfoModel.isAttention {
            ...
            dict[(notiResolutionPath.replacingOccurrences(of: "accounting", with: "r") + "veAtten" + String(main_giveText.prefix(7)))] = userInfoModel.uid
            //: isattion = false
            isattion = false
            //: } else {
        } else {
            //: dict["attentionUid"] = userInfoModel.uid
            dict[(data_disagreeUrl.replacingOccurrences(of: "complete", with: "t") + String(app_streetText.suffix(7)))] = userInfoModel.uid
        }
    ...
    func accountingData() {
        ...
        queue.async(group: group) {
            ...
            BoundRequestManager.down(uid: self.uid) { succeed, result, _ in
                //: self.setNoremarlView()
                self.note()
                //: if self.uid == MentionTriggerAppManager.share.loginUid {
                if self.uid == MentionTriggerAppManager.share.loginUid {
                    //: self.bottomView.showEditBtn()
                    self.bottomView.remain()
                }
                //: if succeed == true {
                if succeed == true {
                    ...
                    if let userModel = JSONDeserializer<TipRugInfoModel>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        ...
                        (PhallicApplication.shared as! PhallicApplication).closeMe(uid: userModel.uid, videoUrl: userModel.videoUrl, headPic: userModel.headPic)
                    }
                }
                //: group.leave()
                group.leave()
        ...
        queue.async(group: group) {
            ...
            BoundRequestManager.object(uid: self.uid) { succeed, result, _ in
                //: if succeed == true {
                if succeed == true {
                    ...
                    for dic in result as! [[String: Any]] {
                        ...
                        if let model = ToothPeerMeasurable.deserialize(from: dic as? Dictionary) {
    ...
    private func requestRow() {
        ...
        fullPhaseOfTheMoonStatus()

        // profile分区数据
        //: var profileSectionArr = [TUserDetailCellType.profileType]
        var profileSectionArr = [MarketCaseIterable.profileType]
        ...
        self.profileVC.drown(giftsArr, profileSectionArr, userInfoModel)
    }
}
...
extension TotalerViewController {
    ...
    private func fullPhaseOfTheMoonStatus() {
        //: self.followBtn.isHidden = (userInfoModel.uid == MentionTriggerAppManager.share.loginUid)
        self.followBtn.isHidden = (userInfoModel.uid == MentionTriggerAppManager.share.loginUid)
        //: let followImgStr = self.userInfoModel.isAttention ? "bth_me_follow_pre":"bth_me_follow_nor"
        let followImgStr = self.userInfoModel.isAttention ? String(bytes: show_argumentPath.reversed(), encoding: .utf8)! : (String(notiGalleryMsg.suffix(5)) + String(notiPickPath.prefix(5)) + "low_nor")
        //: self.followBtn.image = UIImage.BundleImageNamed(name: followImgStr)
        self.followBtn.image = UIImage.lastTo(name: followImgStr)
    }
    ...
    private func dewlap() {
        ...
        guard userInfoModel.loungePlus else { return }
        //: guard let vipModel = MentionTriggerAppManager.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first else { return }
        guard let vipModel = MentionTriggerAppManager.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first else { return }
        ...
        vipCenterView.contactMoment(urlStr: vipModel.vipSkinCenter, placeImg: nil)
        //: vipBottomView.setUrlImage(urlStr: vipModel.vipSkinBottom, placeImg: nil)
        vipBottomView.contactMoment(urlStr: vipModel.vipSkinBottom, placeImg: nil)
    }
}
...
extension TotalerViewController: JXSegmentedViewDelegate {
...
extension TotalerViewController: JXPagingViewDelegate {
...
extension TotalerViewController {
    ...
    private func note() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(dataPathFormat)
        }
    }

    /// 添加约束
    //: private func setupSubViewsConstraint() {
    private func setupLikeConfinement() {
        //: pagerView.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight)
        pagerView.frame = CGRect(x: 0, y: 0, width: show_objectValue, height: constBlockIdentityContent)
    ...
    private func someDetectProduct() {
        // 个人信息更新成功刷新UI
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(refreshUserInfoNotification),
                                               selector: #selector(airVersion),
