...
/*: "Face verification" :*/
fileprivate let constAdScopeMessage:[Character] = ["F","a","c","e"," ","v","e","r","i","f","i"]
fileprivate let userMoveMsg:String = "catiadministratorn"
...
class OutOfDoorsThen: EverlastinglyThen {
    ...
    override func viewDidLoad() {
        ...
        title = (String(constAdScopeMessage) + userMoveMsg.replacingOccurrences(of: "administrator", with: "o")).localized
        //: self.createUI()
        self.afterUi()
    ...
    lazy var mainView: EyebrowView = {
        //: let view = TalkingVerificationExampleView()
        let view = EyebrowView()
        //: return view
        return view
        //: }()
    }()
...
extension OutOfDoorsThen: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        ...
        self.dismiss(animated: true) {
            ...
            if let photo = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
                ...
                if self.isRegisterPush {
                    ...
                    let vc = PhotoControllerDelegate(photoImage: photo)
...
extension OutOfDoorsThen {
    ...
    func afterUi() {
        ...
        mainView.btnClickBlock = { [weak self] () in
            ...
            GatePermissionTool.successfullyFor(true) { isOpen in
