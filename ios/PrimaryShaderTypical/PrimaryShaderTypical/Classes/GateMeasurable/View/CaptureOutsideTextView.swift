...
fileprivate let dataWhiteKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Edit Title" :*/
fileprivate let constCurrentKey:[Character] = ["E","d","i","t"," ","T","i"]
fileprivate let appStuffMsg:String = "tlweight"

/*: "Finish" :*/
fileprivate let showRugFarPath:[Character] = ["F","i","n","i","s"]
fileprivate let userErrorId:String = "doc"

/*: "btn_delete" :*/
fileprivate let kLiteralStr:String = "btn_deljob mail gap inmate"
fileprivate let appAnyoneValue:String = "fartfar"
...
class CaptureOutsideTextView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(straddleBrushDoingRaft))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataWhiteKey.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(constCurrentKey) + appStuffMsg.replacingOccurrences(of: "weight", with: "e")).localized
    ...
    lazy var inputTView: AccessibleToothHasDelegate = {
        //: let textView = TalkingTextView()
        let textView = AccessibleToothHasDelegate()
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(showRugFarPath) + userErrorId.replacingOccurrences(of: "doc", with: "h")).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.tillOfCreation(colors: UIColor.spectralColor(), size: CGSize(width: 267, height: 50)), for: .normal)
        //: btn.layer.cornerRadius = 25
        btn.layer.cornerRadius = 25
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(finishBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(labelPrice), for: .touchUpInside)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.lastTo(name: (String(kLiteralStr.prefix(7)) + appAnyoneValue.replacingOccurrences(of: "far", with: "e"))), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(balance), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension CaptureOutsideTextView {
    //: @objc func dismissView() {
    @objc func straddleBrushDoingRaft() {}

    //: @objc func finishBtnClick() {
    @objc func labelPrice() {
        //: dismiss()
        popular()
        //: if textUpload != nil {
        if textUpload != nil {
...
extension CaptureOutsideTextView: UITextViewDelegate {
