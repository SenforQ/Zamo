...
fileprivate let dataReserveMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Birthday" :*/
fileprivate let user_vidPath:String = "Birthis makeup"

/*: "btn_me_edit" :*/
fileprivate let main_spendMsg:String = "bot regular mobile variationbtn_m"
...
class DiskViewDelegate: UITableViewCell, TreeViewDelegate {
    //: var myDateView: TalkingDateView!
    var myDateView: PubliclyRocketView!
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataReserveMessage.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(user_vidPath.prefix(4)) + "hday").localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var editImag: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.lastTo(name: (String(main_spendMsg.suffix(5)) + "e_edit")))
        //: return imag
        return imag
        //: }()
    }()
    ...
    lazy var dataBtn: UIButton = {
        ...
        //: dataBtn.setTitle(MentionTriggerAppManager.share.loginUserMode.birthday, for: .normal)
        dataBtn.setTitle(MentionTriggerAppManager.share.loginUserMode.birthday, for: .normal)
        //: dataBtn.setTitleColor(UIColor.appTitleColor(), for: .normal)
        dataBtn.setTitleColor(UIColor.roundChromaticColor(), for: .normal)
        //: dataBtn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 16)
        dataBtn.titleLabel?.font = UIFont.landEnableSize(type: .Medium, fontSize: 16)
        //: dataBtn.addTarget(self, action: #selector(dataBtnClick), for: .touchUpInside)
        dataBtn.addTarget(self, action: #selector(simply), for: .touchUpInside)
        //: return dataBtn
        return dataBtn
        //: }()
    }()
    ...
    @objc func simply() {
        ...
        if parts?.count ?? 0 >= 3 {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
                ...
                self.myDateView.momentEnableMedal(year: year, month: month, day: day)
            }
        }
    }
