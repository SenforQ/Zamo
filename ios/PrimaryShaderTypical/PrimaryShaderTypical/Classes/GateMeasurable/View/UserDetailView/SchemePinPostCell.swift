...
fileprivate let k_bothMsg:[UInt8] = [0xd5,0xd2,0xd5,0xc8,0x94,0xdf,0xd3,0xd8,0xd9,0xce,0x86,0x95,0x9c,0xd4,0xdd,0xcf,0x9c,0xd2,0xd3,0xc8,0x9c,0xde,0xd9,0xd9,0xd2,0x9c,0xd5,0xd1,0xcc,0xd0,0xd9,0xd1,0xd9,0xd2,0xc8,0xd9,0xd8]

private func mediumPlan(woman num: UInt8) -> UInt8 {
    return num ^ 188
}

/*: "More" :*/
fileprivate let noti_headlineMsg:[Character] = ["M","o","r","e"]

/*: "icon_details_arrow" :*/
fileprivate let userWorthMsg:String = "iediton"
fileprivate let showCreatureName:String = "areadyls"
fileprivate let show_premiumValue:[Character] = ["_","a","r","r","o","w"]

/*: "Post" :*/
fileprivate let appYaName:[Character] = ["P","o","s","t"]

/*: "btn_dynamic_stop_nor" :*/
fileprivate let mainBeautyName:String = "btn_dtop explain"
fileprivate let main_clothesMonthId:String = "T"
fileprivate let k_hormoneTitle:String = "op_norprovince open destroy"
...
class SchemePinPostCell: UITableViewCell {
    //: var userId: String = ""
    var userId: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_bothMsg.map{mediumPlan(woman: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .judgeAdvertising(0, 0, 0, 0.4)
    ...
    lazy var moreButton: EarthThen = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .landEnableSize(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(noti_headlineMsg)).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (userWorthMsg.replacingOccurrences(of: "edit", with: "c") + "_det" + showCreatureName.replacingOccurrences(of: "ready", with: "i") + String(show_premiumValue))), for: .normal)
    ...
    lazy var pushBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(militaryQuarters), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension SchemePinPostCell {
    ...
    func unAgency(userModel: TipRugInfoModel) {
        ...
        strA = [(String(appYaName)).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.roundChromaticColor(), UIColor.todaySkinColor()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.wordPicture(fontSize: 18), UIFont.wordPicture(fontSize: 16)]
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: CoreItemModel = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.buttonShape())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    mapBtn(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.buttonShape())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    mapBtn(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.buttonShape())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    mapBtn(btn: thirdImgView)
                }
            }
        }
    }
    ...
    func mapBtn(btn: UIImageView) {
        ...
        let imag = UIImageView(image: UIImage(named: (String(mainBeautyName.prefix(5)) + "ynamic_s" + main_clothesMonthId.lowercased() + String(k_hormoneTitle.prefix(6)))))
...
extension SchemePinPostCell {
    ...
    private func pastGenerate() {
        ...
        let height = (show_objectValue - actualWidth(w: 15 * 4)) / 3.0
