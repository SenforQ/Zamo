...
- (NSData *)beyondQuote {
    ...
    UIImage *image = self.sharedCompress;
...
- (UIImage *)nameImage {
    ...
    return [UIImage imageWithData:[self beyondQuote]];
}
...
- (UIImage *)fill {
    ...
    return [self error:YES];
}

//: - (UIImage *)wcTimelineCompress {
- (UIImage *)sharedCompress {
...
- (UIImage *)error:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self venaFacialisSize:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self faceMethod:size];
...
- (CGSize)venaFacialisSize:(Boolean)isSession {
...
+ (NSData *)captureMenu:(UIImage *)image storage:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image description:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG strength:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG resize:scalesize countRelationEyeDigitiser:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG strength:(fileSize*1024)];
    }
...
- (UIImage*)description:(CGSize)targetSize{
...
- (UIImage *)resize:(CGSize)targetSize countRelationEyeDigitiser:(BOOL)isScale{
...
- (NSData *)strength:(NSInteger)fileSize{
...
- (UIImage *)faceMethod:(CGSize)newSize {
