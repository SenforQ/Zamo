...
/*: "recharge" :*/
fileprivate let user_mannerUrl:String = "pagechar"
fileprivate let constJudgeTitle:[Character] = ["g","e"]

/*: "recharge_ :*/
fileprivate let data_squareFormat:[Character] = ["r","e","c","h","a"]
fileprivate let notiCurMoonData:String = "rge_retain join"

/*: ".png" :*/
fileprivate let data_lookId:String = ".pngway ex age bulb"

/*: .png" :*/
fileprivate let showTagPath:String = ".pnginsert business telephone"

/*: ".jpg" :*/
fileprivate let show_artifactText:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let kMotivationName:[Character] = [".","j","p","g"]
...
class EarthThen: UIButton {
    ...
    var imageAlignment: GrayTotal = .left
...
extension UIButton {
    ...
    func someoneTitle(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        written(interval: interval, topOffset: 0)
    }
    ...
    func written(interval: CGFloat, topOffset: CGFloat) {
    ...
    func statuteTitleKeep(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func pastAccess(color: UIColor, forState: UIControl.State) {
    ...
    func mistakeUs(urlStr: String,
                   //: placeImg: UIImage? = UIImage.placeImgSquare(),
                   placeImg: UIImage? = UIImage.buttonShape(),
                   ...
                   loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: appTurnMessage.birthOrShape(), with: dataCancelMessage)
    ...
    func sinceHeadQuit(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((user_mannerUrl.replacingOccurrences(of: "page", with: "re") + String(constJudgeTitle))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (user_mannerUrl.replacingOccurrences(of: "page", with: "re") + String(constJudgeTitle)), with: (String(data_squareFormat) + String(notiCurMoonData.prefix(4))) + "\(user_underFormat)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.mistakeUs(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(data_lookId.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(data_lookId.prefix(4))), with: "_\(user_underFormat)" + (String(showTagPath.prefix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(show_artifactText))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(show_artifactText)), with: "_\(user_underFormat)" + (String(kMotivationName)))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.mistakeUs(urlStr: newUrl, placeImg: nil)
    }
