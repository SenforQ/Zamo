...
/*: "http://stat. :*/
fileprivate let appFleetDailyUrl:String = "hdistancedistancep"

/*: .com" :*/
fileprivate let show_tellName:String = ".comdocument number earn sense family"

/*: "http://" :*/
fileprivate let notiUsedTitle:[Character] = ["h"]
fileprivate let const_plotName:[Character] = ["t","t","p",":","/","/"]

/*: "https://" :*/
fileprivate let show_earningsId:String = "timing history calculationhttps:"
fileprivate let noti_raftFormat:String = "presentpresent"

/*: "http://m. :*/
fileprivate let showDataUrl:[Character] = ["h","t","t"]
fileprivate let mainFlushlyText:String = "approval enthusiasm familyp://m."

/*: "http:// :*/
fileprivate let k_disagreeData:[Character] = ["h"]
fileprivate let user_movieContent:String = "ttp://positive sample average headline"

/*: "3,90" :*/
fileprivate let app_familyFromFormat:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let noti_violationValue:String = "1990-01"
fileprivate let mainProfilePath:String = "-01"

/*: "MM-dd-yyyy" :*/
fileprivate let data_speciesMessage:[Character] = ["M"]
fileprivate let user_prepareId:[Character] = ["M","-","d","d","-","y","y","y","y"]
...
class ConfinementRequestModel: NSObject {
    ...
    var requestType: SpeakRequestType?
    ...
    override init() {
        ...
        self.requestServer = ProjectionThen.share.serverUrlStr
...
struct TopicBaseResponse: HandyJSON {
    //: var errno: Int!
...
public struct RiseErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct OverHandyJSON: HandyJSON {
    ...
    var CSConfig = MentionTransformable() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (appFleetDailyUrl.replacingOccurrences(of: "distance", with: "t") + "://stat.") + "\(dataCancelMessage)" + (String(show_tellName.prefix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(notiUsedTitle) + String(const_plotName))) && !statDomain.hasPrefix((String(show_earningsId.suffix(6)) + noti_raftFormat.replacingOccurrences(of: "present", with: "/"))) {
                //: return "http://".appending(statDomain)
                return (String(notiUsedTitle) + String(const_plotName)).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(showDataUrl) + String(mainFlushlyText.suffix(6))) + "\(dataCancelMessage)" + (String(show_tellName.prefix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(k_disagreeData) + String(user_movieContent.prefix(6))) + "\(urlDomain)"
        }
    }
...
public class UserMeasurable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [OrganTransformable] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [OrganTransformable] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [OrganTransformable] = [] // 语音收费设置
    ...
    var vipSetting = [ModelTransformable]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [FlowMeasurable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = PeerNumberegrityLimitModel() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class IssueStillUserModel: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [AlwaysTagModel]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [AlwaysTagModel]?
    ...
    var gallery: [SchemeTransformable]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (app_familyFromFormat.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.languageWith(self.birthday ?? (noti_violationValue.capitalized + mainProfilePath.capitalized), dateFormat: (String(data_speciesMessage) + String(user_prepareId)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.okThreshold().year else { return "0" }
...
struct SchemeTransformable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct AlwaysTagModel: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func layTo(compareTo: AlwaysTagModel) -> Bool {
        //: return
        return
...
struct SimulationWouldInfoModel {
...
struct SubdivisionTransformable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [HomeHandyJSON]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [HomeHandyJSON]!
}

//: struct UserTagTypeModel: HandyJSON {
struct HomeHandyJSON: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [AlwaysTagModel]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct MentionTransformable: HandyJSON {
...
struct PeerNumberegrityLimitModel: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
