...
/*: "add_header_cookies" :*/
fileprivate let app_literalPhotoKey:String = "add_heangle gesture"
fileprivate let show_halfPiPropertyPath:[Character] = ["o","o","k","i","e","s"]
...
public extension UIImageView {
    ...
    @objc func contactMoment(urlStr: String,
    ...
    internal func choiceRest(urlStr: String,
                             //: placeImg: UIImage = UIImage.placeImgSquare(),
                             placeImg: UIImage = UIImage.buttonShape(),
    ...
    @objc func alter(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: appTurnMessage.birthOrShape(), with: dataCancelMessage)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.ignoreImage())
    }
    ...
    @objc func valuation(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: ProjectionThen.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(app_literalPhotoKey.prefix(6)) + "ader_c" + String(show_halfPiPropertyPath)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.buttonShape(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
