...
Byte k_micTaskKey[] = {24, 33, 3, 7, 59, 251, 124, 67, 94, 96, 98, 62, 96, 113, 102, 108, 107, 66, 115, 98, 107, 113, 92, 70, 107, 113, 98, 111, 115, 94, 105, 92, 46, 47, 92, 99, 111, 94, 106, 98, 61};

//: FaceActionEvent_Interval_1_second
Byte noti_pullData[] = {3, 33, 57, 4, 13, 40, 42, 44, 8, 42, 59, 48, 54, 53, 12, 61, 44, 53, 59, 38, 16, 53, 59, 44, 57, 61, 40, 51, 38, 248, 38, 58, 44, 42, 54, 53, 43, 61};

//: hasFace
Byte show_clipStr[] = {82, 7, 72, 10, 113, 169, 189, 63, 168, 48, 32, 25, 43, 254, 25, 27, 29, 56};

//: key.Notification.Tap.To.Modify.Exposure.Point
Byte main_provideTitle[] = {57, 45, 55, 6, 187, 201, 52, 46, 66, 247, 23, 56, 61, 50, 47, 50, 44, 42, 61, 50, 56, 55, 247, 29, 42, 57, 247, 29, 56, 247, 22, 56, 45, 50, 47, 66, 247, 14, 65, 57, 56, 60, 62, 59, 46, 247, 25, 56, 50, 55, 61, 169};

//: M_SenseME_Face_Video_5.3.4
Byte userMobileFormat[] = {69, 26, 45, 10, 85, 61, 203, 139, 10, 110, 32, 50, 38, 56, 65, 70, 56, 32, 24, 50, 25, 52, 54, 56, 50, 41, 60, 55, 56, 66, 50, 8, 1, 6, 1, 7, 214};

//: model
Byte k_dimensionMessage[] = {73, 5, 6, 4, 103, 105, 94, 95, 102, 200};

//: camera_focus_red
Byte noti_rejectName[] = {77, 16, 21, 12, 37, 120, 239, 200, 183, 253, 63, 90, 78, 76, 88, 80, 93, 76, 74, 81, 90, 78, 96, 94, 74, 93, 80, 79, 250};
...
//
//#import "AlivePeer.h"
//#import "AlwaysReactiveCompatible.h"
//ST_MOBILE

// __M_A_C_R_O__
//: #import "STFilterView.h"
#import "LightView.h"
//: #import "PrimaryShaderTypical/PrimaryShaderTypical-Swift.h"
#import "PrimaryShaderTypical/PrimaryShaderTypical-Swift.h"
...
@interface LightView ()<DictionaryEntryCooperative,UIGestureRecognizerDelegate>
{
...
//: @property (nonatomic, strong) DirectlyLicHelper * lichelper;
@property (nonatomic, strong) DirectlyLicHelper * lichelper;
//: @property (nonatomic, strong) SupporterReactiveCompatible * ParamUtil;
@property (nonatomic, strong) SupporterReactiveCompatible * ParamUtil;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //: self.lichelper = [DirectlyLicHelper share];
        self.lichelper = [DirectlyLicHelper share];
        //: self.ParamUtil = [[SupporterReactiveCompatible alloc]init];
        self.ParamUtil = [[SupporterReactiveCompatible alloc]init];
        //: _currentDropCount = 5;
        ...
        if ([self.lichelper ageLicense]) {
            ...
            //: [[AlivePeer share] getLocalModel];
            [[AlivePeer share] attentionJoin];
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   ...
                                                   selector:@selector(tapses:)
                                                       //: name:@"key.Notification.Tap.To.Modify.Exposure.Point"
                                                       name:StringFromDimensionData(main_provideTitle)
                                                     //: object:nil];
                                                     object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(withSave:) name:self.ParamUtil.kNotificationSelectBeautyFilterStr object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(captures:) name:self.ParamUtil.kNotificationBeautyFilterValueChangeStr object:nil];
...
- (instancetype)initMagnitudeerestConversation:(NSString*)sessionPreset {
    ...
    if (self = [super init]) {
        ...
        if ([self.lichelper ageLicense]) {
            ...
            [self initModelView:sessionPreset];

        //: } else {
        } else {
            //: [DirectlyLicHelper.share checkRemoteLicInfoWithCompletion:^(BOOL x) {
            [DirectlyLicHelper.share aboveBodyWithCompletion:^(BOOL x) {
                //: if (x) {
                if (x) {
                    //: [self initFilterView:sessionPreset];
                    [self initModelView:sessionPreset];
...
- (void)initModelView:(NSString*)sessionPreset {
    //: self.layer.masksToBounds = YES;
    self.layer.masksToBounds = YES;
    //: self.clipsToBounds = YES;
    self.clipsToBounds = YES;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(withSave:)
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(captures:)
...
- (void)available {
    ...
    //: if ([AlivePeer share].filterSuite.filterModelPath.length > 0) {
    if ([AlivePeer share].filterSuite.filterModelPath.length > 0) {
        //: AlwaysReactiveCompatible* model = [[AlivePeer share] getFilterDataModel];
        AlwaysReactiveCompatible* model = [[AlivePeer share] userTransform];
        //: if (model) {
        if (model) {
...
- (void)setCurrentSessionPreset:(NSString *)currentSessionPreset {
    ...
    [self.stCamera line:self.previewCenter location:self.previewFrame];
}
...
- (void)capital:(BOOL)isFullScreen {
    ...
    [self focusIcon:width pushFloat:height];
}
/// 设置视频通话预览的采集大小
...
- (void)last {
    ...
    [self.stCamera askDark];

}

//: - (void)setupCameraAndPreview {
- (void)linguisticContextPlus {

    //: int fps = 18;
    int fps = 18;

    //: _stCamera = [[STFilterCamera alloc] initWithDevicePosition:AVCaptureDevicePositionFront
    _stCamera = [[Camera alloc] initWithPostAvailable:AVCaptureDevicePositionFront
                                          //: sessionPresset:self.currentSessionPreset
                                          wallTable:self.currentSessionPreset
                                                     //: fps:fps
                                                     wait:fps
                                           //: needYuvOutput:NO];
                                           ask:NO];
...
- (void)focusIcon:(CGFloat)width pushFloat:(CGFloat)height {
    //: CGRect previewRect = [self.stCamera getScaleRect:CGRectMake(0 , 0 , width, height)
    CGRect previewRect = [self.stCamera origin:CGRectMake(0 , 0 , width, height)
                                                   //: scaleToFit:NO];
                                                   roundCapacity:NO];
    ...
    if (self.glPreview.superview == nil) {
        ...
        @synchronized (self) {
           ...
           _glPreview = [[PathCompareView alloc] initWithCapacity:previewRect glassesPresent:previewContext];
        }
...
- (void)model {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(shapeLocal) name:UIApplicationWillResignActiveNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(clearContent) name:UIApplicationDidEnterBackgroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appWillEnterForeground) name:UIApplicationWillEnterForegroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(enterMin) name:UIApplicationWillEnterForegroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidBecomeActive) name:UIApplicationDidBecomeActiveNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(activeAgent) name:UIApplicationDidBecomeActiveNotification object:nil];
}
...
- (void)press:(CMAccelerometerData *)accelerometerData {
...
- (st_rotate_type)circle {
    ...
    [self press:self.motionManager.accelerometerData];
...
- (void)captureLocation {
    ...
    //初始化检测模块句柄
    //: NSString *strModelPath = [EverlastinglyReactiveCompatible.getModelFileName stringByAppendingPathComponent:@"M_SenseME_Face_Video_5.3.4"];
    NSString *strModelPath = [EverlastinglyReactiveCompatible.advert stringByAppendingPathComponent:StringFromDimensionData(userMobileFormat)];
    //: strModelPath = [strModelPath stringByAppendingPathExtension:@"model"];
    strModelPath = [strModelPath stringByAppendingPathExtension:StringFromDimensionData(k_dimensionMessage)];
...
- (BOOL)addBuffer:(CVPixelBufferRef)pixelBuffer {
...
- (void)tapses:(NSNotification *)sender {
    ...
    if ([tapGesture isKindOfClass:UITapGestureRecognizer.class]) {
        ...
        [self delayedAction:tapGesture];
    }

}

//: - (void)tapScreen:(UITapGestureRecognizer *)tapGesture {
- (void)delayedAction:(UITapGestureRecognizer *)tapGesture {
    ...
    [self.stCamera line:point location:self.frame];
...
- (UIImageView *)focusImageView {
    ...
    if (!_focusImageView) {
        ...
        _focusImageView.image = [UIImage imageNamed:StringFromDimensionData(noti_rejectName)];
...
- (void)withSave:(NSNotification*) notice {
    //: if ([notice.object isKindOfClass:[AlwaysReactiveCompatible class]]) {
    if ([notice.object isKindOfClass:[AlwaysReactiveCompatible class]]) {
        //: AlwaysReactiveCompatible* model = (AlwaysReactiveCompatible*)notice.object;
        AlwaysReactiveCompatible* model = (AlwaysReactiveCompatible*)notice.object;
        //: [self handleFilterChanged:model];
        [self to:model];
    }
...
//: - (void)handleFilterChanged:(AlwaysReactiveCompatible *)model {
- (void)to:(AlwaysReactiveCompatible *)model {
...
- (void)captures:(NSNotification*) notice {
    //: AlwaysReactiveCompatible* model = (AlwaysReactiveCompatible*)notice.object;
    AlwaysReactiveCompatible* model = (AlwaysReactiveCompatible*)notice.object;
    //: if (_hFilter) {
    if (_hFilter) {
...
- (void)spark {
    ...
    //: MarketEntiretycostModel *beautyValueCache = AlivePeer.share.currentBeautySuite;
    MarketEntiretycostModel *beautyValueCache = AlivePeer.share.roomsWeight;

    // 设置美白参数
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    // 设置默认红润参数
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    // 设置默认磨皮参数
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    // 设置默认大眼参数
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    // 设置默认瘦脸参数
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    // 设置小脸参数
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];

    //瘦脸型
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    //窄脸
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    //圆眼
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    //下巴
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    //额头
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    //瘦鼻翼
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    //长鼻
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    //嘴形
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self available:beautyValueCache.fMouthStrength]];
    //缩人中
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self available:beautyValueCache.fPhiltrumStrength]];

    //设置对比度参数
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self available:beautyValueCache.fContrastStrength]];

    //设置饱和度参数
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self available:beautyValueCache.fSaturationStrength]];

    //锐化参数
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self available:beautyValueCache.fSharpenStrength]];

    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self available:beautyValueCache.fAppleMusleStrength]];

    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];

    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];

    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];

    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];

    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    //: [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
    [self.ParamUtil passMEigenvalueOfAMatrixWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];

}

//: - (float)rangeMappedValueFrom:(float) rawValue {
- (float)available:(float) rawValue {
...
void replicaSmallLineamentFace(st_mobile_animal_face_t *src, int faceCount, st_mobile_animal_face_t *dst) {
...
void groupLine(st_mobile_animal_face_t *src, int faceCount) {
    //: if (faceCount > 0) {
    if (faceCount > 0) {
        //: for (int i = 0; i < faceCount; ++i) {
        for (int i = 0; i < faceCount; ++i) {
...
- (void)callbackAndBuffer:(CMSampleBufferRef)originalSampleBuffer
                       //: resultPixelBuffer:(CVPixelBufferRef)resultPixelBufffer {
                       relieve:(CVPixelBufferRef)resultPixelBufffer {
    ...
    if ( originalSampleBuffer && resultPixelBufffer ) {
        ...
        if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(perform:applicationProperty:)]) {
            ...
            if (sampleBuffer) {
                ...
                [tmpDelegate perform:self applicationProperty:sampleBuffer];
            }
...
- (CVPixelBufferRef)connection:(CVPixelBufferRef)pixelBuffer {
...
- (void)bright:(AVCaptureOutput *)captureOutput index:(CMSampleBufferRef)sampleBuffer cur:(AVCaptureConnection *)connection {
    ...
    st_rotate_type stMobileRotate = [self circle];
    ...
    if (_hDetector) {
        ...
        if (self.iCurrentAction > 0) {
            ...
            if (_faceFrameCount == 12) { // 第12帧时
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:StringFromDimensionData(k_micTaskKey) object:nil userInfo: @{StringFromDimensionData(show_clipStr): @(hasFace)}];
            }
            ...
            if (_faceFrameCount%18 == 0) { // 每秒发送一次
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:StringFromDimensionData(noti_pullData) object:nil userInfo: @{StringFromDimensionData(show_clipStr): @(hasFace)}];
            }
        }
    }
    ...
    if (faceCount > 0) {
        ...
        replicaSmallLineamentFace(_detectResult1, faceCount, newDetectResult1);
    }
    ...
    dispatch_async(self.renderQueue, ^{
       ...
       BOOL isTextureOriginReady = [self addBuffer:pixelBuffer];
       ...
       // ST_MOBILE 以下为滤镜部分
        ...
        if (callBackFlag) [self callbackAndBuffer:sampleBuffer relieve:resultPixelBufffer];
        ...
        if (faceCount > 0) {
            ...
            groupLine(newDetectResult1, faceCount);
        }
        //: if (callBackFlag) {
        if (callBackFlag) {
            //: @synchronized (self) {
            @synchronized (self) {
                ...
                [self.glPreview window:textureResult];
            }
        }
...
Byte * DimensionDataToCache(Byte *data) {
    ...
    Byte op = data[2];
    int oddLevel = data[3];
    if (!best) return data + oddLevel;
    for (int i = oddLevel; i < oddLevel + colon; i++) {
        int value = data[i] + op;
        if (value > 255) {
            value -= 256;
        }
    ...
    data[0] = 0;
    data[oddLevel + colon] = 0;
    return data + oddLevel;
}
