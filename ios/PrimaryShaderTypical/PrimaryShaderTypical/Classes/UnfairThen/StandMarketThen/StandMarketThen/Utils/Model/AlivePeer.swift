...
/*: "filterSuite" :*/
fileprivate let showSearchedValue:[Character] = ["f","i","l","t","e"]
fileprivate let show_relationPath:String = "charm observerrSuite"

/*: "customSuite" :*/
fileprivate let app_reserveRemarkTitle:String = "custexecutive"

/*: "App/AlivePeer" :*/
fileprivate let constMinimizeMessage:[Character] = ["A","p","p","/","S","T","F","i","l","t","e","r","C"]
fileprivate let mainBarGloryData:[Character] = ["a","c","h","e","C","o","s","t"]
...
//: @objcMembers public class AlivePeer: NSObject, NSSecureCoding {
@objcMembers public class AlivePeer: NSObject, NSSecureCoding {
    //: var curBeautySuite: BeautyItemType?
    var curBeautySuite: ConstraintTransformable?
    //: var curFeature: STEffectsType?
    var curFeature: PassCloseHashable?
    /// 用户的滤镜设置
    //: public var filterSuite: MarketEntiretycostModel?
    public var filterSuite: MarketEntiretycostModel?
    /// 滤镜列表模型数组
    //: var filterModelArray = Array<AlwaysReactiveCompatible>.init()
    var filterModelArray = Array<AlwaysReactiveCompatible>.init()

    //: var customSuite =  MarketEntiretycostModel.init()
    var customSuite = MarketEntiretycostModel() // 用户自定义的美颜

    //: static public let share = AlivePeer()
    public static let share = AlivePeer()
    ...
    public required init?(coder: NSCoder) {
        ...
        let filDict = coder.decodeObject(forKey: (String(showSearchedValue) + String(show_relationPath.suffix(6))))
        //: let custDict =  coder.decodeObject(forKey: "customSuite")
        let custDict = coder.decodeObject(forKey: (app_reserveRemarkTitle.replacingOccurrences(of: "executive", with: "o") + "mSuite"))
        //: if filDict != nil {
        if filDict != nil {
            //: filterSuite = try! MarketEntiretycostModel.init(dictionary: filDict as? [AnyHashable: Any])
            filterSuite = try! MarketEntiretycostModel(dictionary: filDict as? [AnyHashable: Any])
        }
        //: if custDict != nil {
        if custDict != nil {
            //: customSuite = try! MarketEntiretycostModel.init(dictionary: custDict as? [AnyHashable: Any])
            customSuite = try! MarketEntiretycostModel(dictionary: custDict as? [AnyHashable: Any])
        }
    }
    ...
    public func encode(with coder: NSCoder) {
        ...
        coder.encode(filDict, forKey: (String(showSearchedValue) + String(show_relationPath.suffix(6))))
        //: coder.encode(custDict, forKey: "customSuite")
        coder.encode(custDict, forKey: (app_reserveRemarkTitle.replacingOccurrences(of: "executive", with: "o") + "mSuite"))
    }
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(bindSender(sender:)), name: mainBackFormat, object: nil)
    }
...
//: @objc extension AlivePeer {
@objc extension AlivePeer {
    /// 获取缓存
    //: public func getLocalModel() {
    public func attentionJoin() {
        //: let path = self.cachePath()
        let path = self.cacheAlbum()
        ...
        if FileManager.default.fileExists(atPath: path) {
            ...
            if #available(iOS 11.0, *) {
                ...
                do {
                    ...
                    let data = try Data(contentsOf: URL(fileURLWithPath: path))
                    //: if let model: AlivePeer =  try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? AlivePeer {
                    if let model: AlivePeer = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? AlivePeer {
                        //: obj  = model
                        obj = model
                    }
                ...
                //: let model: AlivePeer? = NSKeyedUnarchiver.unarchiveObject(withFile: path) as? AlivePeer
                let model: AlivePeer? = NSKeyedUnarchiver.unarchiveObject(withFile: path) as? AlivePeer
                //: if  model != nil {
                if model != nil {
                    //: obj  = model
    ...
    //: public func currentBeautySuite() -> MarketEntiretycostModel {
    public func roomsWeight() -> MarketEntiretycostModel {
    ...
    //: func checkAllSuite() {
    func noneCoverSuite() {
        //: customSuite  = MarketEntiretycostModel.init().initWith(suiteType: .Face)
        customSuite = MarketEntiretycostModel().nature(suiteType: .Face)

        //: if filterSuite == nil {
        if filterSuite == nil {
            //: filterSuite  = MarketEntiretycostModel.init()
            filterSuite = MarketEntiretycostModel()
        }
    }
    ...
    func cacheAlbum() -> String {
        //: let userId = 0
        let userId = 0

        //: let documentDictPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDictPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDictPath as NSString).appendingPathComponent("App/AlivePeer")
        let appDirectoryPath = (documentDictPath as NSString).appendingPathComponent((String(constMinimizeMessage) + String(mainBarGloryData)))
    ...
    func pariahDog(curBeautySuite: ConstraintTransformable) {
    ...
    func inSwitcheCase() {
        ...
        self.customSuite.suiteMale()
        //: self.curBeautySuite = .Face
        self.curBeautySuite = .Face
        //: self.filterSuite = MarketEntiretycostModel.init()
        self.filterSuite = MarketEntiretycostModel()
    }

    // notification
    //: func didReceiveFeatureValueChange(sender: NSNotification) {
    func bindSender(sender: NSNotification) {
        //: var cache: MarketEntiretycostModel?
        var cache: MarketEntiretycostModel?
        //: cache = self.customSuite
        cache = self.customSuite
        //: let featureModel: AlwaysReactiveCompatible = sender.object as! AlwaysReactiveCompatible
        let featureModel: AlwaysReactiveCompatible = sender.object as! AlwaysReactiveCompatible
        //: handleFeatureValueChangeWith(beautySuite: cache ?? MarketEntiretycostModel.init(), featureModel: featureModel)
        soyModel(beautySuite: cache ?? MarketEntiretycostModel(), featureModel: featureModel)
    }

    //: func handleFeatureValueChangeWith(beautySuite: MarketEntiretycostModel, featureModel: AlwaysReactiveCompatible) {
    func soyModel(beautySuite: MarketEntiretycostModel, featureModel: AlwaysReactiveCompatible) {
        //: if (self.curBeautySuite == .Face) {
        if self.curBeautySuite == .Face {
            //: if featureModel.isKind(of: AlwaysReactiveCompatible.self) == true {
            if featureModel.isKind(of: AlwaysReactiveCompatible.self) == true {
                // [0,1]
    ...
    /// -------初始化滤镜资源
    //: func reloadFilterData(model: AlwaysReactiveCompatible) {
    func flagRecover(model: AlwaysReactiveCompatible) {
    ...
    //: public func getFilterDataModel() -> AlwaysReactiveCompatible? {
    public func userTransform() -> AlwaysReactiveCompatible? {
    ...
    func valueEquity(rawValue: Float) -> Float {
    ...
    //: func initFilterData() {
    func promptSort() {
        //: let orignArray  = AlwaysReactiveCompatible.init().arrayOfFeatureModelWith(categoryType: .Filter)
        let orignArray = AlwaysReactiveCompatible().lifeStyleType(categoryType: .Filter)

        //: if (orignArray.count > 0) {
        if orignArray.count > 0 {
            //: let model = orignArray.first
            let model = orignArray.first
            //: self.filterModelArray = NSMutableArray.init(array: model?.subFilterItemsArr ?? [AlwaysReactiveCompatible].init()) as! [AlwaysReactiveCompatible]
            self.filterModelArray = NSMutableArray(array: model?.subFilterItemsArr ?? [AlwaysReactiveCompatible].init()) as! [AlwaysReactiveCompatible]
        }
    }
}
