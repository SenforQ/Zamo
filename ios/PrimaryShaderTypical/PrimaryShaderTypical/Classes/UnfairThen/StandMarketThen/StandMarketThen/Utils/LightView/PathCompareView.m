...
NSString *StringFromIncludingData(Byte *data);


//: STDisplayShader
Byte showFrameText[] = {36, 15, 15, 7, 161, 91, 78, 98, 99, 83, 120, 130, 127, 123, 112, 136, 98, 119, 112, 115, 116, 129, 169};
...
@implementation PathCompareView
...
- (instancetype)initWithCapacity:(CGRect)frame glassesPresent:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self beforeProgram:StringFromIncludingData(showFrameText)
                //: fragmentShader:@"STDisplayShader"
                extreme:StringFromIncludingData(showFrameText)
                    //: forProgram:&stDisplayProgram];
                    fail:&stDisplayProgram];
...
- (BOOL)beforeProgram:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          extreme:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              fail:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self aboveWhite:&vertexShader model:0x8B31 layer:appFitMsg]) {
    ...
    if (![self aboveWhite:&fragShader model:0x8B30 layer:noti_uniformPath]) {
...
- (BOOL)aboveWhite:(GLuint *)shader model:(GLenum)type layer:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
...
Byte * IncludingDataToCache(Byte *data) {
    int monthDraw = data[0];
    int pronounceableAgree = data[1];
    Byte relational = data[2];
    int windowStat = data[3];
    if (!monthDraw) return data + windowStat;
    for (int i = windowStat; i < windowStat + pronounceableAgree; i++) {
        int value = data[i] - relational;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[windowStat + pronounceableAgree] = 0;
    return data + windowStat;
}

NSString *StringFromIncludingData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)IncludingDataToCache(data)];
}
