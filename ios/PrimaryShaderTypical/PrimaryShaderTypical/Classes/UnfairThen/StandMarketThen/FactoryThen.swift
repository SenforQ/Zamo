...
fileprivate let data_againstName:String = "btn_continue dominant register ting"
fileprivate let main_cutStr:String = "hidden list follow pretty singleack_c"
fileprivate let dataAtUrl:String = "meetingtinue"

/*: "init(coder:) has not been implemented" :*/
fileprivate let app_lovelyTotalStr:[UInt8] = [0x43,0x44,0x43,0x5e,0x2,0x49,0x45,0x4e,0x4f,0x58,0x10,0x3,0xa,0x42,0x4b,0x59,0xa,0x44,0x45,0x5e,0xa,0x48,0x4f,0x4f,0x44,0xa,0x43,0x47,0x5a,0x46,0x4f,0x47,0x4f,0x44,0x5e,0x4f,0x4e]
...
class FactoryThen: UIView {
    //: private var alertConfig: ShowAlertConfig
    private var alertConfig: NumberegrityAlertConfig
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.thirdRevenue().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(replyChin), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        let image = UIImage.lastTo(name: alertConfig.rightBackgroundImage).withTintColor(.thirdRevenue())
        //: rightBtn.setBackgroundImage(image, for: .normal)
        rightBtn.setBackgroundImage(image, for: .normal)
        //: rightBtn.addTarget(self, action: #selector(rightClick), for: .touchUpInside)
        rightBtn.addTarget(self, action: #selector(rightConfirmSince), for: .touchUpInside)
        //: return rightBtn
        return rightBtn
        //: }()
    }()
    ...
    init(title: String? = nil,
         ...
         config: NumberegrityAlertConfig)
    {
        //: alertConfig = config
        alertConfig = config
        // 只有一个按钮，展示大按钮
        ...
        if leftBtnTitle == nil && leftBtnAttributedTitle == nil {
            ...
            alertConfig.rightBackgroundImage = (String(data_againstName.prefix(4)) + "me_b" + String(main_cutStr.suffix(5)) + dataAtUrl.replacingOccurrences(of: "meeting", with: "on"))
        }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_lovelyTotalStr.map{$0^42}, encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func insideVoice(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.lastTo(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.lastTo(name: alertConfig.rightBackgroundImage).withTintColor(.thirdRevenue())
