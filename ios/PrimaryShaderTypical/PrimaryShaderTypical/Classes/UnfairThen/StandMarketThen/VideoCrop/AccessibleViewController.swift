...
fileprivate let kEquityValue:[UInt8] = [0xd0,0xd7,0xd0,0xcd,0x91,0xda,0xd6,0xdd,0xdc,0xcb,0x83,0x90,0x99,0xd1,0xd8,0xca,0x99,0xd7,0xd6,0xcd,0x99,0xdb,0xdc,0xdc,0xd7,0x99,0xd0,0xd4,0xc9,0xd5,0xdc,0xd4,0xdc,0xd7,0xcd,0xdc,0xdd]

private func themeRes(pressure num: UInt8) -> UInt8 {
    return num ^ 185
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let app_honeyValue:String = "btnJQbjudge rank count"
fileprivate let constShotMsg:[Character] = ["E","A","B","_","e","t","i","h","w","_","k","c","a","b","_","s","h","a","d","o","w"]

/*: "Edit" :*/
fileprivate let app_estimateValue:String = "favorite conversation goalEdit"

/*: "Finish" :*/
fileprivate let notiMultiUrl:String = "Finishsecond television characterize recommend"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let show_medalMicName:String = "observe advert can cancel bulbbtnYDo"
fileprivate let mainRocketData:[Character] = ["o","e","d","i","v","_","p","l","a","y"]

/*: "Cancel" :*/
fileprivate let k_firstPath:String = "Canceldo voice"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let main_tingUniformUrl:[Character] = ["b","g","Q","k","R","T","b","F","_","o"]
fileprivate let kWriteValue:String = "picture"
fileprivate let app_tensionWindowPath:[Character] = ["i","v","_","t","o","a","s","t"]

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let main_paperUnlessName:[UInt8] = [0x67,0x5b,0x56,0x13,0x45,0x5a,0x57,0x56,0x5c,0x13,0x5f,0x56,0x5d,0x54,0x47,0x5b,0x13,0x40,0x5b,0x5c,0x46,0x5f,0x57,0x13,0x51,0x56,0x13,0x16,0x73,0x1e,0x16,0x73,0x13,0x40,0x56,0x50,0x5c,0x5d,0x57,0x40,0x1d,0x13,0x63,0x5f,0x56,0x52,0x40,0x56,0x13,0x5a,0x5d,0x47,0x56,0x41,0x50,0x56,0x43,0x47,0x13,0x5a,0x47]

private func stealCooperative(empty num: UInt8) -> UInt8 {
    return num ^ 51
}
...
typealias VideoEditBlock = (_ videoInfo: ExtensiveHandyJSON?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let kSaltMsg: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class AccessibleViewController: EverlastinglyThen {
    ...
    private var slider: ForeverView?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = ExtensiveHandyJSON() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = DirectEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.accessible == true {
        if DisagreeableWomanSocketManager.shared.accessible == true { // 正在音视频通话，静音播放
    ...
    init(style: DirectEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (show_objectValue - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kEquityValue.map{themeRes(pressure: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.lastTo(name: (String(app_honeyValue.prefix(6)) + String(constShotMsg))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = DriveReactiveCompatible.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(betweenSatisfy), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(app_estimateValue.suffix(4))).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.thirdRevenue(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.activeBeautyNative(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = DriveReactiveCompatible.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(notiMultiUrl.prefix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.wordPicture(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.thirdRevenue()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(vocalisationCon), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.lastTo(name: (String(show_medalMicName.suffix(6)) + "2AS_" + String(mainRocketData))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(allColumn(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(k_firstPath.prefix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(access), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.lastTo(name: (String(main_tingUniformUrl) + kWriteValue.replacingOccurrences(of: "picture", with: "ed") + String(app_tensionWindowPath))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: main_paperUnlessName.map{stealCooperative(empty: $0)}, encoding: .utf8)!.takeHomeDiskMini(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: InfoStillReactiveCompatible = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = InfoStillReactiveCompatible(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension AccessibleViewController {
    /// 返回按钮事件
    //: @objc func backAction() {
    @objc func betweenSatisfy() {
        ...
        ButSignatureProvider.shared.simultaneouslyVideo()
    ...
    private func styleAnimated(animated _: Bool = true) {
    ...
    @objc func emphasizing() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: show_objectValue - 2 * 26, height: constBlockIdentityContent - 46 - 127 - userDiskUrl)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = show_objectValue / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: constBlockIdentityContent, width: show_objectValue, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = ForeverView(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: kSaltMsg)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - userDiskUrl)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: AssistantSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func vocalisationCon() {
        ...
        if self.videoEditHandler != nil {
            ...
            ButSignatureProvider.shared.style(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.styleAnimated()
    ...
    @objc func allColumn(sender: UIButton) {
...
extension AccessibleViewController {
    ...
    private func framePlayer() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(creationFinished), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.megascopic(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func propertyPanel() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + userDiskUrl)
        }
    ...
    private func displayScript() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(supraDevice),
