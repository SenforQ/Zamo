...
fileprivate let userRemHowData:String = "btn_study just moon"
fileprivate let main_observerId:String = "than support about viaalertB"
fileprivate let notiRangeMsg:[Character] = ["o","u","n","d"]

/*: "btn_alertBackground_false" :*/
fileprivate let show_throughMinimumBeamName:String = "enable spec register interval acceleratebtn_"
fileprivate let appMustContent:String = "ackproperty"
fileprivate let showAfterHangId:[Character] = ["r","o","u","n","d","_","f","a","l","s","e"]
...
public class StandMarketThen: NSObject {
    /// 通用回调
...
public extension StandMarketThen {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: NumberegrityAlertConfig) -> Void
    ...
    internal class func labelConfig(title: String? = nil,
                                    //: message: String?  = nil,
                                    message: String? = nil,
                                    ...
                                    config: NumberegrityAlertConfig? = nil)
    {
        //: customAlert(title: title,
        objectPresent(title: title,
                      //: message: message,
                      message: message,
    ...
    @objc internal class func recreation(title: String? = nil,
    ...
    class func objectPresent(title: String? = nil,
                             ...
                             config: NumberegrityAlertConfig? = nil)
    {
        ...
        let model = config ?? NumberegrityAlertConfig()

        //: let alertView = TalkingAlertView(title: title,
        let alertView = FactoryThen(title: title,
        ...
        //: EverlastinglyReactiveCompatible.getWindow().addSubview(alertView)
        EverlastinglyReactiveCompatible.insideWindow().addSubview(alertView)
    ...
    class func alertCur() {
        //: EverlastinglyReactiveCompatible.getWindow().subviews.forEach { (view) in
        EverlastinglyReactiveCompatible.insideWindow().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: FactoryThen.self) {
    ...
    class func detachmentGo(title: String, isTouch: Bool) {
        //: EverlastinglyReactiveCompatible.getWindow().subviews.forEach { (view) in
        EverlastinglyReactiveCompatible.insideWindow().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: FactoryThen.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: FactoryThen = view as! FactoryThen
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.insideVoice(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class NumberegrityAlertConfig {
    /// 执行动画时间
    ...
    public var maskType: TuskMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .landEnableSize(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.roundChromaticColor()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .landEnableSize(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.roundChromaticColor()
    ...
    public var buttonFont: UIFont = .landEnableSize(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.thirdRevenue()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(userRemHowData.prefix(4)) + "show_" + String(main_observerId.suffix(6)) + "ackgr" + String(notiRangeMsg))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(show_throughMinimumBeamName.suffix(4)) + "alertB" + appMustContent.replacingOccurrences(of: "property", with: "g") + String(showAfterHangId))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .landEnableSize(type: .Medium, fontSize: 16)
