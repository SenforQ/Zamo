...
fileprivate let kDisplayKey:[UInt8] = [0xd5,0xd2,0xd5,0xc8,0x94,0xdf,0xd3,0xd8,0xd9,0xce,0x86,0x95,0x9c,0xd4,0xdd,0xcf,0x9c,0xd2,0xd3,0xc8,0x9c,0xde,0xd9,0xd9,0xd2,0x9c,0xd5,0xd1,0xcc,0xd0,0xd9,0xd1,0xd9,0xd2,0xc8,0xd9,0xd8]

/*: "bg_medals_podium" :*/
fileprivate let noti_addedData:String = "profit stat userbg_me"
fileprivate let main_placementPath:String = "logodium"

/*: "icon_rank_first_bg" :*/
fileprivate let k_referFitName:String = "icosection"
fileprivate let showThreeMessage:[Character] = ["r","s","t","_","b","g"]

/*: "icon_rank_first" :*/
fileprivate let notiWavePathTitle:[Character] = ["i"]
fileprivate let const_operateValue:[Character] = ["c","o","n","_","r","a","n","k","_","f","i","r","s","t"]

/*: "icon_ammount_points" :*/
fileprivate let showExtraNoAgainstTitle:String = "irepublicn"
fileprivate let mainColumnVisualRandomKey:String = "ount_cookie accuracy count content as"
fileprivate let showSearchionKey:String = "tcorrect"

/*: "icon_lounge" :*/
fileprivate let user_renderTitle:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]

/*: "icon_rank_coin" :*/
fileprivate let user_styleTitle:String = "compare"
fileprivate let notiCluePath:String = "feed land kit sale weakcon_"
fileprivate let kCityGroupForceId:String = "_coinbit rating"

/*: "icon_rank_coin2" :*/
fileprivate let appScoreStr:[Character] = ["i","c","o","n","_","r","a","n"]
fileprivate let showForceName:String = "agent used score goingk_coin2"

/*: "icon_rank_second" :*/
fileprivate let kStickEnableceKey:[Character] = ["i","c","o","n","_","r","a","n"]
fileprivate let show_happyQualityStr:[Character] = ["k","_","s","e","c","o","n","d"]

/*: "icon_rank_third" :*/
fileprivate let showPurchaseMessage:String = "ICO"
fileprivate let showAtContent:String = "depth mobile define theren_ra"
...
class PerplexityThen: UIView {
    ...
    var dataArray: [SocialTransformable] = [] {
        //: didSet {
        didSet {
    ...
    override init(frame: CGRect) {
        ...
        self.liveNumbereraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: kDisplayKey.map{$0^188}, encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.lastTo(name: (String(noti_addedData.suffix(5)) + "dals_" + main_placementPath.replacingOccurrences(of: "log", with: "p")))
    }

    //: lazy var firstView = SocialRankHeaderItemView.init(frame: .zero, rankType: .firstRankType).then {
    lazy var firstView = AmbitiousReactiveCompatible(frame: .zero, rankType: .firstRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 0
        $0.tag = 0
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(registTapAction))
        let tap = UITapGestureRecognizer(target: self, action: #selector(positionTo))
        //: $0.addGestureRecognizer(tap)
        $0.addGestureRecognizer(tap)
    }

    //: lazy var secondView = SocialRankHeaderItemView.init(frame: .zero, rankType: .secondRankType).then {
    lazy var secondView = AmbitiousReactiveCompatible(frame: .zero, rankType: .secondRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 1
        $0.tag = 1
    ...
    lazy var thirdView = AmbitiousReactiveCompatible(frame: .zero, rankType: .thirdRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 2
        $0.tag = 2
...
extension PerplexityThen {
    //: private func bindInteraction() {
    private func liveNumbereraction() {}

    //: private func updateSubviewsWithData() {
    private func remainData() {
        //: firstView.updateItemWithModel(model: dataArray[0], type: self.index)
        firstView.foreRemoteType(model: dataArray[0], type: self.index)
        //: secondView.updateItemWithModel(model: dataArray.count>1 ? dataArray[1]: nil, type: self.index)
        secondView.foreRemoteType(model: dataArray.count > 1 ? dataArray[1] : nil, type: self.index)
        //: thirdView.updateItemWithModel(model: dataArray.count>2 ? dataArray[2]: nil, type: self.index)
        thirdView.foreRemoteType(model: dataArray.count > 2 ? dataArray[2] : nil, type: self.index)
    }

    //: @objc func registTapAction(tapGesture: UITapGestureRecognizer) {
    @objc func positionTo(tapGesture: UITapGestureRecognizer) {
...
extension PerplexityThen {
    ...
    private func nowCrush() {
        //: let firstWidth = (ScreenWidth - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        let firstWidth = (show_objectValue - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (show_objectValue - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
// MARK: - AmbitiousReactiveCompatible

//: class SocialRankHeaderItemView: UIView {
class AmbitiousReactiveCompatible: UIView {
    ...
    public init(frame: CGRect, rankType: ButGuide) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.beyondAction(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.lastTo(name: (k_referFitName.replacingOccurrences(of: "section", with: "n") + "_rank_fi" + String(showThreeMessage)))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.lastTo(name: (String(notiWavePathTitle) + String(const_operateValue)))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .landEnableSize(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = EarthThen().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.lastTo(name: (showExtraNoAgainstTitle.replacingOccurrences(of: "republic", with: "co") + "_amm" + String(mainColumnVisualRandomKey.prefix(5)) + "poin" + showSearchionKey.replacingOccurrences(of: "correct", with: "s"))), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .landEnableSize(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.lastTo(name: (String(user_renderTitle)))
    }
}
...
extension AmbitiousReactiveCompatible {
    //: func updateItemWithModel(model: SocialRankItemModel?, type: Int) {
    func foreRemoteType(model: SocialTransformable?, type: Int) {
        ...
        headPic.choiceRest(urlStr: model!.headPic, placeImg: UIImage.delayConvert(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.lastTo(name: (user_styleTitle.replacingOccurrences(of: "compare", with: "i") + String(notiCluePath.suffix(4)) + "rank" + String(kCityGroupForceId.prefix(5)))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.lastTo(name: (String(appScoreStr) + String(showForceName.suffix(7)))), for: .normal)
        }
    }
}
...
extension AmbitiousReactiveCompatible {
    ...
    func beyondAction(type: ButGuide) {
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.lastTo(name: (String(notiWavePathTitle) + String(const_operateValue)))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.judgeAdvertising(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.lastTo(name: (String(kStickEnableceKey) + String(show_happyQualityStr)))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.judgeAdvertising(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.lastTo(name: (showPurchaseMessage.lowercased() + String(showAtContent.suffix(4)) + "nk_third"))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.judgeAdvertising(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
