...
/*: "Search" :*/
fileprivate let userCurVidAlreadyUrl:[Character] = ["S","e","a","r","c","h"]

/*: "#F5F5F5" :*/
fileprivate let constDirectKey:[Character] = ["#","F","5","F","5"]
fileprivate let show_hideValue:String = "apply"

/*: "Search by UID" :*/
fileprivate let userPlayerKey:[Character] = ["S","e","a","r","c","h"," "]
fileprivate let const_bringId:String = "by UIDhis canvas golf space"

/*: "icon_search_sea" :*/
fileprivate let userBearWomanValue:String = "above case computer less loadicon_s"
fileprivate let main_unknownUrl:[Character] = ["e","a"]

/*: "Can’t find~" :*/
fileprivate let kShallName:String = "Can’publicly area"

/*: "icon_kong_kong_default" :*/
fileprivate let noti_weightText:[Character] = ["i","c","o","n","_","k","o","n","g","_","k"]
fileprivate let notiRoundName:String = "bulb square drawong_"
fileprivate let data_agreeUrl:String = "customefault"

/*: "Please enter the query UID" :*/
fileprivate let show_menuKey:[Character] = ["P","l","e","a","s","e"]
fileprivate let dataReflectionWomanNowFormat:String = "reject var extent ent"
fileprivate let mainSpendData:String = "bag make gold region slowhe que"
...
class OutsideViewController: EverlastinglyThen {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(userCurVidAlreadyUrl)).localized, for: .normal)
        //: btn.setTitleColor(.white, for: .normal)
        btn.setTitleColor(.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 15)
        btn.titleLabel?.font = UIFont.activeBeautyNative(fontSize: 15)
        ...
        btn.setBackgroundImage(UIImage.tillOfCreation(colors: UIColor.spectralColor(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(someSnap), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(constDirectKey) + show_hideValue.replacingOccurrences(of: "apply", with: "F5")))
        //: tf.textColor = UIColor.appTitleColor()
        tf.textColor = UIColor.roundChromaticColor()
        //: tf.font = UIFont.pingfangRugularFont(fontSize: 16)
        tf.font = UIFont.activeBeautyNative(fontSize: 16)
        ...
        tf.placeholder = (String(userPlayerKey) + String(const_bringId.prefix(6))).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.lastTo(name: (String(userBearWomanValue.suffix(6)) + "earch_s" + String(main_unknownUrl)))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(CompartmentThen.self, forCellReuseIdentifier: CompartmentThen.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(kShallName.prefix(4)) + "t find~").localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(noti_weightText) + String(notiRoundName.suffix(4)) + data_agreeUrl.replacingOccurrences(of: "custom", with: "d"))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + const_matchText, width: show_objectValue, height: constBlockIdentityContent - (14 + const_matchText + appMarginFormat)), style: style)
    ...
    private lazy var reqManager: PackCheckManager = //: return PackCheckManager()
        .init()
    //: }()
}
...
extension OutsideViewController {
    ...
    private func seekSuccessfully() {
        //: self.view.endEditing(true)
        self.view.endEditing(true)
        //: guard searchTF.text?.count ?? 0 > 0 else {
        guard searchTF.text?.count ?? 0 > 0 else {
            //: self.func__showStatusBarErrorMsg(showMsg: "Please enter the query UID".localized)
            self.outBarMsg(showMsg: (String(show_menuKey) + String(dataReflectionWomanNowFormat.suffix(4)) + "er t" + String(mainSpendData.suffix(6)) + "ry UID").localized)
            //: return
            return
        }
        ...
        reqManager.daily(uid: searchTF.text ?? "") { [weak self] _, _, _ in
            //: ProgressHUD.dismiss()
...
extension OutsideViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.towardPlatform(model: model)
...
extension OutsideViewController: UITextFieldDelegate {
...
extension OutsideViewController: JXPagingViewListViewDelegate {
...
extension OutsideViewController {
    ...
    private func youngJoinConstraint() {
