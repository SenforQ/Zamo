...
class DisembarrassTool: NSObject {
    //: var infoModel: TalkingVideoChatModel?          
    var infoModel: LovelyChatModel?          // 视频通话model
    //: var config = TalkingVideoChatConfig()          
    var config = CircumferenceChatConfig()          // 视频渲染配置类
    ...
    private var trtcView: TipVideoView?    // 预览view（使用商汤美颜）
...
extension DisembarrassTool {
    ...
    func estimatedFront(_ frontCamera: Bool) {
        ...
        self.payingAttention(front: frontCamera)
    }
    
    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func bassContext(_ openCamera: Bool) {
    ...
    func yellowness() {
        ...
        DispatchQueue.main.async {
            ...
            self.addReward(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension DisembarrassTool {
    ...
    private func payingAttention(front: Bool) {
        //: guard SenseTime_Use == true else {
        guard user_voiceData == true else {
        ...
        self.trtcView?.notStatUp(isFront: front)
    }
    
    /// 是否开启摄像头
    /// - Parameter isOpen: 是否开启
    //: private func setTRTCCamera(isOpen: Bool) {
    private func soap(isOpen: Bool) {
        //: guard SenseTime_Use == true else {
        guard user_voiceData == true else {
            //: if isOpen {
            if isOpen {
        ...
        self.trtcView?.candidCamera(isOpen: isOpen)
...
extension DisembarrassTool {
    ...
    func andGame() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(enterConnection(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(containNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(timeOpen(notif:)),
    ...
    @objc private func enterConnection(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func joint(notif: Notification?) {
        //: if notif == nil { 
    ...
    @objc private func containNotif(notif: Notification) {
    ...
    @objc func timeOpen(notif: Notification) {
...
extension DisembarrassTool {
    ...
    func addReward(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard user_voiceData == true else {
        ...
        if self.trtcView == nil {
            ...
            self.trtcView = TipVideoView()
        }
        //: self.trtcView?.startPreView(isSmallWindow: smallWindow)
        self.trtcView?.beauty(isSmallWindow: smallWindow)
        //: if smallWindow { 
...
//    }
    ...
    private func weltanschauung() {
        ...
        //: if self.config.accessible == true {
        if self.config.accessible == true {
