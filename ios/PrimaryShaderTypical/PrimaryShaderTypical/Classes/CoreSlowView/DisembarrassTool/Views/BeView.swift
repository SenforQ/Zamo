...
/*: "icon_randow_user_bg" :*/
fileprivate let user_sincePath:String = "bold curtain tableicon_r"
fileprivate let show_zzDenyObserverKey:String = "down"
fileprivate let mainExistGravityName:String = "ser_bgdark bind variation ion out"
...
class BeView: UIView {
    ...
    private let leftAvatarView = StillView()
    //: private let centerAvatarView = AvatarView()
    private let centerAvatarView = StillView()
    //: private let rightAvatarView = AvatarView()
    private let rightAvatarView = StillView()
    ...
    private func libraryCount() {
        ...
        centerBgView.image = UIImage.lastTo(name: (String(user_sincePath.suffix(6)) + "andow_" + show_zzDenyObserverKey.replacingOccurrences(of: "down", with: "u") + String(mainExistGravityName.prefix(6))))
    }

    //: private func updateAvatarDisplay() {
    private func anyExhibit() {
...
extension BeView {
    ...
    func errAvatars(avatars: [String]) {
...
class StillView: UIView {
    ...
    private func tinkle() {
        ...
        imageView.contactMoment(urlStr: self.imageurl ?? "")
    }
}
