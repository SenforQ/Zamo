...
class ToothReactiveCompatible: NSObject {

    //: var windownsPostion: CGPoint = CGPoint(x: ScreenWidth-4, y: ScreenHeight-TabBarViewHeight-66-38)   
    var windownsPostion: CGPoint = CGPoint(x: show_objectValue-4, y: constBlockIdentityContent-constSceneId-66-38)   // 窗口默认位置
    //: var advSuperView: UIView?                                                       
    var advSuperView: UIView?                                                       // 窗口容器
    // singleton
    //: static let shared = TalkingAdvertisingManager()
    static let shared = ToothReactiveCompatible()
...
extension ToothReactiveCompatible {
    /// 创建窗口最小化容器
    ...
    func distill() {
        ...
        let windowView = CoreContainerView()
        //: let keywindow = EverlastinglyReactiveCompatible.getWindow()
        let keywindow = EverlastinglyReactiveCompatible.insideWindow()
