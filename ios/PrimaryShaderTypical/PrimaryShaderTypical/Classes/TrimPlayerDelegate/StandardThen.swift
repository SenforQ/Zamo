...
/*: "/TXCache" :*/
fileprivate let show_coreId:String = "/TXCthanks healthy strong nothing offer"
...
enum PubliclyWouldPlayerStatus {
    //: case Unknown
    case Unknown // 未知
    //: case ReadyToPlay
...
enum FirstAppearanceRenderMode {
    //: case FILL_SCREEN
...
protocol DirectlyThen: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func guideStatus(player: StandardThen, status: PubliclyWouldPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func birthday(player: StandardThen, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func clueWithoutStride(player: StandardThen, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class StandardThen: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: DirectlyThen?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: PubliclyWouldPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = StandardThen()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(show_coreId.prefix(4)) + "ache")) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(situationRefuse), userInfo: nil, repeats: true)
...
extension StandardThen {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func gapCap(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: appTurnMessage.birthOrShape(), with: dataCancelMessage)
    ...
    @objc func situationRefuse() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.clueWithoutStride(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func floodStyleMannerRenderPoise(renderMode: FirstAppearanceRenderMode) {
    ...
    func detailBelow(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.accessible == true {
        if DisagreeableWomanSocketManager.shared.accessible == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func distanceManager() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.guideStatus(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func illegalize() -> Bool {
...
extension StandardThen: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.birthday(player: self, duration: Int(duration), currentTime: Int(currentTime))
