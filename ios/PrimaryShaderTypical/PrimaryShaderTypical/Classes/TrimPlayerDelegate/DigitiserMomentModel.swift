...
struct DigitiserMomentModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct PhotoMotivationMeasurable: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum EyebrowOutputStream: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct ModelType: HandyJSON {
    ...
    var status: OutsideBidirectionalCollection?
}

///
//: enum MomentVideoStatus: String {
enum OutsideBidirectionalCollection: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension DigitiserMomentModel {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func range(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(MarketReactiveCompatible.gestureBig(text: content!, size: CGSize(width: show_objectValue - 54, height: CGFloat(MAXFLOAT)), font: UIFont.landEnableSize(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == EyebrowOutputStream.Photo.rawValue || momentType == EyebrowOutputStream.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
