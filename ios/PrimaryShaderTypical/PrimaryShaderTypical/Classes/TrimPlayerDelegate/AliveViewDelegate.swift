...
fileprivate let user_particleMessage:[UInt8] = [0xe3,0xe4,0xe3,0xfe,0xa2,0xe9,0xe5,0xee,0xef,0xf8,0xb0,0xa3,0xaa,0xe2,0xeb,0xf9,0xaa,0xe4,0xe5,0xfe,0xaa,0xe8,0xef,0xef,0xe4,0xaa,0xe3,0xe7,0xfa,0xe6,0xef,0xe7,0xef,0xe4,0xfe,0xef,0xee]

private func maximumContain(unless num: UInt8) -> UInt8 {
    return num ^ 138
}

/*: "Save successfully" :*/
fileprivate let k_twentyPath:String = "Save accommodate draft"
fileprivate let dataPairFormat:String = "depthcc"
fileprivate let showHolderContent:String = "essprofessionallly"
...
class AliveViewDelegate: TopicViewController {
    ...
    init(momentModel: DigitiserMomentModel, index: Int, type: SortPhotoType) {
        ...
        if DriveReactiveCompatible.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(show_objectValue), y: 0)
    ...
    init(images: [UIImage], index: Int, type: SortPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_particleMessage.map{maximumContain(unless: $0)}, encoding: .utf8)!)
    }
...
//        }
    ...
    override func switchicialClick() {
        //: super.publishDeleteButtonClick()
        super.switchicialClick()
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: show_objectValue, height: constBlockIdentityContent)
        ...
        view.frame = CGRect(x: 0, y: 0, width: show_objectValue, height: constBlockIdentityContent)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(show_objectValue), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(show_objectValue), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(show_objectValue), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(MotivationReusableView.self, forCellWithReuseIdentifier: MotivationReusableView.className())
...
//    lazy var downloadBtn: EarthThen = {
//        let btn = EarthThen.init()
...
//        GatePermissionTool.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: PhotoMotivationMeasurable = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func mobileMatch(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.noExtra(showMsg: (String(k_twentyPath.prefix(5)) + dataPairFormat.replacingOccurrences(of: "depth", with: "su") + showHolderContent.replacingOccurrences(of: "professional", with: "fu")).localized)
                }
                //: } else {
            } else {
...
extension AliveViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func old() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func pop() {
        //: if  currenModel.sex==MentionTriggerAppManager.share.loginUserMode.sex {
        if currenModel.sex == MentionTriggerAppManager.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: MotivationReusableView = collectionView.dequeueReusableCell(withReuseIdentifier: MotivationReusableView.className(), for: indexPath) as! MotivationReusableView
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: PhotoMotivationMeasurable = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.child(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.commitAdmin(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.commitAdmin(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: MotivationReusableView = cell as! MotivationReusableView
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / show_objectValue)
        //: if myType == .normal {
        if myType == .normal {
